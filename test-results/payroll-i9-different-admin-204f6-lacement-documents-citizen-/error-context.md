...
- Name: payroll-i9-different.spec.ts >> admin signs different replacement documents (citizen)
...
Locator: getByRole('region', { name: 'Different-document replacement workspace', exact: true }).getByRole('status').filter({ hasText: 'Private draft saved' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('region', { name: 'Different-document replacement workspace', exact: true }).getByRole('status').filter({ hasText: 'Private draft saved' })
...
- group:
  ...
  - definition: 2026-09-13T07:29:32.160Z
  ...
  - list:
    ...
    - listitem:
      - button "I9-A1-copy-57474286.pdf"
  - region "Retained employer I-9 evidence":
    ...
    - group:
      - text: Current certification · cycle 1 · 2026-09-13T07:29:32.160Z
