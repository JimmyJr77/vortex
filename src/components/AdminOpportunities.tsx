...
import { Bot, ExternalLink, Mail, MapPin, Pencil, Plus, Search, Target, X } from 'lucide-react'
...
const blank: OpportunityDraft = { name: '', category: 'Community events', kind: 'event', status: 'new', priority: 'medium', location: '', eventDate: null, deadline: null, contactName: '', contactEmail: '', contactPhone: '', websiteUrl: '', sourceUrl: '', audience: '', supportOffer: '', edgeStrategy: '', notes: '' }
...
export default function AdminOpportunities() {
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-col gap-3 rounded-2xl border bg-white p-4 lg:flex-row lg:justify-between"><div className="flex flex-wrap gap-2">{['All', ...categories].map(c => <button key={c} onClick={() => setCategory(c)} className={`rounded-full px-3 py-2 text-xs font-semibold ${category === c ? 'bg-red-600 text-white' : 'bg-gray-100'}`}>{c}</button>)}</div><label className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-gray-400" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search" className="rounded-xl border py-2.5 pl-9 pr-3 text-sm" /></label></div>
    ...
    {loading ? <div className="p-10 text-center text-gray-500">Loading research…</div> : <div className="grid gap-4 xl:grid-cols-2">{visible.map(item => <article key={item.id} className="rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex justify-between gap-3"><div><div className="flex flex-wrap gap-2 text-[11px] font-bold uppercase"><span className="rounded-full bg-gray-100 px-2 py-1">{item.category}</span><span className="rounded-full bg-red-100 px-2 py-1 text-red-700">{item.priority}</span><span className="rounded-full bg-emerald-100 px-2 py-1 text-emerald-700">{item.status}</span></div><h3 className="mt-2 text-xl font-bold">{item.name}</h3></div><button onClick={() => setEditing(item)}><Pencil className="h-4 w-4 text-gray-500" /></button></div>
