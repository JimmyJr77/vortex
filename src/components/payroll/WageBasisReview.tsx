...
type EmployeeBasis={employeeId:string;employeeNumber:string;employeeName:string;finalizedRunCount:number;verifiedRunCount:number;socialSecurityWages:string|null;medicareWages:string|null;additionalMedicareWages:string|null;review:string;federalWages:string|null;marylandWages:string|null;incomeVerifiedRunCount:number;incomeReview:string;importedPaymentCount?:number;importedDetailStatus?:string|null;verifiedImportedPaymentCount?:number}
...
export default function WageBasisReview({start,end,valid}:{start:string;end:string;valid:boolean}){
 ...
 {rows.map(row=><article key={row.employeeId} className="space-y-2 rounded-xl border border-slate-200 p-3"><h4 className="break-words font-bold">{row.employeeName} · {row.employeeNumber}</h4><p>{row.verifiedRunCount} of {row.finalizedRunCount} finalized records have verified wage bases.</p>{row.importedPaymentCount?<p>{row.verifiedImportedPaymentCount||0} of {row.importedPaymentCount} imported payments have verified reporting detail. Status: {row.importedDetailStatus?.replaceAll('_',' ')||'Review required'}.</p>:null}
 {row.socialSecurityWages===null||row.medicareWages===null||row.additionalMedicareWages===null?<><p className="font-semibold text-amber-900">Wage totals unavailable — review required.</p><p className="break-words text-slate-600">{row.review}</p></>:<><p className="font-semibold text-emerald-800">{row.importedPaymentCount?'Native and imported wage bases reconcile.':'Finalized wage bases reconcile.'}</p><dl className="space-y-1"><div><dt className="inline">Social Security taxable wages: </dt><dd className="inline font-bold">{money(row.socialSecurityWages)}</dd></div><div><dt className="inline">Medicare taxable wages: </dt><dd className="inline font-bold">{money(row.medicareWages)}</dd></div><div><dt className="inline">Additional Medicare taxable wages: </dt><dd className="inline font-bold">{money(row.additionalMedicareWages)}</dd></div></dl></>}
