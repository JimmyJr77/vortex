...
export default function RetirementProcessingReview({planId}:{planId:string}){
 ...
 return <section aria-label={`Processing review ${planId}`} className="mt-4 space-y-3 rounded border border-slate-300 p-3">
  ...
  {data?<>{data.executionIssues?.length?<div role="alert" className="rounded border border-amber-300 bg-amber-50 p-3"><p className="font-bold">Payroll processing requirements</p><ul className="list-disc pl-5">{data.executionIssues.map(issue=><li key={issue}>{issue}</li>)}</ul></div>:null}<p>Current review: {data.status.replaceAll('_',' ')}</p><ul className="list-disc pl-5">{Object.keys(data.policies).map(key=><li key={key}>{policyLabels[key]||key}</li>)}</ul>
