...
import {workforceApi,type I9EmployerDocument,type I9EmployerDraft as Draft,type I9EmployerDraftState} from '../../utils/workforceApi'
...
export default function I9EmployerDraft({employeeId,taskId,cycle,editable}:{employeeId:number;taskId:number;cycle:number;editable:boolean}){
 ...
 const change=(next:Draft)=>{setDraft(next);setNotice('');pending.current=null}
 const save=async()=>{
  if(!state)return
 ...
 return <section aria-label="Employer I-9 draft" className="my-4 min-w-0 space-y-3 rounded-xl border border-slate-300 p-4">
  ...
  <button type="button" disabled={busy} className="underline" onClick={()=>{setState(null);setDraft(empty());setError('');setNotice('');setReload(n=>n+1)}}>Reload employer draft (replaces unsaved entries)</button>
