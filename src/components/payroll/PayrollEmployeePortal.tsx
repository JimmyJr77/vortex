...
import { CalendarDays, CheckCircle2, Clock3, ExternalLink, FileText, Loader2, LogOut, RefreshCw, ShieldCheck, UserRound, WalletCards } from 'lucide-react'
...
type PortalView = 'home' | 'time' | 'schedule' | 'pay' | 'onboarding'
...
export default function PayrollEmployeePortal() {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        if (!getPayrollEmployeeSession() && invite) {
          const redeemed = await employeePayrollApi.redeem(invite)
          if (cancelled) return
  ...
  const isClockedIn = useMemo(() => data?.timeEntries.some((entry) => !field(entry, 'clock_out')) ?? false, [data?.timeEntries])
  const currentEntry = useMemo(() => data?.timeEntries.find((entry) => !field(entry, 'clock_out')) ?? null, [data?.timeEntries])
  ...
  const logout = async () => {
    ...
    setData(null)
    setError('Your payroll session has ended. Use the most recent invitation link if you need another invitation.')
    setBusy(false)
  }
  ...
  return (
    <div className="min-h-screen bg-slate-100 pb-24">
      ...
      <main className="mx-auto max-w-6xl space-y-5 px-4 py-5 sm:px-6">
        <section className="rounded-3xl bg-gradient-to-br from-slate-950 to-slate-800 p-6 text-white shadow-xl sm:p-8"><div className="flex flex-wrap items-start justify-between gap-5"><div><p className="text-sm font-bold text-slate-300">{data.employee.jobTitle}</p><h1 className="mt-1 text-3xl font-black">Your work, schedule, and onboarding</h1><p className="mt-3 text-sm text-slate-300">{data.employee.primaryWorkLocation}</p></div><div className="rounded-2xl bg-white/10 px-5 py-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-300">Hourly rate</p><p className="mt-1 text-2xl font-black">{money(data.employee.hourlyRateCents)}</p></div></div></section>
        ...
        {view === 'pay' ? <Card title="My pay statements"><div className="space-y-4">{data.payStatements.map((statement) => { const gross = Number(field(statement, 'regular_pay_cents')) + Number(field(statement, 'overtime_pay_cents')) + Number(field(statement, 'other_taxable_pay_cents')); const taxes = Number(field(statement, 'federal_income_tax_cents')) + Number(field(statement, 'state_income_tax_cents')) + Number(field(statement, 'social_security_tax_cents')) + Number(field(statement, 'medicare_tax_cents')) + Number(field(statement, 'additional_medicare_tax_cents')); return <article key={String(field(statement, 'id'))} className="overflow-hidden rounded-2xl border border-slate-200"><div className="flex flex-wrap items-start justify-between gap-3 bg-slate-950 p-4 text-white"><div><p className="font-black">{String(field(statement, 'legal_business_name'))}</p><p className="mt-1 text-xs text-slate-300">{String(field(statement, 'business_address'))} · EIN ending {String(field(statement, 'ein_last4'))}</p></div><div className="text-right"><p className="text-xs text-slate-300">Pay date</p><p className="font-black">{dateOnlyText(field(statement, 'pay_date'))}</p></div></div><div className="p-4"><p className="text-xs font-bold text-slate-500">Period {dateOnlyText(field(statement, 'period_start'))} through {dateOnlyText(field(statement, 'period_end'))}</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-xs text-slate-500">Regular</p><p className="font-black">{(Number(field(statement, 'regular_minutes')) / 60).toFixed(2)} hr · {money(Number(field(statement, 'regular_pay_cents')))}</p></div><div><p className="text-xs text-slate-500">Overtime</p><p className="font-black">{(Number(field(statement, 'overtime_minutes')) / 60).toFixed(2)} hr · {money(Number(field(statement, 'overtime_pay_cents')))}</p></div><div><p className="text-xs text-slate-500">Rate</p><p className="font-black">{money(Number(field(statement, 'hourly_rate_cents')))}/hr</p></div><div><p className="text-xs text-slate-500">Additional taxable pay</p><p className="font-black">{money(Number(field(statement, 'other_taxable_pay_cents')))}</p></div><div><p className="text-xs text-slate-500">Gross wages</p><p className="font-black">{money(gross)}</p></div><div><p className="text-xs text-slate-500">Reimbursements</p><p className="font-black">{money(Number(field(statement, 'reimbursement_cents')))}</p></div><div><p className="text-xs text-slate-500">Taxes</p><p className="font-black">{money(taxes)}</p></div><div><p className="text-xs text-slate-500">Other deductions</p><p className="font-black">{money(Number(field(statement, 'other_deductions_cents')))}</p></div><div className="rounded-xl bg-emerald-50 p-3"><p className="text-xs text-emerald-700">Net pay</p><p className="text-lg font-black text-emerald-900">{money(Number(field(statement, 'net_pay_cents')))}</p></div></div></div></article> })}{data.payStatements.length === 0 ? <p className="py-8 text-center text-sm text-slate-500">No finalized pay statements yet.</p> : null}</div></Card> : null}

        {view === 'onboarding' ? <div className="grid gap-5 lg:grid-cols-2"><Card title="Onboarding status"><div className="space-y-3">{[['Federal W-4', data.employee.w4Status], ['Maryland MW507', data.employee.stateWithholdingStatus], ['Form I-9', data.employee.i9Status], ['Direct deposit', data.employee.directDepositStatus]].map(([label, status]) => <div key={label} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 p-3"><p className="font-bold text-slate-900">{label}</p><Pill status={status} /></div>)}</div><p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-900">Give completed forms and identity documents through the secure process designated by Vortex. Do not send them through ordinary email or paste identifying numbers into notes.</p></Card><Card title="Official materials"><div className="space-y-3">{data.onboardingMaterials.map((material) => <a key={material.key} href={material.url} target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-xl border border-slate-200 p-4 font-bold text-blue-800 hover:bg-blue-50">{material.label}<ExternalLink className="h-4 w-4" /></a>)}</div></Card></div> : null}
      </main>
      <button type="button" onClick={() => void load()} disabled={busy} aria-label="Refresh payroll portal" className="fixed bottom-5 right-5 rounded-full bg-white p-3 text-slate-700 shadow-lg ring-1 ring-slate-200 hover:bg-slate-50"><RefreshCw className={`h-5 w-5 ${busy ? 'animate-spin' : ''}`} /></button>
    </div>
  )
}
