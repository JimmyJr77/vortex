...
export default function W4PdfReview({pdfBase64,onDisplayed,formName='W-4',pageCount=5,sourceUrl=officialFormUrl}:{pdfBase64?:string;onDisplayed?:(page:number)=>Promise<void>;formName?:string;pageCount?:number;sourceUrl?:string}){
 ...
 useEffect(()=>{
  ...
  void import('pdfjs-dist').then(async({getDocument,GlobalWorkerOptions})=>{
   ...
   const task=getDocument(pdfBase64?{data:Uint8Array.from(atob(pdfBase64),c=>c.charCodeAt(0)),isEvalSupported:false}:{url:sourceUrl,isEvalSupported:false})
   dispose=()=>{void task.destroy().catch(()=>{})}
   const document=await task.promise
   if(document.numPages!==pageCount)throw new Error('Unexpected form page count')
   if(live)setPdf(document)
  }).catch(()=>{if(live)setError(`The ${formName} could not be displayed. Reload this step before signing.`)})
  return()=>{live=false;dispose?.()}
 },[pdfBase64,sourceUrl,formName,pageCount])
 useEffect(()=>{
