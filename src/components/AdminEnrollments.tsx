...
export default function AdminEnrollments() {
  ...
  const [allClassIterations, setAllClassIterations] = useState<ClassIteration[]>([])
        ...
        for (const athlete of athletes) {
          try {
            ...
            if (enrollmentsResponse.ok) {
              ...
              fetchedEnrollments.push(...athleteEnrollments.map((e: {
                id: number
                program_id: number
                days_per_week: number
                selected_days: string[] | string
  ...
  const fetchAllIterations = async () => {
    try {
      setIterationsLoading(true)
      const token = localStorage.getItem('adminToken')
      
      // Fetch iterations for all programs
      const allIterations: ClassIteration[] = []
      for (const program of programs) {
        try {
          ...
          console.error(`Error fetching iterations for program ${program.id}:`, error)
        }
      }
      
      setAllClassIterations(allIterations)
    } catch (error) {
  ...
  // Helper function to render a program's content
  const renderProgramContent = (program: Program) => {
    const programEnrollments = allEnrollments.filter(e => e.program_id === program.id)
    const programIterations = allClassIterations.filter(iter => iter.programId === program.id)
      ...
      <div key={program.id} className="space-y-6">
        ...
        {iterationsLoading ? (
          ...
          <div className="space-y-6">
            {programIterations.map((iteration) => {
              const iterationEnrollments = programEnrollments.filter(e => 
            ...
            {programEnrollments.filter(e => !e.iteration_id && !programIterations.some(iter => iter.iterationNumber === 1 && e.program_id === iter.programId)).length > 0 && (
              <div className="bg-white p-4 md:p-6 rounded-lg shadow-lg border border-gray-200">
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                  <div className="space-y-2">
                    {programEnrollments.filter(e => !e.iteration_id && !programIterations.some(iter => iter.iterationNumber === 1 && e.program_id === iter.programId)).map((enrollment) => {
                      const selectedDaysArray = Array.isArray(enrollment.selected_days) 
                        ? enrollment.selected_days 
                        : (typeof enrollment.selected_days === 'string' 
                            ? JSON.parse(enrollment.selected_days || '[]') 
  ...
  const programsToDisplay = selectedProgram ? [selectedProgram] : programs
        ...
        <ClassDropdown
          ...
          onChange={(_programId, program) => {
            ...
            if (!program && _programId) {
              ...
              // When _programId is null (empty string from dropdown), program will also be null
              setSelectedProgram(program)
            }
          }}
          programs={programs}
          filterActiveOnly={true}
          placeholder="View All Classes"
          className="w-full px-3 py-2 bg-white text-black rounded border border-gray-300"
        />
      </div>

      {/* Class Details and Enrollments - Show all or filtered */}
      {programsToDisplay.length > 0 ? (
        <div className="space-y-8">
          {programsToDisplay.map(program => renderProgramContent(program))}
        </div>
      ) : (
        <div className="bg-white p-4 md:p-6 rounded-lg shadow-lg border border-gray-200">
