...
export default function AthleticismAcceleratorPanel() {
  ...
  const sessions = selectedProgram ? getAcceleratorSessions(selectedProgram.id) : []
  const session = sessions[classIndex]
  const guidance = programId ? ACCELERATOR_PLAN_GUIDANCE[programId] : undefined
  const selectProgram = (id: AcceleratorProgramId | null) => { setProgramId(id); setClassIndex(0); setPhase('E'); setDetail(null) }
  const selectClass = (index: number) => { setClassIndex(index); setDetail(null) }

  return <div className="space-y-5 pb-5">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><p className="mb-1 text-[11px] font-bold uppercase tracking-[0.18em] text-gray-500">Session Design</p><h2 className="text-2xl font-bold tracking-tight text-gray-950">Athleticism Accelerator</h2>{!selectedProgram && <p className="mt-1 text-sm text-gray-500">A clear plan. A focused session.</p>}</div>{!selectedProgram && <span className="rounded-full border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-600">{ACCELERATOR_PROGRAMS.length} programs · 12–36 Classes</span>}</header>
      ...
      <div role="region" id={`accelerator-panel-${programId}`} aria-labelledby={`accelerator-title-${programId}`}>
        ...
        <section className="mt-4 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm" aria-label={`Class ${session.n} workout`}><div className="border-b border-gray-100 bg-gradient-to-r from-red-50/70 to-white px-4 py-4 sm:px-5"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2.5"><span className="rounded-md bg-vortex-red px-2 py-1 text-[11px] font-bold uppercase tracking-wide text-white">Class {session.n}</span><h4 className="font-bold text-gray-950">{session.title}</h4></div><button type="button" className={`flex items-center gap-1.5 rounded text-xs text-gray-600 hover:text-vortex-red ${FOCUS_RING}`} onClick={() => setDetail({ title: 'Timing & delivery', content: <DeliveryNotes session={session} /> })}><Clock3 className="h-3.5 w-3.5" /> {timeLabel(session)} <Info className="h-3.5 w-3.5" /></button></div><p className="mt-2 text-sm leading-relaxed text-gray-600">{session.effort}</p></div>
          <div className="grid xl:grid-cols-[minmax(0,1fr)_240px]"><aside className="border-b border-gray-100 bg-gray-50/60 p-4 xl:order-2 xl:border-b-0 xl:border-l" aria-label="Equipment for the day"><h5 className="mb-3 flex items-center gap-2 text-sm font-bold text-gray-900"><Package className="h-4 w-4 text-vortex-red" /> Equipment for the day</h5><ul className="flex flex-wrap gap-2 xl:flex-col">{session.equipment.length ? session.equipment.map((item) => <li key={item} className="flex items-center gap-2 rounded-lg border border-gray-200/80 bg-white px-2.5 py-2 text-xs font-medium text-gray-700"><Check className="h-3.5 w-3.5 shrink-0 text-gray-400" /> {item}</li>) : <li className="flex items-center gap-2 rounded-lg border border-gray-200/80 bg-white px-2.5 py-2 text-xs font-medium text-gray-700"><Check className="h-3.5 w-3.5 shrink-0 text-gray-400" /> Bodyweight</li>}<li className="flex items-center gap-2 rounded-lg border border-gray-200/80 bg-white px-2.5 py-2 text-xs font-medium text-gray-700"><Target className="h-3.5 w-3.5 shrink-0 text-gray-400" /> Clear working space</li></ul><button type="button" className={`mt-3 flex items-center gap-1.5 rounded text-xs font-semibold text-vortex-red hover:underline ${FOCUS_RING}`} onClick={() => setDetail({ title: 'Equipment & station setup', eyebrow: `Class ${session.n}`, content: <EquipmentNotes session={session} /> })}>Setup & substitutions <ArrowUpRight className="h-3.5 w-3.5" /></button><div className="mt-4 hidden border-t border-gray-200 pt-4 xl:block"><p className="text-[10px] font-bold uppercase tracking-wider text-gray-500">Today’s plan</p><div className="mt-2 flex gap-5"><div><p className="text-xl font-bold tabular-nums text-gray-900">14</p><p className="text-[11px] text-gray-500">exercises</p></div><div><p className="text-xl font-bold tabular-nums text-gray-900">6</p><p className="text-[11px] text-gray-500">primary lifts</p></div></div></div></aside>
            <div className="min-w-0 p-4 sm:p-5"><PhaseSelector selected={phase} onSelect={setPhase} /><div role="tabpanel" id={`accelerator-phase-panel-${phase}`} aria-labelledby={`accelerator-phase-${phase}`} className="mt-4">{phase === 'prepare' ? <PreparationView session={session} /> : <><PhaseBrief phase={phase} /><div className="mt-3 divide-y divide-gray-100">{session.exercises.filter((exercise) => exercise.id.startsWith(phase)).map((exercise, index) => <div key={exercise.id}>{phase === 'E' && index % 2 === 0 && session.connections?.[index / 2] && <ConnectionBrief connection={session.connections[index / 2]} />}<ExerciseRow exercise={exercise} anchor={index === 0 && phase !== 'S'} onOpen={() => setDetail({ title: exercise.name, eyebrow: `Class ${session.n} · ${exercise.id}`, content: <ExerciseNotes exercise={exercise} /> })} /></div>)}</div><div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-3"><button type="button" className={`flex items-center gap-1.5 rounded text-xs font-medium text-gray-500 hover:text-vortex-red ${FOCUS_RING}`} onClick={() => setDetail({ title: 'Coaching considerations', eyebrow: `Class ${session.n}`, content: <CoachingNotes session={session} phase={phase} /> })}><Info className="h-3.5 w-3.5" /> Quality, scaling & readiness</button>{phase !== 'P' && <button type="button" className={`flex items-center gap-1 rounded text-xs font-semibold text-vortex-red ${FOCUS_RING}`} onClick={() => setPhase(phase === 'E' ? 'S' : 'P')}>Next: {phase === 'E' ? 'Resilience' : 'Primary strength'} <ChevronRight className="h-4 w-4" /></button>}</div></>}</div></div>
          </div></section>
      </div>
...
function ProgramCards({ onSelect, search, onSearch, programType, onProgramTypeChange }: {
  ...
  const programsByType = ACCELERATOR_PROGRAMS.filter((program) => programType === 'all' || program.classCount === (programType === 'individual' ? 12 : 36))
  const query = search.trim().toLowerCase()
  const programs = programsByType.filter((program) => `${program.title} ${program.description} ${program.category}`.toLowerCase().includes(query))
  ...
  return <>
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <p className="text-xs text-gray-500">Focused courses and complete 36-class development plans</p>
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        <p role="status" className="text-xs text-gray-500">{programs.length} of {ACCELERATOR_PROGRAMS.length} programs</p>
    ...
    {programs.length ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      {programs.map((program) => {
        const Icon = PROGRAM_ICONS[program.icon]
        return <button key={program.id} type="button" onClick={() => onSelect(program.id)} aria-label={`${program.title} ${program.classCount} Classes`} className={`group flex flex-col rounded-xl border border-gray-200 bg-white p-4 text-left transition hover:border-vortex-red hover:shadow-sm ${FOCUS_RING} ${program.classCount === 36 ? 'border-red-300 bg-red-50/50' : ''}`}>
          <div className="mb-4 flex w-full items-center justify-between gap-3">
            <span className={`rounded-xl p-2.5 ${program.classCount === 36 ? 'bg-vortex-red text-white' : 'bg-gray-100 text-gray-600'}`}><Icon className={`h-5 w-5 ${program.icon === 'lower' ? 'rotate-180' : ''}`} /></span>
            <span className="text-[10px] font-bold uppercase tracking-wider text-vortex-red">View {program.classCount} classes</span>
          </div>
          <h4 className="text-base font-bold text-gray-950">{program.title} <span className="whitespace-nowrap text-sm font-medium text-gray-400">{program.classCount} Classes</span></h4>
          ...
          <div className="mt-4 flex w-full items-center justify-between border-t border-gray-200/70 pt-3 text-xs">
            <span className="font-semibold text-vortex-red">Open workout plan</span>
            <ArrowRight className="h-4 w-4 text-gray-400 transition-transform group-hover:translate-x-1 group-hover:text-vortex-red" />
          </div>
        </button>
      })}
    </div> : <div className="rounded-xl border border-dashed border-gray-200 bg-white px-5 py-10 text-center">
...
function ExerciseRow({ exercise, anchor, onOpen }: { exercise: AcceleratorExercise; anchor: boolean; onOpen: () => void }) { return <div className="flex items-start gap-3 py-2.5"><span className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-[10px] font-bold ${anchor ? 'bg-red-50 text-vortex-red' : 'bg-gray-100 text-gray-500'}`}>{exercise.id}</span><div className="grid min-w-0 flex-1 grid-cols-[minmax(0,1fr)_24px] gap-x-2 gap-y-1.5 sm:grid-cols-[minmax(0,1fr)_190px_24px]"><div className="min-w-0">{exercise.sourceTrack && <p className="mb-0.5 text-[10px] font-bold uppercase tracking-wide text-vortex-red">{exercise.sourceTrack} · Source class {exercise.sourceClass}</p>}<button type="button" onClick={onOpen} className={`rounded text-left text-sm font-semibold leading-snug text-gray-900 hover:text-vortex-red ${FOCUS_RING}`}>{exercise.name}</button><p className="mt-1 text-[11px] text-gray-500">{exercise.equipment.join(' · ')}{anchor ? ' · Main focus' : ''}</p></div><button type="button" onClick={onOpen} aria-label={`Coaching notes for ${exercise.name}`} className={`col-start-2 row-start-1 self-start rounded p-1 text-gray-400 hover:bg-red-50 hover:text-vortex-red sm:col-start-3 ${FOCUS_RING}`}><Info className="h-4 w-4" /></button><div className="col-start-1 row-start-2 flex flex-wrap items-center gap-x-3 gap-y-1 sm:col-start-2 sm:row-start-1 sm:block"><span className="inline-block rounded bg-gray-100 px-2 py-1 text-xs font-semibold text-gray-800">{exercise.prescription}</span><span className="flex items-start gap-1 text-[11px] text-gray-500 sm:mt-1"><Clock3 className="mt-0.5 h-3 w-3 shrink-0" /> {exercise.rest}</span></div></div></div> }
function PreparationView({ session }: { session: AcceleratorSession }) { return <div className="rounded-xl border border-dashed border-gray-200 px-4 py-6"><Activity className="mb-3 h-6 w-6 text-vortex-red" /><h5 className="font-semibold text-gray-900">Access & Prepare 1</h5><p className="mt-2 text-sm leading-relaxed text-gray-600">{session.preparation || 'Complete the existing predetermined sequence before explosive work.'}</p></div> }
