...
export const payrollApi = {
  ...
  previewOffCyclePto:(payPeriodId:number,paymentDate:string,offCyclePto:Record<string,unknown>)=>payrollRequest<{preview:{netPayCents:number|null;deductionCents:number;warnings:Array<{message:string}>;employees:Array<{retirementPlans?:unknown[];ptoStateWithholdingCalculation?:{baseIncomeTaxCents:number;additionalWithholdingCents:number;additionalAllocation?:{requestedAdditionalCents:number;committedAdditionalCents:number;period:{start:string;end:string}};stateIncomeTaxCents:number;taxableWagesCents:number;rateBasisPoints:number;localRate:number;sources:{table:string}};ptoStateWithholdingBasis?:{marylandHistory?:{reconciled:boolean;evidence:Array<{runId:string;employeeId:string;paymentDate:string;reconciled:boolean;marylandWagesCents:number|null;stateIncomeTaxCents:number|null;additionalWithholding?:{status:'VERIFIED'|'MISSING'|'INVALID';requestedAdditionalCents:number|null;appliedAdditionalCents:number|null}}> ;issues:Array<{runId:string;message:string}>};fingerprint:string;grossWagesCents:number;marylandWagesCents:number;pretaxCents:number;rothCents:number;federalIncomeTaxCents:number;ficaCents:number};retirementPtoProposals?:Array<{planId:string;planName:string;calculation:{pretaxCents:number;rothCents:number;totalCents:number;notAppliedReason:string|null};taxWages:{federalWagesCents:number;marylandWagesCents:number;socialSecurityWagesCents:number}}>} >}}>('/runs/preview',{method:'POST',body:JSON.stringify({payPeriodId,paymentDate,offCyclePto})}),
