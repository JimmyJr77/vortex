...
export type PromoCodeSourceType = 'discount' | 'free_pass'

export interface PromoCodeRegistryEntry {
  code: string
  sourceType: PromoCodeSourceType
  sourceId: number
  sourceName: string
...
export async function adminFetchPromoCodes(): Promise<PromoCodeRegistryEntry[]> {
  const res = await adminApiRequest('/api/admin/scheduling/promo-codes')
  return parseJson(res)
}
