...
export type I9DifferentContext={signatureId:string|number;sourceKind:'SECTION2'|'SUPPLEMENT_B';rowKey:string;dueOn:string;today:string;originalExaminedOn:string;retainedHiringContext:{attestationKind:string;eVerify:boolean};employerDefaults:{firstDayEmployed:string;businessName:string;businessAddress:string}}
export type I9DifferentPreview={attestation:string;reviewId:string|number;expiresAt:string;previewSha256:string;recordedOn:string;packet:Array<{documentKey:string;documentId?:string|number;sha256:string;pdfBase64:string;pageCount:number}>}
