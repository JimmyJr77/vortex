...
export const initDatabase = async () => {
  try {
    ...
    // The original catalog belongs in migration 002 only. Do not seed programs at
    // runtime: administrators are allowed to delete catalog rows, and boot-time
    // `WHERE NOT EXISTS` inserts would otherwise recreate them after every restart.
...
const startServer = async () => {
  ...
  if (process.env.RUN_MIGRATION_ONLY !== 'true') {
    ...
    server.listen(PORT, () => {
        ...
        startRetirementReplacementReceiptScheduler(pool)
