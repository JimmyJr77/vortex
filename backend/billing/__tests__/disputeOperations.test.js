...
import { listDisputeCases, syncDisputeCase, updateDisputeEvidence } from '../disputeOperations.js'
...
test('dispute reads and evidence mutations are facility scoped', async () => {
  ...
  assert.deepEqual(await listDisputeCases(pool, { facilityId: 8 }), [])
  await assert.rejects(
    updateDisputeEvidence(pool, {
      id: 99,
      evidenceStatus: 'ready',
      evidenceNote: 'Forged cross-facility update.',
      userId: 3,
      facilityId: 8,
    }),
    /open dispute case not found/i,
  )

  assert.deepEqual(calls[0].params, [8])
  assert.match(calls[0].sql, /f\.facility_id=\$1/)
  assert.equal(calls[1].params[4], 8)
  assert.match(calls[1].sql, /scoped_family\.facility_id=\$5/)
})
