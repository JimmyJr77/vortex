...
test('Bar Cast evidence separates amplitude-return casts from terminal handstands, circles, releases, and universal loading claims', () => {
  const registryDocument = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'source-registry.v1.json'),
    'utf8',
  ))
  ...
  const expectedSources = new Map([
    ['world_gymnastics_wag_cast_code_2025_2028', {
      url: 'https://www.gymnastics.sport/publicdir/rules/files/en_1.1.%20WAG%20Code%20of%20Points%202025-2028.pdf',
      kind: 'governing_body',
      publisher: 'World Gymnastics',
      minimumQuality: 94,
    }],
    ['usag_wag_development_optional_cast_rules_2025', {
      url: 'https://static.usagym.org/PDFs/Women/Rules/dpcop/revisedpages_080425_mini.pdf',
      kind: 'governing_body',
      publisher: 'USA Gymnastics',
      minimumQuality: 92,
    }],
    ['usag_wag_compulsory_cast_handstand_development_2023', {
      url: 'https://static.usagym.org/PDFs/Women/development/compulsory/2021/replacement_072323.pdf',
      kind: 'governing_body',
      publisher: 'USA Gymnastics',
      minimumQuality: 90,
    }],
    ['british_gymnastics_cast_amplitude_sequences_2026', {
      url: 'https://mybg.british-gymnastics.org/store/downloadPublic?f=e810bf6f-df1b-4828-957d-6ddafe28f4c6.pdf&t=email',
      kind: 'governing_body',
      publisher: 'British Gymnastics',
      minimumQuality: 84,
    }],
    ['gymnastics_upper_extremity_injuries_return_review', {
      url: 'https://pmc.ncbi.nlm.nih.gov/articles/PMC12088353/',
      kind: 'peer_reviewed_research',
      publisher: 'Journal of the Pediatric Orthopaedic Society of North America',
      minimumQuality: 90,
    }],
    ['young_gymnast_injury_pathology_retrospective', {
      url: 'https://pmc.ncbi.nlm.nih.gov/articles/PMC9955164/',
      kind: 'peer_reviewed_research',
      publisher: 'Children',
      minimumQuality: 82,
    }],
    ['uneven_bar_subject_specific_swing_model', {
      url: 'https://pubmed.ncbi.nlm.nih.gov/18930233/',
      kind: 'peer_reviewed_research',
  ...
  for (const [sourceKey, expected] of expectedSources) {
    const source = registryDocument.sources[sourceKey]
    assert.ok(source, `missing Bar Cast source ${sourceKey}`)
    assert.equal(source.url, expected.url)
    assert.equal(source.kind, expected.kind)
    assert.equal(source.publisher, expected.publisher)
    assert.ok(source.evidenceQuality >= expected.minimumQuality)
  ...
  assert.match(
    registryDocument.sources.world_gymnastics_wag_cast_code_2025_2028.title,
    /Code of Points 2025–2028/i,
  )
  assert.match(
    registryDocument.sources.usag_wag_compulsory_cast_handstand_development_2023.title,
    /Compulsory Replacement Pages 2023/i,
  )
  assert.match(
    registryDocument.sources.uneven_bar_subject_specific_swing_model.title,
    /subject-specific female gymnast model/i,
  )
})

test('Handstand Snap-Down evidence separates the inverted-start stick from a full Round-Off and registers every non-media source', () => {
  const registryDocument = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'source-registry.v1.json'),
    'utf8',
  ))
  ...
  const expectedSources = new Map([
    ['usag_wag_compulsory_roundoff_snapdown_2025', {
      url: 'https://static.usagym.org/PDFs/Women/development/compulsory/replacement_070125_mini.pdf',
      kind: 'governing_body',
      publisher: 'USA Gymnastics',
      minimumQuality: 92,
    }],
    ['safe_gymnastics_roundoff_technique', {
      url: 'https://is.muni.cz/do/fsps/e-learning/safe_gymnastics_4all/pages_en/floor_17_roundoff.html',
      kind: 'professional_standard',
      publisher: 'Masaryk University Faculty of Sports Studies',
      minimumQuality: 84,
    }],
    ['safe_gymnastics_didactic_guidelines', {
      url: 'https://is.muni.cz/do/fsps/e-learning/safe_gymnastics_4all/pages_en/didactic_guidelines.html',
      kind: 'professional_standard',
      publisher: 'Masaryk University Faculty of Sports Studies',
      minimumQuality: 82,
    }],
    ['special_olympics_handstand_snapdown_coaching', {
      url: 'https://special-olympics.be/wp-content/uploads/2022/08/specialolympics_artisticgymnastics.pdf',
      kind: 'governing_body',
      publisher: 'Special Olympics',
      minimumQuality: 80,
    }],
    ['canjump_back_bridge_foundations_manual', {
      url: 'https://www.gymbc.org/media/o4opcs3u/gbc-canjump-manual-1.pdf',
  ...
  for (const [sourceKey, expected] of expectedSources) {
    const source = registryDocument.sources[sourceKey]
    assert.ok(source, `missing Handstand Snap-Down source ${sourceKey}`)
    assert.equal(source.url, expected.url)
    assert.equal(source.kind, expected.kind)
    assert.equal(source.publisher, expected.publisher)
    assert.ok(source.evidenceQuality >= expected.minimumQuality)
  ...
  assert.match(
    registryDocument.sources.usag_wag_compulsory_roundoff_snapdown_2025.title,
    /Compulsory Replacement Pages, July 2025/i,
  )
  assert.match(
    registryDocument.sources.safe_gymnastics_roundoff_technique.title,
    /Roundoff/i,
  )
  assert.match(
    registryDocument.sources.special_olympics_handstand_snapdown_coaching.title,
    /Artistic Gymnastics Coaching Guide/i,
  )
})

test('Lache evidence separates two-bar catch, no-release Tap Swing, and two-foot Precision endpoints with scoped source limits', () => {
  const registryDocument = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'source-registry.v1.json'),
    'utf8',
  ))
  ...
  const expectedSources = new Map([
    ['world_gymnastics_parkour_age_group_bar_elements_2021', {
      url: 'https://www.gymnastics.sport/site/pages/education/agegroup-pk-manual-e.pdf',
      kind: 'governing_body',
      publisher: 'World Gymnastics / Fédération Internationale de Gymnastique',
      minimumQuality: 92,
    }],
    ['world_gymnastics_parkour_table_of_tricks_2025', {
      url: 'https://www.gymnastics.sport/publicdir/rules/files/en_1.1.1%20-%20PK%20Code%20of%20Points%202025-2028%20-%20Table%20of%20tricks%202025.pdf',
      kind: 'governing_body',
      publisher: 'World Gymnastics / Fédération Internationale de Gymnastique',
      minimumQuality: 94,
    }],
    ['urbanleap_parkour_lache_handbook', {
      url: 'https://assets.zyrosite.com/AR0yPVr0V2u089eJ/urban-leap-project---handbook_finalna-verzija-a41oVuj00q1YwJLx.pdf',
      kind: 'professional_standard',
      publisher: 'UrbanLeap Erasmus+ Sport Project',
      minimumQuality: 78,
    }],
    ['horizontal_bar_release_identification_isbs', {
      url: 'https://ojs.ub.uni-konstanz.de/cpa/article/view/2969',
      kind: 'peer_reviewed_research',
      publisher: 'International Society of Biomechanics in Sports Conference Proceedings',
      minimumQuality: 68,
    }],
    ['kovacs_release_regrasp_biomechanics', {
      url: 'https://doi.org/10.1016/j.jbiomech.2016.11.048',
      kind: 'peer_reviewed_research',
      publisher: 'Journal of Biomechanics',
      minimumQuality: 88,
    }],
    ['parkour_injury_risk_survey', {
      url: 'https://pubmed.ncbi.nlm.nih.gov/23860830/',
      kind: 'peer_reviewed_research',
      publisher: 'Sportverletzung Sportschaden',
      minimumQuality: 78,
    }],
  ])
  ...
  for (const [sourceKey, expected] of expectedSources) {
    const source = registryDocument.sources[sourceKey]
    assert.ok(source, `missing Lache-family source ${sourceKey}`)
    assert.equal(source.url, expected.url)
    assert.equal(source.kind, expected.kind)
    assert.equal(source.publisher, expected.publisher)
    assert.ok(source.evidenceQuality >= expected.minimumQuality)
  ...
  assert.match(
    registryDocument.sources.world_gymnastics_parkour_age_group_bar_elements_2021.title,
    /Parkour Age Group Development/i,
  )
  assert.match(
    registryDocument.sources.urbanleap_parkour_lache_handbook.title,
    /Parkour Trainer Handbook/i,
  )
  assert.match(
...
test('90/90 breathing research registers exact sources and compiles three uncontaminated 16-section packets with five candidates each', () => {
  const registryDocument = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'source-registry.v1.json'),
    'utf8',
  ))
  ...
  const expectedSources = new Map([
    ['fms_9090_breathing_lateral_expansion', {
      url: 'https://www.functionalmovement.com/Exercises/803/90_90_breathing_with_lateral_expansion',
      kind: 'expert_instruction',
      publisher: 'Functional Movement Systems',
      minimumQuality: 84,
    }],
    ['diaphragmatic_breathing_systematic_review_2026', {
      url: 'https://pubmed.ncbi.nlm.nih.gov/41482169/',
      kind: 'peer_reviewed_research',
      publisher: 'Complementary Therapies in Medicine',
      minimumQuality: 92,
    }],
    ['diaphragm_postural_function_mri', {
      url: 'https://pubmed.ncbi.nlm.nih.gov/20705944/',
      kind: 'peer_reviewed_research',
      publisher: 'Journal of Applied Physiology',
      minimumQuality: 86,
    }],
    ['boyle_9090_hip_lift_balloon', {
      url: 'https://pmc.ncbi.nlm.nih.gov/articles/PMC2971640/',
      kind: 'peer_reviewed_research',
      publisher: 'North American Journal of Sports Physical Therapy',
      minimumQuality: 74,
    }],
    ['acog_pregnancy_postpartum_exercise', {
  ...
  for (const [sourceKey, expected] of expectedSources) {
    const source = registryDocument.sources[sourceKey]
    assert.ok(source, `missing 90/90 breathing source ${sourceKey}`)
    assert.equal(source.url, expected.url)
    assert.equal(source.kind, expected.kind)
    assert.equal(source.publisher, expected.publisher)
    assert.ok(source.evidenceQuality >= expected.minimumQuality)
  ...
  const mixedBatch = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'batches', '9090-shin-box-foundations.v1.json'),
    'utf8',
  ))
  const reachCard = mixedBatch.cards.find((card) => card.slug === '9090-breathing-with-reach')
  assert.ok(reachCard)
  const reachEvidence = compileResearchEvidence(
    mixedBatch.sharedEvidence,
    reachCard.evidence,
    registryDocument.sources,
    { canonicalName: '90/90 Breathing with Reach' },
  )
  assert.equal(reachEvidence.length, REQUIRED_RESEARCH_SECTIONS.length)
  assert.deepEqual(
    new Set(reachEvidence.map((entry) => entry.sectionKey)),
    new Set(REQUIRED_RESEARCH_SECTIONS),
  )
  assert.equal(reachCard.mediaCandidates.length, 5)
  assert.equal(reachCard.mediaCandidates.every((candidate) => (
    candidate.linkStatus === 'healthy'
      && candidate.embeddingAllowed === true
      && candidate.externalVerification?.method === 'youtube_oembed'
  )), true)
  assert.equal(reachEvidence.some((entry) => /shin.box|get-up/i.test(
    `${entry.sourceTitle} ${entry.claims.join(' ')}`,
  )), false)
  assert.match(reachCard.assessmentSummary.programmingDecision, /Hip Reset source quarantined/i)
  assert.match(reachCard.assessmentSummary.programmingDecision, /distinct definitions/i)

  const breathingBatch = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'batches', '9090-breathing-family.v1.json'),
    'utf8',
  ))
  assert.equal(breathingBatch.researchVersion, '2026-08-02.89')
  assert.equal(breathingBatch.cards.length, 2)
  for (const cardSpec of breathingBatch.cards) {
    const { packet, validation } = buildResearchPacketFromBatch({
      facilityId: 1,
      researchVersion: breathingBatch.researchVersion,
      sharedEvidence: breathingBatch.sharedEvidence,
      sourceRegistry: registryDocument.sources,
      cardSpec,
      currentCard: {
        slug: cardSpec.slug,
        canonicalName: cardSpec.slug === '9090-hip-lift-with-ball-and-balloon'
          ? '90/90 Hip Lift with Ball and Balloon'
          : '90/90 Wall-Supported Breathing with Lateral Expansion',
        familyKey: cardSpec.slug,
    ...
    assert.equal(packet.alternateAssessments.length, 8)
    assert.equal(packet.assessmentSummary.proposedDifficulty.baseOverallDifficulty,
      Math.max(
        ...
        packet.assessmentSummary.proposedDifficulty.absoluteLoadDemand,
      ))
  }
})

test('Crocodile Breathing research preserves prone identity, explicit feedback variants, and human-gated media', () => {
  const registryDocument = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'source-registry.v1.json'),
    'utf8',
  ))
  ...
  const expectedSources = new Map([
    ['fms_crocodile_breathing_exercise', {
      url: 'https://www.functionalmovement.com/exercises/776/single_leg_chop_2_step_motion',
      kind: 'expert_instruction',
      publisher: 'Functional Movement Systems',
      minimumQuality: 86,
    }],
    ['fms_take_a_deep_breath_crocodile', {
      url: 'https://www.functionalmovement.com/articles/780/take_a_deep_breath',
      kind: 'expert_instruction',
      publisher: 'Functional Movement Systems',
      minimumQuality: 82,
    }],
    ['aliverti_prone_supine_chest_wall_2001', {
      url: 'https://doi.org/10.1114/1.1332084',
      kind: 'peer_reviewed_research',
  ...
  for (const [sourceKey, expected] of expectedSources) {
    const source = registryDocument.sources[sourceKey]
    assert.ok(source, `missing Crocodile Breathing source ${sourceKey}`)
    assert.equal(source.url, expected.url)
    assert.equal(source.kind, expected.kind)
    assert.equal(source.publisher, expected.publisher)
    assert.ok(source.evidenceQuality >= expected.minimumQuality)
  ...
  const batch = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'batches', 'crocodile-breathing.v1.json'),
    'utf8',
  ))
  assert.equal(batch.researchVersion, '2026-08-02.90')
  assert.equal(batch.cards.length, 1)
  const cardSpec = batch.cards[0]
  const { packet, validation } = buildResearchPacketFromBatch({
    facilityId: 1,
    ...
    currentCard: {
      slug: cardSpec.slug,
      canonicalName: 'Crocodile Breathing',
      familyKey: 'Breathing / Rib-Pelvis Organization',
      snapshot: { cardVersion: 1, status: 'review' },
    },
    mediaCandidates: [],
  })
  ...
  assert.equal(packet.alternateAssessments.some((alternate) => (
    alternate.name === 'Hooklying Diaphragmatic Breathing'
      && alternate.classification === 'new_definition'
  )), true)
  assert.equal(packet.assessmentSummary.proposedDifficulty.baseOverallDifficulty,
    Math.max(
...
test('Full-Body Joint CARs research preserves an exact composite flow and joint-specific boundaries', () => {
  const registryDocument = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'source-registry.v1.json'),
    'utf8',
  ))
  ...
  const expectedSources = new Map([
    ['ace_controlled_articular_rotations_2024', {
      kind: 'professional_standard', publisher: 'American Council on Exercise', minimumQuality: 88,
    }],
    ['kinstretch_starter_pack_cars', {
      kind: 'expert_instruction', publisher: 'Markow Training Systems', minimumQuality: 74,
    }],
    ['kinstretch_full_body_cars_followalong', {
      kind: 'expert_instruction', publisher: 'Kinstretch with Beard', minimumQuality: 72,
    }],
    ['nonspecific_neck_pain_cpg_2025', {
      kind: 'professional_standard', publisher: 'Deutsches Ärzteblatt International', minimumQuality: 94,
    }],
  ])
  for (const [sourceKey, expected] of expectedSources) {
    const source = registryDocument.sources[sourceKey]
    assert.ok(source, `missing Full-Body Joint CARs source ${sourceKey}`)
    assert.equal(source.kind, expected.kind)
    assert.equal(source.publisher, expected.publisher)
    assert.ok(source.evidenceQuality >= expected.minimumQuality)
  }
  ...
  const batch = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'batches', 'full-body-joint-cars-flow.v1.json'),
    'utf8',
  ))
  assert.equal(batch.researchVersion, '2026-08-02.91')
  assert.equal(batch.cards.length, 1)
  const cardSpec = batch.cards[0]
  const { packet, validation } = buildResearchPacketFromBatch({
    facilityId: 1,
    ...
    currentCard: {
      slug: cardSpec.slug,
      canonicalName: 'Full-Body Joint CARs Flow',
      familyKey: 'Joint scan',
      snapshot: { cardVersion: 1, status: 'review' },
    },
    mediaCandidates: [],
  })
  ...
  assert.match(packet.assessmentSummary.identity, /Vortex review contract/)
  assert.doesNotMatch(JSON.stringify(packet), /athleteSkill|proficiencyLevel|ageLevel/i)
})

test('Neck CARs research consolidates the tall-posture duplicate and preserves cervical task boundaries', () => {
  const registryDocument = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'source-registry.v1.json'),
    'utf8',
  ))
  ...
  const expectedSources = new Map([
    ['ace_controlled_articular_rotations_2024', {
      url: 'https://www.acefitness.org/continuing-education/certified/october-2024/8725/controlled-articular-rotations-shifting-mobility-into-high-gear/',
      kind: 'professional_standard', publisher: 'American Council on Exercise', minimumQuality: 88,
    }],
    ['kinstretch_starter_pack_cars', {
      url: 'https://markowtrainingsystems.com/wp-content/uploads/2020/02/2020KinstretchStarterPack.pdf',
      kind: 'expert_instruction', publisher: 'Markow Training Systems', minimumQuality: 74,
    }],
    ['nonspecific_neck_pain_cpg_2025', {
      url: 'https://pmc.ncbi.nlm.nih.gov/articles/PMC12620902/',
      kind: 'professional_standard', publisher: 'Deutsches Ärzteblatt International', minimumQuality: 94,
    }],
    ['cervical_rom_compensatory_strategies_2019', {
      url: 'https://pmc.ncbi.nlm.nih.gov/articles/PMC6341704/',
      kind: 'peer_reviewed_research', publisher: 'Chiropractic & Manual Therapies', minimumQuality: 86,
    }],
  ])
  for (const [sourceKey, expected] of expectedSources) {
    const source = registryDocument.sources[sourceKey]
    assert.ok(source, `missing Neck CARs source ${sourceKey}`)
    assert.equal(source.url, expected.url)
    assert.equal(source.kind, expected.kind)
    assert.equal(source.publisher, expected.publisher)
    assert.ok(source.evidenceQuality >= expected.minimumQuality)
  ...
  const batch = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'batches', 'neck-cars.v1.json'),
    'utf8',
  ))
  assert.equal(batch.researchVersion, '2026-08-02.92')
  assert.equal(batch.cards.length, 1)
  const cardSpec = batch.cards[0]
  const { packet, validation } = buildResearchPacketFromBatch({
    facilityId: 1,
  ...
  assert.equal(packet.alternateAssessments.some((alternate) => (
    alternate.name === 'Neck CARs with Tall Posture'
      && alternate.classification === 'same_identity'
      && alternate.distinguishingDimensions.sourceLegacyExerciseId === 897
  )), true)
  assert.equal(packet.alternateAssessments.some((alternate) => (
    alternate.name === 'Wall Cervical Rotation + Chin Nod'
      && alternate.classification === 'new_definition'
  )), true)
  assert.equal(packet.assessmentSummary.proposedDifficulty.baseOverallDifficulty,
    Math.max(
  ...
  assert.match(packet.assessmentSummary.identity, /chin nod or tall posture is setup\/quality/)
  assert.doesNotMatch(JSON.stringify(packet), /athleteSkill|proficiencyLevel|ageLevel/i)
})

test('Cat-Cow research consolidates the segmental duplicate and keeps support-changing alternates quarantined', () => {
  const registryDocument = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'source-registry.v1.json'),
    'utf8',
  ))
  ...
  const expectedSources = new Map([
    ...
    ['pan_imu_spine_rehabilitation_2024', {
      url: 'https://doi.org/10.1186/s12984-024-01366-1',
      kind: 'peer_reviewed_research',
      publisher: 'Journal of NeuroEngineering and Rehabilitation',
      minimumQuality: 84,
    }],
    ['nasm_beginner_cat_cow_program', {
      url: 'https://blog.nasm.org/workout-plan-for-beginners',
      kind: 'expert_instruction', publisher: 'National Academy of Sports Medicine', minimumQuality: 74,
    }],
    ['special_olympics_cat_cow_cooldown', {
      url: 'https://resources.specialolympics.org/sports-essentials/sports-and-coaching/warm-up-and-cool-down-videos/cool-down-cat-cow-stretch',
      kind: 'governing_body', publisher: 'Special Olympics', minimumQuality: 78,
    }],
  ])
  for (const [sourceKey, expected] of expectedSources) {
    const source = registryDocument.sources[sourceKey]
    assert.ok(source, `missing Cat-Cow source ${sourceKey}`)
    assert.equal(source.url, expected.url)
    assert.equal(source.kind, expected.kind)
    assert.equal(source.publisher, expected.publisher)
    assert.ok(source.evidenceQuality >= expected.minimumQuality)
  ...
  const batch = JSON.parse(readFileSync(
    path.join(RESEARCH_ROOT, 'batches', 'cat-cow.v1.json'),
    'utf8',
  ))
  assert.equal(batch.researchVersion, '2026-08-02.93')
  assert.equal(batch.cards.length, 1)
  const cardSpec = batch.cards[0]
  const { packet, validation } = buildResearchPacketFromBatch({
    facilityId: batch.facilityId,
  ...
  assert.equal(packet.alternateAssessments.some((alternate) => (
    alternate.name === 'Cat-Cow Segmental Wave'
      && alternate.classification === 'new_variant'
      && alternate.distinguishingDimensions.sourceLegacyExerciseId === 889
  )), true)
  assert.equal(packet.alternateAssessments.some((alternate) => (
    alternate.name === 'Quadruped Spinal Circles'
      && alternate.classification === 'new_definition'
