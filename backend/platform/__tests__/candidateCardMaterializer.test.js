...
const cleanInstallCandidateFixtures = [
  {
    ...
    forbiddenTaxonomy: ['"transition"', '"head"'],
    requiredToken: '"movementPatterns":["brace","squat"]',
  },
  {
    contract: path.join(
      repoRoot,
    ...
    forbiddenTaxonomy: ['"transition"'],
    requiredToken: '"archiveKey":"identity-quarantine-source-283"',
  },
]
...
test('clean-install candidate contracts retain controlled taxonomy and archived source references', () => {
  for (const fixture of cleanInstallCandidateFixtures) {
    ...
    assert.equal(migration.includes(fixture.requiredToken), true)
    for (const forbiddenToken of fixture.forbiddenTaxonomy) {
      assert.equal(migration.includes(forbiddenToken), false)
    }
  }
})
