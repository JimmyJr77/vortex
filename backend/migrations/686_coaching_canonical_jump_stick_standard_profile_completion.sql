-- Normalize planning fields for canonical-authored exact jump-to-stick candidates.
-- Estimates remain candidate-only; no approval, media verification, or publication is created.
DO $canonical_jump_stick_standard_profile_completion$
DECLARE
  migration_key CONSTANT TEXT := '686_coaching_canonical_jump_stick_standard_profile_completion';
  spec RECORD;
  definition_id_value UUID;
  prior_version INTEGER;
BEGIN
  FOR spec IN SELECT * FROM (VALUES
    ('bilateral-90-degree-rotational-broad-jump-to-stick'::TEXT, 'stationary-bilateral-forward-diagonal-broad-jump-90-degree-whole-body-turn-to-bilateral-stick'::TEXT, 1, 30, 56, 1, 36),
    ('opposite-leg-90-degree-rotational-bound-to-stick'::TEXT, 'stationary-opposite-leg-forward-diagonal-bound-90-degree-whole-body-turn-to-stick'::TEXT, 1, 35, 64, 1, 42),
    ('single-leg-forward-hop-to-stick'::TEXT, 'stationary-low-amplitude-forward-same-leg-stick'::TEXT, 1, 18, 42, 1, 24),
    ('single-leg-forward-hop-to-stick'::TEXT, 'stationary-moderate-distance-forward-same-leg-stick'::TEXT, 1, 22, 52, 1, 36),
    ('single-leg-vertical-hop-to-stick'::TEXT, 'stationary-low-to-moderate-vertical-same-leg-stick'::TEXT, 1, 16, 44, 1, 24)
  ...
  LOOP
    ...
    SET load_profile_json = COALESCE(load_profile_json, '{}'::jsonb) || jsonb_build_object(
          ...
          'landingContactsPerRep', 1, 'externalLoadMethod', 'bodyweight'
    ...
    IF NOT EXISTS (SELECT 1 FROM coaching.exercise_variant_v1 WHERE definition_id = definition_id_value
      ...
      RAISE EXCEPTION '% failed standardized profile completion for %', migration_key, spec.slug;
    END IF;
  ...
  END LOOP;
END;
$canonical_jump_stick_standard_profile_completion$;
