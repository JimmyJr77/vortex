-- Consolidate two names for the same lateral-shuffle-to-rotational
-- medicine-ball throw into one stable definition.
--
-- Survivor:
--   Shuffle-to-Rotational Medicine Ball Throw
--
-- Consolidated source:
--   Med Ball Shuffle-to-Rotation Throw
--
-- Both legacy cards describe a lateral shuffle/crow-hop into a two-hand
-- rotational medicine-ball projection. Their different names and seed scores
-- do not define different exercises. Neither source fixes throw-only versus
-- rebound-and-catch delivery, so both legacy baseline variants remain
-- archived, nonselectable provenance. This migration creates no review,
-- approval, publication, media verification, calibration approval, or
-- exercise proficiency level. IDEMPOTENT and fail-closed.

DO $$
DECLARE
  survivor_slug CONSTANT TEXT := 'shuffle-to-rotational-medicine-ball-throw';
  duplicate_slug CONSTANT TEXT := 'med-ball-shuffle-to-rotation-throw';
  survivor_id UUID;
  duplicate_id UUID;
...
BEGIN
  ...
  IF survivor_id IS NULL AND duplicate_id IS NOT NULL THEN
    RAISE EXCEPTION
      'Shuffle rotational-throw consolidation found active duplicate % without survivor %',
      duplicate_slug,
      survivor_slug;
  END IF;
  ...
  IF protected_records > 0 THEN
    RAISE EXCEPTION
      'Shuffle rotational-throw consolidation refused to override % protected records',
      protected_records;
  END IF;
  ...
  SELECT
    ...
    'Both cards describe a declared lateral shuffle or crow-hop into a controlled plant, whole-body rotational medicine-ball projection, and reset. Name order and inconsistent seed scores do not create distinct exercise identities.',
    jsonb_build_object(
      'match', 'lateral_shuffle_to_two_hand_rotational_medicine_ball_projection',
      'survivorSlug', survivor.slug,
      'resolvedSlug', duplicate.slug,
      'declaredApproach', 'lateral_shuffle_or_crow_hop',
      'declaredProjection', 'two_hand_rotational_medicine_ball_throw',
  ...
  SET definition_id = survivor_id,
      ...
      provenance_json = provenance_json || jsonb_build_object(
        ...
        'resolution', 'lateral_shuffle_to_two_hand_rotational_medicine_ball_projection',
        'declaredApproach', 'lateral_shuffle_or_crow_hop',
        'declaredReturnContract', 'unspecified'
      )
  ...
  SET aliases = ARRAY(
      ...
      provenance_json = survivor.provenance_json || jsonb_build_object(
        'identityResolution', 'lateral_shuffle_to_two_hand_rotational_medicine_ball_projection',
  ...
  SET status = 'quarantined',
      blocking_issues_json = blocking_issues_json || jsonb_build_array(
        jsonb_build_object(
          'code', 'shuffle_rotational_throw_identity_reaudit_required',
          'message', 'Re-run the canonical card audit after shuffle rotational-throw identity consolidation.'
        )
      ),
