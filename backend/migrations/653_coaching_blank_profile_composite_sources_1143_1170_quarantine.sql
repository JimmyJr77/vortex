-- These composite jump, step-up, throw, and slam cards lack all operational
-- requirements, load, fatigue, dosage, quality, and stop-rule profiles. They
-- remain traceable in lineage but are not eligible for automated selection.
DO $blank_profile_composite_sources_1143_1170_quarantine$
DECLARE source_id_value INTEGER; definition_id_value UUID;
BEGIN
  FOREACH source_id_value IN ARRAY ARRAY[1143,1144,1147,1158,1164,1166,1169,1170]
  LOOP
  ...
  END LOOP;
END;
$blank_profile_composite_sources_1143_1170_quarantine$;
