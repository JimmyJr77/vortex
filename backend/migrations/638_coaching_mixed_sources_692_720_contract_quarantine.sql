-- These legacy source cards are generic or compound and omit critical movement,
-- station, loading, dose, fatigue, and safety contracts. Existing exact sprint
-- and plyometric variants remain unchanged.
DO $mixed_sources_692_720_quarantine$
DECLARE source_id_value INTEGER; definition_id_value UUID;
BEGIN
  FOREACH source_id_value IN ARRAY ARRAY[692,693,694,695,696,697,700,701,703,704,705,710,711,712,713,715,718,720]
  LOOP
    ...
    UPDATE coaching.exercise_variant_v1 SET variant_key='identity-quarantine-source-'||source_id_value,display_name='Mixed Contract Quarantine — Source '||source_id_value,status='archived',requirements_json=jsonb_build_object('selectable',false,'identityQuarantine',true,'exerciseCardDoesNotClassifyParticipants',true,'unresolvedContract','exact_support_interface_start_orientation_action_sequence_range_load_or_assistance_dose_fatigue_and_stop_rule_are_unspecified'),difficulty_json='{}'::jsonb,load_profile_json='{}'::jsonb,fatigue_profile_json='{}'::jsonb,programming_profile_json=jsonb_build_object('selectable',false,'humanReviewRequired',true,'publicationQuarantined',true) WHERE definition_id=definition_id_value AND variant_key='baseline';
  ...
  END LOOP;
END;
$mixed_sources_692_720_quarantine$;
