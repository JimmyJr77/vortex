-- These source baselines contain only legacy difficulty fragments and no
-- requirements, load, fatigue, dosage, quality, or stop-rule profile. Preserve
-- their provenance, but prevent a generator from treating names as contracts.
DO $blank_profile_sources_995_1020_quarantine$
DECLARE source_id_value INTEGER; definition_id_value UUID;
BEGIN
  FOREACH source_id_value IN ARRAY ARRAY[995,998,999,1000,1013,1019,1020]
  LOOP
  ...
  END LOOP;
END;
$blank_profile_sources_995_1020_quarantine$;
