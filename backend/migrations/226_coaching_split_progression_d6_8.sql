...
CROSS JOIN (VALUES
  (
    ...
    'INTERMEDIATE', 11, 17, 4, 5, 15, 45, 60,
  ...
  (
    ...
    'INTERMEDIATE', 11, 17, 3, 6, 45, 45, 90,
    ...
    'Squat strength', 'capacity', 'squat_strength', 'squat_strength',
...
JOIN (VALUES
  ('heavy-med-ball-chest-pass-d7', 5, 6, 7, 11, 17, 'Progression from med-ball-chest-pass'),
  ('goblet-squat-tempo-d6', 5, 5, 6, 11, 17, 'Progression from goblet-squat'),
  ('kettlebell-deadlift-heavy-d7', 5, 6, 7, 11, 17, 'Progression from kettlebell-deadlift-trap-bar-deadlift'),
  ('reactive-broad-jump-d7', 6, 5, 7, 11, 17, 'Progression from standing-broad-jump')
...
SELECT e.id, sp.id, 6, 'secondary', v.order_slot, pos.order_index,
...
JOIN (VALUES
  ...
  ('kettlebell-deadlift-heavy-d7', 'capacity', 'hinge_strength', 4, 0),
  ('reactive-broad-jump-d7', 'output', 'loaded_drive_strength', 4, 2)
...
WHERE e.slug IN ('heavy-med-ball-chest-pass-d7', 'reactive-broad-jump-d7')
ON CONFLICT (exercise_id, facet_type, facet_id) DO UPDATE SET weight = GREATEST(coaching.exercise_tag.weight, EXCLUDED.weight);
