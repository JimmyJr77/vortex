-- Consolidate the abbreviated Single-Leg RDL definition into the full-name
-- Single-Leg Romanian Deadlift identity without losing legacy sources,
-- variants, delivery profiles, candidate research, or candidate media.
--
-- Human-reviewed records fail closed instead of being reassigned
-- automatically. The archived definition remains as an immutable provenance
-- target for the identity-resolution record and any duplicate candidate rows.
-- IDEMPOTENT.
...
BEGIN
  ...
  IF protected_records > 0 THEN
    RAISE EXCEPTION
      'Single-Leg RDL identity consolidation requires human review: % protected records',
      protected_records;
  END IF;
  ...
  SELECT
    ...
    'RDL is the established abbreviation for Romanian deadlift; both cards describe the same single-leg hinge identity. The full-name card remains canonical and the abbreviated name remains searchable.',
    jsonb_build_object(
      'match', 'reviewed_identity_equivalence',
      'survivor_slug', survivor.slug,
      'resolved_slug', duplicate.slug,
      'shared_aliases', ARRAY['Single Leg RDL', 'Single-Leg RDL']
    ),
  ...
  -- Candidate rows with new evidence are moved to the active identity. Exact
  -- duplicate rows stay attached to the archived definition for provenance.
  ...
  SET status = 'quarantined',
      blocking_issues_json = blocking_issues_json || jsonb_build_array(
        jsonb_build_object(
          ...
          'message', 'Re-run the canonical card audit after the Single-Leg RDL identity consolidation.'
        )
      ),
