-- Preserve the completed Cossack-family candidate values while making the
-- task-only physical-difficulty dimension explicit for canonical consumers.
-- Legacy unresolved/archived placeholders remain untouched.  Fail closed
-- rather than rewrite a reviewed or published family.

DO $cossack_squat_physical_difficulty_backfill$
DECLARE
...
BEGIN
  ...
  IF protected_count > 0 THEN
    RAISE EXCEPTION
      '535_coaching_cossack_squat_physical_difficulty_backfill refused % protected records',
      protected_count;
  END IF;
  ...
  IF invalid_count > 0 THEN
    RAISE EXCEPTION
      '535_coaching_cossack_squat_physical_difficulty_backfill found % invalid scored variants',
      invalid_count;
  END IF;
END;
$cossack_squat_physical_difficulty_backfill$;
