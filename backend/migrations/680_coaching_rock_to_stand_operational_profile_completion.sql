-- Complete standardized planning fields for the single exact review candidate.
-- Values are unapproved planning estimates and the card remains quarantined.
DO $rock_to_stand_operational_profile_completion$
DECLARE
  migration_key CONSTANT TEXT := '680_coaching_rock_to_stand_operational_profile_completion';
  definition_id_value UUID;
  prior_version INTEGER;
BEGIN
  SELECT id, card_version INTO definition_id_value, prior_version
  FROM coaching.exercise_definition_v1
  WHERE facility_id = 1 AND slug = 'rock-and-roll-to-stand' AND status = 'review';
  ...
  WHERE definition_id = definition_id_value
    AND variant_key = 'stable-floor-seated-tuck-upper-back-rock-tall-stand-no-hand'
    AND status = 'review';
  IF NOT EXISTS (
  ...
  WHERE definition_id = definition_id_value AND reviewed_card_version = prior_version AND review_status = 'candidate';
END;
$rock_to_stand_operational_profile_completion$;
