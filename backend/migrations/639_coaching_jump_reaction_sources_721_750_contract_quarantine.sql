-- Generated jump, reaction, and eccentric baselines below omit exact contact,
-- station, support, range, loading, dose, fatigue, and safety contracts.
DO $jump_reaction_sources_721_750_quarantine$
DECLARE source_id_value INTEGER; definition_id_value UUID;
BEGIN
  FOREACH source_id_value IN ARRAY ARRAY[721,722,723,724,727,728,729,730,731,734,736,737,739,740,741,742,743,747,750]
  LOOP
    ...
    UPDATE coaching.exercise_variant_v1 SET variant_key='identity-quarantine-source-'||source_id_value,display_name='Jump / Reaction Contract Quarantine — Source '||source_id_value,status='archived',requirements_json=jsonb_build_object('selectable',false,'identityQuarantine',true,'exerciseCardDoesNotClassifyParticipants',true,'unresolvedContract','exact_start_contact_or_support_action_sequence_range_load_or_assistance_dose_fatigue_and_stop_rule_are_unspecified'),difficulty_json='{}'::jsonb,load_profile_json='{}'::jsonb,fatigue_profile_json='{}'::jsonb,programming_profile_json=jsonb_build_object('selectable',false,'humanReviewRequired',true,'publicationQuarantined',true) WHERE definition_id=definition_id_value AND variant_key='baseline';
  ...
  END LOOP;
END;
$jump_reaction_sources_721_750_quarantine$;
