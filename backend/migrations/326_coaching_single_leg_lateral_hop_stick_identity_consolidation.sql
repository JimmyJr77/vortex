-- Consolidate the line-target wording into the stable same-leg lateral
-- hop-to-stick definition.
--
-- Survivor:
...
--
-- Emory explicitly describes the lateral line task as a single-leg start,
-- lateral line crossing, same-leg landing, and held finish. The line is an
-- environmental target rather than an identity boundary. The generic legacy
-- line source does not itself recover takeoff laterality, so its baseline
-- remains archived provenance. Continuous line hops and contralateral skater
-- bounds remain separate tasks. This migration creates no review, approval,
-- publication, media verification, calibration approval, or exercise skill
-- level. IDEMPOTENT and fail-closed.

DO $$
DECLARE
  survivor_slug CONSTANT TEXT := 'single-leg-lateral-hop-to-stick';
  duplicate_slug CONSTANT TEXT := 'lateral-line-hop-to-single-leg-stick';
  survivor_id UUID;
  duplicate_id UUID;
...
BEGIN
  ...
  IF survivor_id IS NULL AND duplicate_id IS NOT NULL THEN
    RAISE EXCEPTION
      'Single-leg lateral hop-stick consolidation found active duplicate % without survivor %',
      duplicate_slug,
      survivor_slug;
  END IF;
  ...
  IF protected_records > 0 THEN
    RAISE EXCEPTION
      'Single-leg lateral hop-stick consolidation refused to override % protected records',
      protected_records;
  END IF;
  ...
  SELECT
    ...
    'Both cards end a discrete lateral hop with a controlled single-leg stick. A floor line declares the target boundary but does not change takeoff, projection, landing, hold, or reset identity. Exact amplitude and intent belong to difficulty-scored variants; continuous line hopping and contralateral bounds remain separate.',
    jsonb_build_object(
      'match', 'ipsilateral_single_leg_lateral_hop_to_terminal_stick',
      'survivorSlug', survivor.slug,
      'resolvedSlug', duplicate.slug,
      'declaredProjection', 'lateral',
      'declaredLanding', 'single_leg_terminal_stick',
      'lineRole', 'environmental_target_boundary',
      'duplicateSourceTakeoffContract', 'unresolved',
      'continuousLineHopsSameIdentity', FALSE,
      'contralateralSkaterBoundSameIdentity', FALSE,
      'amplitudeRequiresDifficultyVariant', TRUE,
  ...
  SET definition_id = survivor_id,
      ...
      provenance_json = provenance_json || jsonb_build_object(
        ...
        'resolution', 'ipsilateral_single_leg_lateral_hop_to_terminal_stick',
        'sourceTarget', 'floor_line',
        'sourceTakeoffContract', 'unresolved',
        'sourceTerminalContract', 'single_leg_stick'
      )
  ...
  SET definition_id = survivor_id,
      variant_key = left(
        'legacy-ambiguous-takeoff-line-target-source-'
  ...
  SET aliases = ARRAY(
        ...
        FROM unnest(
          ...
          || ARRAY[
            ...
            'Single-Leg Lateral Line Jump with Hold',
            'Single-Leg Lateral Hop and Stick',
            'Lateral Single-Leg Hop to Stick'
          ]
        ) alias
  ...
  SET status = 'quarantined',
      blocking_issues_json = blocking_issues_json || jsonb_build_array(
        jsonb_build_object(
          'code', 'single_leg_lateral_hop_stick_identity_reaudit_required',
          'message', 'Re-run the canonical card audit after single-leg lateral hop-stick identity consolidation.'
        )
      ),
