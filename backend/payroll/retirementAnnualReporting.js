...
export async function retirementAnnualReporting(db,facility,employeeId){
 ...
 (SELECT count(*)::int FROM payroll_retirement_employer_run_ledger l WHERE l.facility_id=r.facility_id AND l.run_id=r.id AND l.employee_id=re.employee_id) AS employer_ledger_count
 ...
 const records=[];let pretax=0n,roth=0n,matching=0n,nonelective=0n
 for(const run of rows){
  const employees=(run.calculation_snapshot?.employees||[]).filter(e=>String(e.employeeId)===String(employeeId))
  const hasEvidence=run.retirement_ledger_count>0||run.employer_ledger_count>0||run.statement_snapshot?.retirement||employees.some(e=>e.retirementPlans?.length||e.employerRetirementPlans?.length||e.retirement401k||e.payItems?.some(i=>i.kind?.startsWith('RETIREMENT_')))
  ...
  if(employee.employerRetirementPlans?.some(p=>!employee.retirementPlans.some(e=>e.planId===p.planId)))throw fail('Annual employer contributions require matching employee plan evidence.')
  ...
  for(const entry of employee.retirementPlans){
   ...
   const employer=employee.employerRetirementPlans?.find(p=>p.planId===entry.planId)?.calculation.contribution.proposed
   const employerMatchingCents=employer?.matchingCents??0,employerNonelectiveCents=employer?.nonelectiveCents??0
   matching+=BigInt(employerMatchingCents);nonelective+=BigInt(employerNonelectiveCents)
   records.push({runId:String(run.id),paymentDate:run.pay_date,planId:entry.planId,planName:c.planName||entry.planId,ordinaryPretaxCents:c.ordinary.pretax,ordinaryRothCents:c.ordinary.roth,catchUpPretaxCents:c.catchUp.pretax,catchUpRothCents:c.catchUp.roth,employerMatchingCents,employerNonelectiveCents})
  }
 }
 return records.length?{year:2026,status:'RECONCILED',pretaxDeferrals:money(pretax),rothDeferrals:money(roth),hasEmployeeDeferrals:pretax+roth>0n,hasEmployerContributions:matching+nonelective>0n,employerMatching:money(matching),employerNonelective:money(nonelective),records}:null
}
