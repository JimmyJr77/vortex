...
export async function benefitContinuationSource(db,facility,employeeId,paymentDate){
 ...
 const issues=[],task=tasks.length===1?tasks[0]:null,authorization=benefitsDeductionProposal(employee,task,settings,today)
 ...
 const basis={version:1,facilityId:String(facility),employeeId:String(employeeId),paymentDate,month,employeeName:`${employee.legal_first_name} ${employee.legal_last_name}`,employmentStatus:employee.employment_status,hireDate:employee.hire_date,terminationDate:employee.termination_date,employmentPeriods:periods,onboardingCycle:task?.onboarding_cycle||null,authorization:authorization.saved,authorizationStatus:authorization.status,monthlyCents:proposal?.monthlyCents||0,coverage,issues}
