...
export function registerTaxElectionRoutes(app,pool) {
 ...
 app.patch('/api/admin/payroll/employees/:id/tax-elections',async(req,res)=>{
  ...
  try {
   ...
   try {calculateWithholding2026({grossPayCents:200000,paymentDate:[nativeW4?.effectiveOn,nativeMW507?.receivedOn].filter(Boolean).sort().at(-1),election:{...elections,verified:true},payFrequency:settings.pay_frequency,year:2026,workState:employee.work_state,residenceState:employee.residence_state})}
