...
async function jobs(db,facility,runId){return (await db.query(`SELECT j.*,m.details AS mapping_details,c.journal_id AS claimed,v.result,
 COALESCE((SELECT jsonb_agg(jsonb_build_object('id',o.id::text,'source',o.source,'result',o.result,'create_attempted',o.create_attempted,'retry_id',o.retry_id) ORDER BY o.id) FROM payroll_settlement_journal_observation o WHERE o.journal_id=j.id),'[]'::jsonb) AS observations,
 COALESCE((SELECT jsonb_agg(jsonb_build_object('id',r.id,'reference',r.reference,'priorSubmissionId',r.prior_submission_id::text) ORDER BY r.created_at,r.id) FROM payroll_settlement_unsent_retry r WHERE r.journal_id=j.id),'[]'::jsonb) AS retries FROM payroll_settlement_journal j JOIN payroll_payment_accounting_mapping m ON m.id=j.mapping_id LEFT JOIN payroll_settlement_journal_claim c ON c.journal_id=j.id LEFT JOIN LATERAL(SELECT result FROM payroll_settlement_journal_observation WHERE journal_id=j.id ORDER BY id DESC LIMIT 1)v ON true WHERE j.facility_id=$1 AND j.payroll_run_id=$2 ORDER BY j.created_at,j.id`,[facility,runId])).rows}
export async function settlementPostingPlan(db,facility,runId){
 ...
 const retryStates=new Map(saved.map(job=>{
  const proof=settlementUnsentProof(job.observations,job.retries),item=items.find(i=>i.key===job.event_key)
  const currentSource=!issues.length&&item&&!item.issues.length&&current&&Number(job.mapping_id)===Number(current.id)&&job.realm_id===destination?.realmId&&job.environment===destination?.environment&&Number(job.payroll_journal_id)===Number(gross?.id)&&journalMatches(item.payload,job.payload)
  return [job.id,{...proof,canRetry:proof.canRetry&&!!currentSource,reason:proof.reason||(!currentSource?'Restore the original current accounts, payroll and bank evidence before retrying.':null)}]
 }))
 const data={issues,items,mapping:current?{id:Number(current.id),bank:current.details.bank,clearing:current.details.clearing}:null,destination,payrollJournalId:gross?Number(gross.id):null,jobs:saved.map(j=>({id:j.id,eventKey:j.event_key,kind:j.event.kind,postedDate:j.event.postedDate,sourceKind:j.event.sourceKind,employeeName:j.event.employeeName,amountCents:j.event.amountCents,bank:j.mapping_details.bank,clearing:j.mapping_details.clearing,realmId:j.realm_id,environment:j.environment,status:j.result?.status||'UNCERTAIN',journalId:j.result?.journalId||null,payload:j.payload,reference:j.reference,retry:retryStates.get(j.id),retries:j.retries}))}
...
export async function postSettlementJournal(pool,facility,runId,body,{actorId,fetcher=fetch,automationId=null,automationCheckId=null}={}){
 ...
 try{
  if(!Number.isSafeInteger(runId)||runId<=0)throw fail('Choose a valid payroll run.',400)
  if(!['SUBMIT','RECOVER','RETRY_UNSENT'].includes(body?.action))throw fail('Choose posting or recovery.',400)
  ...
  const retryRequested=body.action==='RETRY_UNSENT';let retryId=null
  let job=['RECOVER','RETRY_UNSENT'].includes(body.action)?saved.find(j=>j.id===body.jobId):saved.find(j=>j.event_key===body.eventKey&&j.realm_id===connection.realm_id&&j.environment===connection.environment)
  let recovery=!!job
  if(['RECOVER','RETRY_UNSENT'].includes(body.action)&&!job)throw fail('Saved settlement journal not found.',404)
  ...
  if(!job||retryRequested){
   ...
   const plan=await settlementPostingPlan(db,facility,runId),item=plan.items.find(i=>i.key===(retryRequested?job.event_key:body.eventKey))
   const retryState=retryRequested?plan.jobs.find(j=>j.id===job.id)?.retry:null
   if(plan.fingerprint!==body.fingerprint||(!retryRequested&&!item?.canPost)||(retryRequested&&(!retryState?.canRetry||body.observationId!==retryState.observationId||actorId==null)))throw fail('Settlement inputs changed or need review. Refresh the posting plan.')
   ...
   if(retryRequested){
    // Absence alone is insufficient: the scoped plan has already proved every
    // original/retry submission completed without attempting a create request.
    const absent=await resolveSettlementJournal(job,(path,options={})=>quickbooksRequest(db,connection,path,{...options,fetcher}))
    const observation=(await db.query("INSERT INTO payroll_settlement_journal_observation(journal_id,source,result,create_attempted) VALUES($1,'RECOVERY',$2,false) RETURNING id",[job.id,absent])).rows[0]
    if(absent.status!=='NOT_FOUND'){
     await db.query("INSERT INTO payroll_audit_log(facility_id,actor_user_id,action,entity_type,entity_id,after_data) VALUES($1,$2,'SETTLEMENT_RETRY_NOT_STARTED','settlement_journal',$3,$4)",[facility,actorId,job.id,{result:absent,reference:body.reference.trim(),createAttempted:false}])
     if(absent.status==='SYNCED')await db.query("UPDATE payroll_alert SET status='DISMISSED',dismissed_at=now() WHERE facility_id=$1 AND dedupe_key=$2 AND status='OPEN'",[facility,`settlement-journal-${job.id}`])
     await refreshPaymentReturnCases(db,facility,runId);await refreshCheckStopCases(db,facility,runId)
     await db.query('COMMIT');return {jobId:job.id,recovery:true,result:absent}
    }
    retryId=randomUUID()
    await db.query('INSERT INTO payroll_settlement_unsent_retry(id,journal_id,prior_submission_id,absence_observation_id,reference,created_by) VALUES($1,$2,$3,$4,$5,$6)',[retryId,job.id,retryState.observationId,observation.id,body.reference.trim(),actorId])
    recovery=false
   }else{
  ...
  if(!current||current.realm_id!==job.realm_id||current.environment!==job.environment)result={status:'BLOCKED_CONFIGURATION'}
   ...
   result=available?await resolveSettlementJournal(job,(path,options={})=>quickbooksRequest(db,current,path,{...options,fetcher}),{allowCreate:!recovery,onCreateAttempt:()=>{createAttempted=true}}):{status:'BLOCKED_CONFIGURATION'}
  }
  await db.query('INSERT INTO payroll_settlement_journal_observation(journal_id,source,result,create_attempted,retry_id) VALUES($1,$2,$3,$4,$5)',[job.id,recovery?'RECOVERY':'SUBMISSION',result,createAttempted,retryId])
  await db.query("INSERT INTO payroll_audit_log(facility_id,actor_user_id,action,entity_type,entity_id,after_data) VALUES($1,$2,'SETTLEMENT_JOURNAL_CHECKED','settlement_journal',$3,$4)",[facility,actorId,job.id,{source:recovery?'RECOVERY':'SUBMISSION',...result,retryId,createAttempted,automatic:actorId==null,automationId:job.automation_id?Number(job.automation_id):null}])
  ...
  await db.query('COMMIT');return {jobId:job.id,recovery,result,...(retryId?{retryId}:{})}
