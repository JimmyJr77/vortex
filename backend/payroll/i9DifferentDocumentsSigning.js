...
export async function signI9DifferentDocuments(db,ctx,taskId,body){
 ...
 const context={...review.current.retained.context,...review.current.currentHiringContext,today:clock.today,dueOn:root.due_on,originalExaminedOn:review.current.retained.examination.examinedOn}
 ...
 await db.query("UPDATE payroll_compliance_task SET status='COMPLETE',completion_note=$1,completed_at=$2,completed_by=$3,last_verified_on=$4,next_review_on=NULL,updated_at=clock_timestamp() WHERE id=$5",[`Signed new Section 2 ${signed.id} retained with original I-9 and examination findings.`,clock.signed_at,ctx.admin,clock.today,task.id])
 await db.query("INSERT INTO payroll_audit_log(facility_id,actor_user_id,action,entity_type,entity_id,after_data) VALUES($1,$2,'I9_DIFFERENT_SIGNED','i9_different_signature',$3,$4)",[ctx.facility,ctx.admin,String(signed.id),{employeeId:ctx.employee,complianceTaskId:task.id,documentId:document.id,reviewId:review.row.id,sourceSignatureId:root.id,documentSha256:hash(pdf)}])
