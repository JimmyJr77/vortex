...
import {allocationFields,employerAllocationFields} from './retirementAllocationFormat.js'
const fail=message=>Object.assign(new Error(message),{status:409})
export const retirementReceiptFields=['sourceFileName','sourceSha256','batchId',...allocationFields,'status','recordedAt']
const employeeCategories=['ordinaryPretaxCents','ordinaryRothCents','catchUpPretaxCents','catchUpRothCents']
...
export function retirementReceiptContractInput(b){
 ...
 if(!['CENTS','DOLLARS'].includes(b.amountFormat)||!['ISO','US'].includes(b.dateFormat)||!Array.isArray(b.columns)||![retirementReceiptFields.length,retirementReceiptFields.length+employerAllocationFields.length].includes(b.columns.length))throw fail('Review every receipt column and its exact amount and date formats.')
 const fields=new Set(),headers=new Set(),columns=b.columns.map(c=>{if(![...retirementReceiptFields,...employerAllocationFields].includes(c?.field)||fields.has(c.field)||typeof c.header!=='string'||!/^[A-Za-z][A-Za-z0-9 _().-]{0,79}$/.test(c.header)||headers.has(c.header.toLowerCase()))throw fail('Receipt fields and headers must be unique and complete.');fields.add(c.field);headers.add(c.header.toLowerCase());return {field:c.field,header:c.header}})
 const employerIncluded=columns.length>retirementReceiptFields.length
 if(retirementReceiptFields.some(field=>!fields.has(field))||employerIncluded&&employerAllocationFields.some(field=>!fields.has(field)))throw fail('Receipt columns must include all employee fields and both employer contribution categories.')
 if(employerIncluded&&b.employerContributionsConfirmed!==true||!employerIncluded&&b.employerContributionsConfirmed===true)throw fail('Independently confirm employer matching and nonelective receipt categories with both reviewed columns.')
 ...
 return {version:employerIncluded?3:b.participantReversalConfirmed===true?2:1,...(employerIncluded?{employerContributionsConfirmed:true}:{}),...(b.participantReversalConfirmed===true?{participantReversalConfirmed:true}:{}),confirmed:true,sourceHashConfirmed:true,cumulativeAmountsConfirmed:true,participantPostingConfirmed:true,reference:b.reference.trim(),columns,statusValues,amountFormat:b.amountFormat,dateFormat:b.dateFormat,encoding:'UTF-8',delimiter:',',includeHeader:true}
}
...
const amounts=(row,format,categories)=>{const values=Object.fromEntries([...categories,'totalCents'].map(k=>[k,cents(row[k],format)]));if(sum(categories.map(k=>values[k]))!==values.totalCents)throw fail('Receipt contribution categories do not reconcile to the total.');return values}
...
export function reconcileRetirementAllocationReceipt({receiptBytes,contract,allocationBytes,basis,fileName,claimedAt,now=new Date()}){
 ...
 const employerIncluded=reviewed.employerContributionsConfirmed===true
 const expectedFields=[...allocationFields,...(employerIncluded?employerAllocationFields:[])]
 if(!Array.isArray(basis.columns)||basis.columns.length!==expectedFields.length||expectedFields.some(f=>!basis.columns.some(c=>c.field===f)))throw fail('Original allocation columns are incomplete or differ from the reviewed receipt categories.')
 const categories=[...employeeCategories,...(employerIncluded?employerAllocationFields:[])]
 ...
 for(let i=0;i<sourceRows.length;i++){
  const row=sourceRows[i],a=basis.allocations[i],employeeId=typeof a?.employeeId==='string'?a.employeeId:Number.isSafeInteger(a?.employeeId)?String(a.employeeId):'',value=amounts(row,basis.amountFormat,categories),withheldDate=date(row.withheldDate,basis.dateFormat)
 ...
 for(const row of receiptRows){
  ...
  batchId=row.batchId;const participant=original.get(key(row)),status=receiptStatuses(reviewed).find(s=>reviewed.statusValues[s]===row.status),reported=amounts(row,reviewed.amountFormat,categories)
