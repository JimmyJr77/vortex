...
export async function prepareRetirementSettlementRelease(db,facility,id,{fetcher=fetch}={}){
 ...
 if((await db.query('SELECT authorization_id FROM payroll_retirement_settlement_cancellation WHERE authorization_id=$1',[id])).rowCount)throw fail('This settlement authorization is already cancelled.')
 ...
 for(const job of jobs)job.observations=(await db.query('SELECT id,source,create_attempted,result FROM payroll_retirement_settlement_observation WHERE journal_id=$1 ORDER BY id',[job.id])).rows
...
export function registerRetirementSettlementReleasePreview(app,pool,options){
 app.post('/api/admin/payroll/retirement-settlement-authorizations/:id/release-preview',async(req,res)=>{
  ...
  try{
   ...
   const data=await prepareRetirementSettlementRelease(db,facility,req.params.id,options)
