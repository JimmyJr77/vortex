import {readI9Qualification,saveI9Qualification} from './i9Qualification.js'
import {readI9DifferentSupplementDraft,saveI9DifferentSupplementDraft} from './i9DifferentSupplementDraft.js'
import {signI9DifferentSupplement} from './i9DifferentSupplementSigning.js'
import {listI9DifferentSupplementCopies,uploadI9DifferentSupplementCopy,viewI9DifferentSupplementCopy,recordI9DifferentSupplementCopyPage} from './i9DifferentSupplementCopies.js'
...
import {listI9DifferentDocumentsCopies,uploadI9DifferentDocumentsCopy,viewI9DifferentDocumentsCopy,recordI9DifferentDocumentsCopyPage} from './i9DifferentDocumentsCopies.js'
...
export function registerWorkforceAdminRoutes(app,pool) {
 ...
 app.get('/api/admin/payroll/employees/:id/i9/different-documents/:taskId/copies',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>listI9DifferentDocumentsCopies(db,context(req),req.params.taskId,req.query))})
 app.post('/api/admin/payroll/employees/:id/i9/different-documents/:taskId/copies',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>uploadI9DifferentDocumentsCopy(db,context(req),req.params.taskId,req.body||{}))})
 ...
 app.post('/api/admin/payroll/employees/:id/i9/different-supplement/:taskId/copies/list',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>listI9DifferentSupplementCopies(db,context(req),req.params.taskId,req.body||{}))})
 app.post('/api/admin/payroll/employees/:id/i9/different-supplement/:taskId/copies',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>uploadI9DifferentSupplementCopy(db,context(req),req.params.taskId,req.body||{}))})
 app.post('/api/admin/payroll/employees/:id/i9/different-supplement/:taskId/copy',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>viewI9DifferentSupplementCopy(db,context(req),req.params.taskId,req.body||{}))})
 app.post('/api/admin/payroll/employees/:id/i9/different-supplement/:taskId/copy-page',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>recordI9DifferentSupplementCopyPage(db,context(req),req.params.taskId,req.body||{}))})
 app.get('/api/admin/payroll/i9/qualification',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>readI9Qualification(db,context(req)))})
 app.post('/api/admin/payroll/i9/qualification',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>saveI9Qualification(db,context(req),req.body||{}))})
 app.post('/api/admin/payroll/employees/:id/i9/different-supplement/:taskId/sign',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>signI9DifferentSupplement(db,context(req),req.params.taskId,req.body||{}))})
 app.get('/api/admin/payroll/employees/:id/i9/different-supplement/:taskId/draft',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>readI9DifferentSupplementDraft(db,context(req),req.params.taskId))})
 app.post('/api/admin/payroll/employees/:id/i9/different-supplement/:taskId/draft',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>saveI9DifferentSupplementDraft(db,context(req),req.params.taskId,req.body||{}))})
 ...
 app.get('/api/admin/payroll/employees/:id/i9/different-documents/:taskId/context',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>i9DifferentDocumentsContext(db,context(req),req.params.taskId))})
 ...
 app.post('/api/admin/payroll/employees/:id/i9/receipt/:taskId/sign',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>signI9Receipt(db,context(req),req.params.taskId,req.body||{}))})
 app.get('/api/admin/payroll/employees/:id/i9/receipt/:taskId/copies',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>listI9ReceiptCopies(db,context(req),req.params.taskId,req.query))})
 app.post('/api/admin/payroll/employees/:id/i9/receipt/:taskId/copies',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>uploadI9ReceiptCopy(db,context(req),req.params.taskId,req.body||{}))})
 app.post('/api/admin/payroll/employees/:id/i9/receipt/:taskId/copy',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>viewI9ReceiptCopy(db,context(req),req.params.taskId,req.body||{}))})
 app.post('/api/admin/payroll/employees/:id/i9/receipt/:taskId/copy-page',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>recordI9ReceiptCopyPage(db,context(req),req.params.taskId,req.body||{}))})
 ...
 app.post('/api/admin/payroll/employees/:id/onboarding/:taskId/i9/employer-draft',(req,res)=>{res.setHeader('Cache-Control','no-store');return transaction(pool,res,db=>saveI9EmployerDraft(db,context(req),req.params.taskId,req.body||{}))})
