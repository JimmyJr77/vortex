...
export async function runRetirementReplacementBankSweep(pool,{facility=null,fetcher=fetch,paymentFetcher=fetch,reader,transfer,now=new Date(),dispatchNow=()=>new Date()}={}){
 ...
 try{
  locked=(await lock.query("SELECT pg_try_advisory_lock(hashtextextended('retirement-replacement-bank-sweep',0)) AS locked")).rows[0].locked
  if(!locked)return {attempted:0,verified:0,skipped:true}
  const candidates=(await pool.query(`WITH candidates AS (${replacementBankCandidates}) SELECT * FROM candidates WHERE ($1::bigint IS NULL OR facility_id=$1) AND auto_process AND NOT cancelled AND (claimed OR file_ready) AND (checked_at IS NULL OR checked_at<=$2::timestamptz-CASE WHEN status='BANK_POSTED' THEN interval '24 hours' ELSE interval '5 minutes' END) ORDER BY checked_at NULLS FIRST,id LIMIT 10`,[facility,timestamp])).rows
  let attempted=0,verified=0
  for(const row of candidates){
   let status,message
   try{
    const outcome=await dispatchRetirementReplacementBank(pool,Number(row.facility_id),row.id,{fetcher,paymentFetcher,reader,transfer,now:dispatchNow,automaticAt:timestamp})
    if(outcome.skipped)continue
    status=outcome.result.status==='COMPLETED'&&outcome.result.settlementStatus==='BANK_POSTED'?'BANK_POSTED':outcome.result.status;message=status==='BANK_POSTED'?'Replacement bank withdrawal matches the retained instruction. Participant credit and accounting require separate evidence.':'Recover or review the retained replacement bank outcome.'
    if(status==='BANK_POSTED')verified++
   }catch(e){status='BLOCKED';message=e.status?e.message:'Replacement bank configuration or evidence requires review.'}
   ...
   try{
    ...
    await db.query("INSERT INTO payroll_retirement_replacement_attempt(authorization_id,kind,status,message,created_at) VALUES($1,'BANK',$2,$3,$4)",[row.id,status,message,timestamp])
