...
export async function refreshRetirementTimingAlerts(pool,facility=null,{now=new Date()}={}){
 const rows=(await pool.query(`SELECT DISTINCT l.facility_id,l.run_id,l.plan_id,r.payment_date::text AS payment_date,c.checked_at, EXISTS(SELECT 1 FROM payroll_retirement_employer_run_ledger e WHERE e.facility_id=l.facility_id AND e.run_id=l.run_id AND e.plan_id=l.plan_id AND e.matching_cents+e.nonelective_cents>0) AS employer_required
 ...
 WHERE r.status='FINALIZED' AND r.payment_date>='2026-01-01' AND r.payment_date<'2027-01-01' AND ((l.calculation->>'totalCents')::bigint>0 OR EXISTS(SELECT 1 FROM payroll_retirement_employer_run_ledger e WHERE e.facility_id=l.facility_id AND e.run_id=l.run_id AND e.plan_id=l.plan_id AND e.matching_cents+e.nonelective_cents>0))
 ...
 for(const row of rows){const db=await pool.connect();try{
  ...
  const assessment=await retirementTimingAssessment(db,row.facility_id,row.plan_id,row.payment_date,{now,employerContributionsRequired:row.employer_required}),key=`retirement-timing-${row.run_id}-${row.plan_id}`
