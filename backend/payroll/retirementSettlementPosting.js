...
import {verifyRetirementAccountingEvidence} from './retirementAccountingEvidence.js'
...
export async function postRetirementSettlement(pool,facility,id,{fetcher=fetch,paymentFetcher=fetch,actorId=null,recoveryOnly=false}={}){
 ...
 try{
  ...
  const a=(await db.query('SELECT a.* FROM payroll_retirement_settlement_authorization a JOIN payroll_retirement_remittance_authorization p ON p.id=a.payment_authorization_id WHERE a.id=$1 AND p.facility_id=$2',[id,facility])).rows[0]
  if(!a)throw fail('Retirement settlement authorization was not found.',404)
  if((await db.query('SELECT authorization_id FROM payroll_retirement_settlement_cancellation WHERE authorization_id=$1',[id])).rows.length)throw fail('This retirement settlement authorization was cancelled.')
  const existing=(await db.query('SELECT authorization_id FROM payroll_retirement_settlement_claim WHERE authorization_id=$1',[id])).rows.length>0
  const verify=async()=>{const current=await prepareRetirementSettlement(db,facility,a.payment_authorization_id,{fetcher,paymentFetcher});if(current.fingerprint!==a.fingerprint)throw fail('Authorized bank, payroll journal, mapping or period evidence changed. Review the settlement authorization.')}
  if(!existing){
   if(recoveryOnly)throw fail('No settlement posting has started. Recovery cannot create journals.')
   await verify();await db.query('INSERT INTO payroll_retirement_settlement_claim(authorization_id,created_by) VALUES($1,$2)',[id,actorId])
   for(const item of a.preview.journals)await db.query('INSERT INTO payroll_retirement_settlement_journal(id,authorization_id,facility_id,realm_id,environment,event_key,payload) VALUES($1,$2,$3,$4,$5,$6,$7)',[randomUUID(),id,facility,a.preview.realmId,a.preview.environment,item.event.key,item.payload])
  }
  ...
  await db.query('BEGIN');await db.query('SELECT facility_id FROM payroll_settings WHERE facility_id=$1 FOR UPDATE NOWAIT',[facility])
  const jobs=(await db.query('SELECT * FROM payroll_retirement_settlement_journal WHERE authorization_id=$1 ORDER BY event_key',[id])).rows
  ...
  if(!preflightFailure&&connectionMatches)try{accountingEvidence=await verifyRetirementAccountingEvidence(db,facility,a,{fetcher,paymentFetcher})}catch{}
  ...
  for(const job of jobs){
   ...
   await db.query('INSERT INTO payroll_retirement_settlement_observation(journal_id,result,source,create_attempted) VALUES($1,$2,$3,$4)',[job.id,result,existing?'RECOVERY':'SUBMISSION',createAttempted])
   results.push({jobId:job.id,eventKey:job.event_key,...result})
  }
  const status=accountingEvidence&&results.every(r=>r.status==='SYNCED')?'SYNCED':'NEEDS_REVIEW',key=`retirement-settlement-${id}`
  ...
  else await db.query("INSERT INTO payroll_alert(facility_id,dedupe_key,severity,title,message) VALUES($1,$2,'WARNING','Retirement settlement accounting needs attention','Open the retirement payment bank accounting review to recover its retained journals. Recovery never sends another journal.') ON CONFLICT(facility_id,dedupe_key) DO UPDATE SET status='OPEN',dismissed_at=NULL,message=EXCLUDED.message",[facility,key])
  await db.query("INSERT INTO payroll_audit_log(facility_id,actor_user_id,action,entity_type,entity_id,after_data) VALUES($1,$2,'RETIREMENT_SETTLEMENT_POSTING_OBSERVED','retirement_settlement_authorization',$3,$4)",[facility,actorId,id,{status,recovery:existing,results}])
  await reconcileRetirementObservation(db,facility,a.payment_authorization_id,{actorId});await db.query('COMMIT');return {status,recovery:existing,results}
...
export function registerRetirementSettlementPostingRoutes(app,pool,options){
 app.post('/api/admin/payroll/retirement-settlement-authorizations/:id/post',async(req,res)=>{
  ...
  try{if(req.body?.confirmed!==true||!['POST','RECOVER'].includes(req.body.action)||!/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(req.params.id))throw fail('Confirm posting or recovery of the retained settlement.',400)
   res.json({success:true,data:await postRetirementSettlement(pool,req.canonicalAccess.facilityId,req.params.id,{...options,actorId:req.adminId,recoveryOnly:req.body.action==='RECOVER'})})
