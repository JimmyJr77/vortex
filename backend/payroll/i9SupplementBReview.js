import {readI9Qualification,qualificationAllowsAlternative} from './i9Qualification.js'
...
export async function i9DocumentFollowupBasis(db,ctx,taskId,kind){
 ...
 let followupExaminedOn=retained.examination.examinedOn
 ...
 for(const prior of (await db.query('SELECT s.*,d.content_sha256,d.encrypted_content,d.task_id FROM payroll_i9_supplement_signature s JOIN payroll_private_document d ON d.id=s.document_id WHERE s.signature_id=$1 ORDER BY s.id',[row.id])).rows){
  ...
  if(row.followup_row_key===`SUPPLEMENT:${prior.id}`)followupExaminedOn=retainedPrior.examination.examinedOn
 ...
 for(const prior of (await db.query('SELECT s.*,d.content_sha256,d.encrypted_content,d.task_id FROM payroll_i9_receipt_signature s JOIN payroll_private_document d ON d.id=s.document_id WHERE s.signature_id=$1 ORDER BY s.id',[row.id])).rows){
  ...
  if(row.followup_row_key===`RECEIPT:${prior.id}`)followupExaminedOn=evidence.examination.examinedOn
 ...
 for(const prior of (await db.query('SELECT s.*,d.content_sha256,d.encrypted_content,d.task_id FROM payroll_i9_different_signature s JOIN payroll_private_document d ON d.id=s.document_id WHERE s.signature_id=$1 ORDER BY s.id',[row.id])).rows){
  ...
  if(row.followup_row_key.startsWith(`DIFFERENT:${prior.id}:`))followupExaminedOn=evidence.facts.examination.examinedOn
 ...
 for(const prior of (await db.query('SELECT s.*,d.content_sha256,d.encrypted_content,d.task_id FROM payroll_i9_different_supplement_signature s JOIN payroll_private_document d ON d.id=s.document_id WHERE s.signature_id=$1 ORDER BY s.id',[row.id])).rows){
  const evidence=JSON.parse(decryptDocument(prior.encrypted_evidence,`i9-different-supplement-signature:${ctx.facility}:${ctx.employee}:${prior.compliance_task_id}:${prior.actor_user_id}`).toString())
  const pdf=decryptDocument(prior.encrypted_content,`${ctx.facility}:${ctx.employee}:${prior.task_id}`)
  if(hash(pdf)!==prior.content_sha256||evidence.documentSha256!==prior.content_sha256||!Number.isInteger(evidence.pageCount)||evidence.pageCount<2||evidence.pageCount>100)throw fail('A prior different-document certification failed its integrity check.')
 ...
 const examinationContext={qualification,eVerify:qualificationAllowsAlternative(qualification,retained.context.eVerify)}
...
export async function recordI9SupplementBPage(db,ctx,taskId,body){
 if(body.displayed!==true||!(['supplement','source'].includes(body.documentKey)||/^(prior|receipt|different|different-supplement):[1-9]\d*$/.test(body.documentKey))||!Number.isInteger(body.page)||body.page<1)throw fail('Display a page from the current Supplement B review packet.',400)
