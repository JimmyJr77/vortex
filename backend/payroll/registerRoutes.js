...
import {registerHealthPlanQualification} from './healthPlanQualification.js'
...
import {registerRetirementReplacementSettlementAuthorization} from './retirementReplacementSettlementAuthorization.js'
import {registerRetirementReplacementSettlementPosting} from './retirementReplacementSettlementPosting.js'
...
export async function loadBasePreview(pool, facilityId, payPeriodId, paymentDate = null, excludedRunId = null, skipHistoricalCoverage = false, timeOverride = null, additionalAdjustments = [], leaveCorrection = null, retirement401kByEmployee = {}, health125ByEmployee = {}) {
  await ensureEmployerSetup(pool,facilityId)
  ...
  const employmentTaxHistory=await loadEmploymentTaxWageHistory(pool,facilityId,employees.map(e=>e.id),periodResult.rows[0].pay_date,excludedRunId,Object.keys(health125ByEmployee))
  const {ytdTaxWagesByEmployee}=employmentTaxHistory
  ...
  let preview = buildPayrollPreview({ settings, employees, entries, complianceTasks: tasks, ytdByEmployee, ytdTaxWagesByEmployee, priorApprovedMinutesByEmployee, priorApprovedSegmentsByEmployee, adjustments, taxYear, retirement401kByEmployee, health125ByEmployee, payPeriod:periodResult.rows[0] })
  ...
  if(Object.keys(weightedSettlementsByEmployee).length) {
    ...
    preview=buildPayrollPreview({settings,employees,entries,complianceTasks:tasks,ytdByEmployee,ytdTaxWagesByEmployee,priorApprovedMinutesByEmployee,priorApprovedSegmentsByEmployee,adjustments,taxYear,weightedSettlementsByEmployee,retirement401kByEmployee,health125ByEmployee,payPeriod:periodResult.rows[0]})
    for(const employeePreview of preview.employees) {
  ...
  leaveWarnings.push(...employmentTaxHistory.warnings)
  for(const employee of preview.employees)if(employmentTaxHistory.evidenceByEmployee[employee.employeeId])employee.employmentTaxWageHistory=employmentTaxHistory.evidenceByEmployee[employee.employeeId]
  ...
  for(const employee of preview.employees)if(employee.health125&&employee.benefitCollection?.status!=='COLLECT_THIS_RUN'){
   const warning={employeeId:employee.employeeId,code:'HEALTH_COLLECTION_CHANGED',severity:'critical',blocking:true,message:'Health collection changed during payroll calculation. Refresh and reconcile the current monthly authorization before approval.'}
   employee.warnings.push(warning);preview.warnings.push(warning)
  }
  const pendingHealth=Object.fromEntries(preview.employees.filter(e=>e.pendingHealth125).map(e=>[e.employeeId,e.pendingHealth125]))
  if(Object.keys(pendingHealth).length&&!Object.keys(health125ByEmployee).length)return loadBasePreview(pool,facilityId,payPeriodId,paymentDate,excludedRunId,skipHistoricalCoverage,timeOverride,additionalAdjustments,leaveCorrection,retirement401kByEmployee,pendingHealth)
...
export function registerPayrollRoutes(app, pool, {retirementReceiptReader,retirementAllocationTransfer,retirementSftpVerifier,remittanceNow=()=>new Date(),now=()=>new Date(),invitationSender=sendEmail,paymentFetcher=fetch,quickbooksFetcher=fetch,carrierNoticeSender} = {}) {
  ...
  registerRetirementReplacementSettlementPreview(app,pool,{fetcher:quickbooksFetcher,paymentFetcher})
  registerRetirementReplacementSettlementAuthorization(app,pool,{fetcher:quickbooksFetcher,paymentFetcher})
  registerRetirementReplacementSettlementPosting(app,pool,{fetcher:quickbooksFetcher,paymentFetcher})
  registerRetirementReplacementSettlementReleasePreview(app,pool,{fetcher:quickbooksFetcher})
  registerRetirementReplacementSettlementRelease(app,pool,{fetcher:quickbooksFetcher})
  ...
  app.patch('/api/admin/payroll/runs/:id/status', async (req, res) => {
    ...
    try {
      ...
      if(requested==='APPROVED'){
        await retainRetirementRunLedger(client,req.canonicalAccess.facilityId,run.id)
        await integrateEmployerRetirementApproval(client,req.canonicalAccess.facilityId,run.id)
        rows[0]=(await client.query('SELECT * FROM payroll_run WHERE id=$1',[run.id])).rows[0]
      }
