...
export function registerBenefitCarrierInvoiceRoutes(app,pool,{fetcher=fetch,paymentFetcher=fetch,now=()=>new Date()}={}){
 ...
 app.get('/api/admin/payroll/benefit-carrier-invoices',endpoint(false,async(db,req)=>{
  ...
  const records=(await db.query('SELECT i.id,i.invoice,i.source_snapshot,i.source_fingerprint,i.revision,i.created_at,EXISTS(SELECT 1 FROM payroll_benefit_carrier_invoice newer WHERE newer.facility_id=i.facility_id AND newer.carrier_key=i.carrier_key AND newer.invoice_key=i.invoice_key AND newer.revision>i.revision) AS superseded FROM payroll_benefit_carrier_invoice i WHERE i.facility_id=$1 AND i.coverage_month=$2 ORDER BY i.created_at DESC,i.revision DESC,i.id DESC',[facility,month])).rows
