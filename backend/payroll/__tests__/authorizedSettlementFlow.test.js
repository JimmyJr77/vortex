...
for(const initial of ['HOURLY','SALARY'])test(`${initial}-first authorized allocation payroll approves, settles once and delivers frozen statements`,{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 await api('/pay-schedule/configure',{frequency:'WEEKLY',anchorStart:'2026-08-03',paymentLagDays:5,source:'Synthetic published split payroll schedule',confirmed:true})
 ...
 const allocationInput={payPeriodId:period.id,week:'2026-08-03',salaryAllocations:[{employmentStart:initial==='HOURLY'?'2026-08-07':'2026-08-03',earningsCents:120000,source:'Synthetic verified allocation of all straight-time salary earnings'}]}
 ...
 const calculated=preview.employees[0];assert.equal(calculated.grossPayCents,initial==='HOURLY'?198000:217800);assert.equal(calculated.sickLeaveAccrualMinutes,100)
 ...
 assert.equal((await h.pool.query('SELECT SUM(minutes)::int n FROM payroll_leave_transaction WHERE employee_id=$1',[e.id])).rows[0].n,100)
