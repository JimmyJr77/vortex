...
for(const mode of ['SUCCESS','LOST_RESPONSE','CHANGED','NOT_SENT','CHANGED_AFTER_CLAIM','NOT_SENT_FOUND','REVERSAL_LOST_RESPONSE'])test(`carrier premium posting commits its claim and recovers without duplicate creation (${mode})`,{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 if(['SUCCESS','REVERSAL_LOST_RESPONSE'].includes(mode)){
  ...
  const postedInvoice=(await api('/benefit-carrier-invoices?month=2026-09')).history[0];assert.equal(postedInvoice.accountingStatus,'REVERSAL_POSTED');const postedHistory=postedInvoice.authorizations[0].reversals.find(row=>row.id===renewedReversal.id);assert.ok(postedHistory);assert.equal(postedHistory.claimed,true);assert.equal(postedHistory.posting_result.status,'SYNCED')
  ...
  await h.pool.query("UPDATE payroll_quickbooks_connection SET realm_id='999' WHERE facility_id=1");assert.equal((await recoverCarrierReversals(h.pool,1,{fetcher,now:new Date(recoveryAt.getTime()+25*60*60*1000)})).checked,1);assert.equal((await api('/benefit-carrier-invoices?month=2026-09')).history[0].authorizations[0].reversals.find(row=>row.id===renewedReversal.id).posting_result.status,'CONNECTION_CHANGED');await h.pool.query("UPDATE payroll_quickbooks_connection SET realm_id='123' WHERE facility_id=1");assert.equal(reversalPosts,1)
