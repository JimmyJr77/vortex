...
for(const pretaxHealth of [false,true])for(const declined of [false,true])test(`annual health=${pretaxHealth} ${declined?'employer-only':'combined'} obligations require participation review without adding employer funding to employee deferral codes`,{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 const healthFixture=pretaxHealth?await healthElectionFixture(h,{benefitsOptions:{hireDate:'2026-09-09'}}):null
 if(healthFixture)await healthFixture.api(healthFixture.path,healthFixture.electionBody,'POST',200,true)
 const {api,employee,periods}=await regularEmployerRetirementFixture(h,{declined,existingFixture:healthFixture})
 ...
 assert.equal(prepared.sourceStatus,'READY_FOR_REVIEW',JSON.stringify(prepared.issues));assert.deepEqual(prepared.retirementContributions,report)
 ...
 const health=await api('/health-coverage-reporting',{year:2026,disposition:pretaxHealth?'REPORT':'SMALL_EMPLOYER_RELIEF',priorYearW2Count:pretaxHealth?null:1,expectedRevision:0,reference:'Synthetic verified prior-year small-employer reporting count',confirmed:true},'POST',201)
 await api(`/employees/${employee.id}/health-coverage-classification`,{year:2026,determinationId:health.revision,sourceFingerprint:prepared.sourceFingerprint,expectedRevision:0,disposition:pretaxHealth?'REPORT':'RELIEF_USED',reportableCostCents:pretaxHealth?57500:null,reference:'Synthetic reviewed small-employer relief for coverage',confirmed:true},'POST',201)
 ...
 assert.equal(actualDraft.status,'DRAFT_REVIEW_REQUIRED');assert.equal(actualDraft.boxes.box13.retirementPlan,true);assert.deepEqual(actualDraft.boxes.box12,[...mapped.boxes.box12,...(pretaxHealth?[{code:'DD',amount:'575.00'}]:[])])
 assert.equal(actualDraft.boxes.box1,pretaxHealth?(declined?'75.00':'65.00'):(declined?'200.00':'190.00'))
 assert.equal(actualDraft.boxes.box16,actualDraft.boxes.box1)
 assert.equal(actualDraft.boxes.box3,pretaxHealth?'75.00':'200.00');assert.equal(actualDraft.boxes.box5,pretaxHealth?'75.00':'200.00')
 ...
 const bytes=Buffer.from(await pdf.arrayBuffer());if(pretaxHealth&&!declined)await writeFile('/tmp/payroll-health-approved-w2.pdf',bytes)
 const packet=await PDFDocument.load(bytes);assert.equal(packet.getPageCount(),6);assert.equal(packet.getForm().getFields().length,0)
 if(pretaxHealth){
  ...
  const changed=(await api('/reports/year-end-preparation?year=2026')).employees[0]
  ...
  await api(`/employees/${employee.id}/w2-approval`,{year:2026,draftFingerprint:actualDraft.fingerprint,expectedRevision:approval.revision,reference:'Synthetic mismatched health premium must block renewed W-2 approval',confirmed:true},'POST',409)
  await h.pool.query('UPDATE payroll_run_employee SET pretax_deduction_cents=pretax_deduction_cents-1 WHERE payroll_run_id=$1',[run.id])
  assert.equal((await api('/reports/year-end-preparation?year=2026')).employees[0].w2ApprovalHistory[0].status,'CURRENT')
 }
