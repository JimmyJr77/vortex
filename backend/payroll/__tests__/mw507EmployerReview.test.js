...
for(const exempt of [false,true])test(`signed Maryland transfer requires current employer review: exempt=${exempt}`,{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 const task=(await api('/onboarding',undefined,'GET',200,true)).tasks.find(t=>t.task_key==='STATE_WITHHOLDING'),path=`/onboarding/${task.id}/mw507`,answers={...syntheticMw507(),additionalWithholdingCents:0,...(exempt?{withholdingRate:null,exemptions:null,claim:{kind:'NO_LIABILITY',priorYearNoTax:true,currentYearNoTax:true,effectiveYear:2026}}:{})}
 ...
 const body=data=>({federal:data.election.elections.federal,maryland:{filingStatus:data.nativeMW507.choices.filingStatus,exemptions:data.nativeMW507.choices.exemptions,extraWithholdingCents:data.nativeMW507.choices.extraWithholdingCents,exempt,localRate:3.2},mw507SubmissionId:data.nativeMW507.submissionId,mw507Fingerprint:data.nativeMW507.fingerprint,confirmed:true,sourceNote:'Reviewed synthetic signed Maryland certificate and local table',mw507EmployerReview:{correctnessConfirmed:true,noRevocationConfirmed:true,localRateConfirmed:true,correctnessEvidence:'Reviewed retained certificate and employer correspondence',localRateEvidence:'Synthetic Maryland resident and verified local table',...(exempt?{expectedWeeklyWagesCents:50000,weeklyWageEvidence:'Reviewed synthetic expected weekly compensation',comptrollerSubmissionEvidence:'Synthetic retained certificate submission receipt'}:{})}})
 ...
 if(exempt)await save({...ready,mw507EmployerReview:{...ready.mw507EmployerReview,comptrollerSubmissionEvidence:''}},409)
 await save(ready)
 const applied=(await get()).election.elections
 assert.equal(applied.mw507ReviewRequired,undefined);assert.equal(applied.mw507Source.submissionId,first.submissionId);assert.equal(applied.maryland.exemptions,exempt?null:5)
 assert.equal(applied.mw507Source.review.correctnessEvidence,ready.mw507EmployerReview.correctnessEvidence)
 assert.ok(Number.isInteger(calculateWithholding2026({grossPayCents:200000,paymentDate:applied.mw507Source.receivedOn,election:{...applied,verified:true},payFrequency:'SEMIMONTHLY',year:2026,workState:'MD',residenceState:'MD'}).stateIncomeTaxCents))
 ...
 answers.exemptions=exempt?null:3;await sign();const pending=(await get()).election.elections
 ...
 await review();await save(ready,409);await save(body(await get()));assert.equal((await get()).election.elections.maryland.exemptions,exempt?null:3)
})
