...
test('carrier settlement mappings retain scoped historical funding, distinct liabilities and current company evidence',{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 const config={organizationId:randomUUID(),originatingAccountId:randomUUID(),apiKey:'synthetic-carrier-mapping-key',mode:'TEST',reference:'Synthetic funding account mapping',confirmed:true,expectedRevision:0}
 const first=await api('/payment-connection',config,201),initial=await api(path)
 const body={confirmed:true,expectedRevision:0,fundingRevisionId:first.revision,connectionGeneration:initial.connection.generation,realmId:'123',environment:'sandbox',bankAccountId:'7',liabilityAccountId:'9',reference:'Reviewed bank and carrier liability association'}
 ...
 const saved=await api(path,body,201);await api(path,body,409);assert.equal((await api(path,{...body,expectedRevision:saved.revision})).reused,true)
 ...
 assert.equal((await api(path,undefined,200,2)).history.length,0);await api(path,{...body,expectedRevision:saved.revision},409,2)
 ...
 const remapped=await api(path,{...body,expectedRevision:saved.revision,bankAccountId:'8'},201)
 ...
 const separate=await api(path,{...body,expectedRevision:remapped.revision,liabilityAccountId:'10'},201)
 ...
 const request={...body,expectedRevision:separate.revision,fundingRevisionId:second.revision}
 ...
 await assert.rejects(h.pool.query('DELETE FROM payroll_carrier_settlement_mapping'),/append-only/)
 await assert.rejects(h.pool.query("INSERT INTO payroll_carrier_settlement_mapping(facility_id,payment_connection_id,connection_generation,realm_id,environment,details,reference,created_by) SELECT facility_id,payment_connection_id,connection_generation,realm_id,environment,'{}',reference,created_by FROM payroll_carrier_settlement_mapping LIMIT 1"),/exact funding/)
 await assert.rejects(h.pool.query("INSERT INTO payroll_carrier_settlement_mapping(facility_id,payment_connection_id,connection_generation,realm_id,environment,details,reference,created_by) SELECT 2,payment_connection_id,connection_generation,realm_id,environment,details,reference,created_by FROM payroll_carrier_settlement_mapping LIMIT 1"),/matching employer/)
 ...
 assert.equal(JSON.stringify(state).includes('synthetic-carrier-mapping-token'),false);assert.equal(JSON.stringify(state).includes(config.apiKey),false)
