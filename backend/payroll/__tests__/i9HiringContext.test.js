import {i9HiringContextEvidence,readI9HiringContext} from '../i9HiringContext.js'
import test from 'node:test'
import assert from 'node:assert/strict'
import {readFile} from 'node:fs/promises'
import {createHarness} from '../testing/harness.js'
import {monthlyBenefitsFixture} from '../testing/monthlyBenefitsFixture.js'
test('I-9 hiring evidence is generated from both required dates and participation status',()=>{
 const input={offerAcceptedOn:'2026-09-01',participationVerifiedOn:'2026-09-02'}
 assert.equal(i9HiringContextEvidence({...input,eVerify:false}),"Accepted employment offer dated 2026-09-01 was reviewed in the employee hiring record. The employer's E-Verify enrollment status and hiring-site participation records were reviewed on 2026-09-02 and confirm that the employer and hiring site do not participate in E-Verify.")
 assert.equal(i9HiringContextEvidence({...input,eVerify:true}),"Accepted employment offer dated 2026-09-01 was reviewed in the employee hiring record. The employer's E-Verify enrollment status and hiring-site participation records were reviewed on 2026-09-02 and confirm that the employer and hiring site participate in E-Verify.")
})
test('I-9 admin context has explicit inputs, scoped revisions and immutable history',{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 await h.pool.query(await readFile(new URL('../../migrations/816_payroll_i9_participation_verification_date.sql',import.meta.url),'utf8'))
 ...
 const body={onboardingCycle:1,expectedRevision:0,offerAccepted:true,offerAcceptedOn:'2026-09-01',participationVerifiedOn:'2026-09-02',eVerify:false,evidence:'This caller-supplied statement must be ignored.'}
 await api(path,{...body,eVerify:undefined},'POST',400);await api(path,{...body,offerAccepted:false},'POST',400);await api(path,{...body,offerAcceptedOn:'2099-01-01'},'POST',400);await api(path,{...body,participationVerifiedOn:''},'POST',400);await api(path,{...body,participationVerifiedOn:'2099-01-01'},'POST',400)
 const saved=await api(path,body);assert.equal(saved.current.eVerify,false);assert.equal(saved.current.offerAcceptedOn,'2026-09-01');assert.equal(saved.current.participationVerifiedOn,'2026-09-02');assert.equal(Number(saved.current.actorUserId),99)
 assert.equal(saved.current.evidence,i9HiringContextEvidence(body))
 assert.match(saved.current.evidence,/do not participate in E-Verify/)
 ...
 const packet=await api('/onboarding',undefined,'GET',200,true);assert.equal(JSON.stringify(packet).includes(saved.current.evidence),false)
