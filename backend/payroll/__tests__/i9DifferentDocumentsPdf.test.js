...
test('new Section 2 keeps employee identity without copying old signatures or unrelated attestations',async()=>{
 ...
 assert.ok(form.getTextField('Additional Information').getText().replaceAll(' ','').includes(input.originalEmployerSha256))
 ...
 const signed=await signI9DifferentDocumentsPdf(preview.pdf,{signature:'Alice Reviewer',signedOn:'2026-09-13'}),result=(await PDFDocument.load(signed)).getForm()
 ...
 await assert.rejects(()=>signI9DifferentDocumentsPdf(signed,{signature:'Other',signedOn:'2026-09-14'}),/already contains/)
