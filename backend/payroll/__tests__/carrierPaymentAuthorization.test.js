...
test('carrier payment authorizations reserve amounts, preserve retries and block concurrent overpayment and corrections',{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 const h=await createHistoricalHarness(t,{quickbooksFetcher:fetcher,paymentFetcher});t.after(()=>h.close());const {api,periods}=await monthlyBenefitsFixture(h)
 ...
 const authPath=`${path}/${invoice.id}/payment-authorizations`,partial=await api(paymentPath,{...payment,amountCents:25000})
 const authorization={amountCents:25000,paymentDate:payment.paymentDate,fingerprint:partial.fingerprint,requestKey:'first-carrier-payment-request',reference:'Reviewed invoice, destination and missing outside activity',confirmed:true,outsideActivityReviewed:true}
 await api(authPath,{...authorization,outsideActivityReviewed:false},'POST',400)
 await api(authPath,{...authorization,fingerprint:'0'.repeat(64)},'POST',409)
 const raced=await Promise.all([api(authPath,authorization),api(authPath,authorization)]);assert.equal(raced[0].id,raced[1].id)
 await api(authPath,{...authorization,requestKey:'competing-carrier-payment-request'},'POST',409)
 const firstPayment=raced[0].id,h1=await api(authPath);assert.equal(h1.reservedCents,25000);assert.equal(h1.availableCents,32500)
 await api(paymentPath,payment,'POST',409)
 const remainder=await api(paymentPath,{...payment,amountCents:32500}),secondPayment=await api(authPath,{...authorization,amountCents:32500,fingerprint:remainder.fingerprint,requestKey:'second-carrier-payment-request'})
 assert.equal((await api(authPath)).availableCents,0)
 await assert.rejects(h.pool.query('INSERT INTO payroll_carrier_payment_authorization(id,invoice_id,premium_authorization_id,payee_id,connection_id,amount_cents,payment_date,preview,fingerprint,request_key,reference,outside_activity_reviewed,created_by) SELECT $1,invoice_id,premium_authorization_id,payee_id,connection_id,amount_cents,payment_date,preview,fingerprint,$2,reference,true,created_by FROM payroll_carrier_payment_authorization WHERE id=$3',[uid(90),'direct-over-capacity-request',firstPayment]),/reservations exceed/)
 await assert.rejects(h.pool.query('DELETE FROM payroll_carrier_payment_authorization'),/append-only/)
 await assert.rejects(h.pool.query('INSERT INTO payroll_carrier_reversal_authorization(id,premium_authorization_id,request_key,preview,fingerprint,reference,created_by) VALUES($1,$2,$3,$4,$5,$6,99)',[uid(91),a.id,'direct-reversal-with-reservation',{authorizationId:a.id,fingerprint:'1'.repeat(64)},'1'.repeat(64),'Reviewed blocked accounting reversal']),/Resolve active carrier payment/)
 await api(`/carrier-premium-authorizations/${a.id}/reversal-preview`,{reversalDate:'2026-09-02'},'POST',409)
 const cancelBody={confirmed:true,reference:'Cancel before dispatch to revise reviewed payment'}
 const cancelPath=`/carrier-payment-authorizations/${firstPayment}/cancel`
 assert.equal((await fetch(`${h.url}/api/admin/payroll${cancelPath}`,{method:'POST',headers,body:JSON.stringify(cancelBody)})).status,404)
 await api(cancelPath,cancelBody);assert.equal((await api(cancelPath,cancelBody)).reused,true)
 await api(authPath,authorization,'POST',409)
 await api(`/carrier-payment-authorizations/${firstPayment}/dispatch`,{action:'SUBMIT',confirmed:true},'POST',409)
 await assert.rejects(h.pool.query('INSERT INTO payroll_carrier_payment_claim(authorization_id,encrypted_instruction,created_by) VALUES($1,$2,99)',[firstPayment,Buffer.from('synthetic blocked instruction')]),/Cancelled carrier payment/)
 assert.equal((await api(authPath)).availableCents,25000)
 assert.equal((await api(paymentPath,{...payment,amountCents:25000})).reservations.availableCents,25000)
 await api(`/carrier-payment-authorizations/${secondPayment.id}/cancel`,cancelBody)
 assert.equal((await api(authPath)).availableCents,57500)
 await assert.rejects(h.pool.query('DELETE FROM payroll_carrier_payment_cancellation'),/append-only/)
