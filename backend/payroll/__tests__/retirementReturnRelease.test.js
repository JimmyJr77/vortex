...
for(const unsent of [true,false])test(`return accounting release ${unsent?'permits a proven non-send replacement':'blocks an uncertain create'}`,{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 try{
  ...
  const history=await f.api(authPath),postedHistory=history.history.find(row=>row.id===auth.id);assert.ok(postedHistory);assert.equal(postedHistory.claimed,true);assert.equal(postedHistory.journals[0].result.status,'SYNCED')
  ...
  const released=(await f.api(approvalPath)).history.find(row=>row.id===renewed.id);assert.ok(released);assert.ok(released.cancelled_at);assert.equal(released.release.reference,releaseBody.reference)
