...
for(const initial of ['HOURLY','SALARY'])test(`${initial}-first earned bonus and leave settle after both hiring portions with linked overtime evidence`,{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 for(const [index,date] of ['2026-08-04','2026-08-08'].entries()){
  const salary=index?initial==='HOURLY':initial==='SALARY'
  const request=(await h.pool.query("INSERT INTO payroll_employee_request(facility_id,employee_id,kind,status,payload) VALUES(1,$1,'LEAVE','APPROVED',$2) RETURNING id",[e.id,{leaveType:'PTO',startDate:date,endDate:date,minutes:60}])).rows[0]
  await h.pool.query('INSERT INTO payroll_paid_leave(facility_id,employee_id,request_id,leave_date,minutes,hourly_rate_cents,included_in_salary) VALUES(1,$1,$2,$3,60,$4,$5)',[e.id,request.id,date,salary?null:index?3000:2500,salary])
 }
 const earnedInput={amountCents:10000,earnedStart:'2026-08-03',earnedEnd:'2026-08-09'}
 const earned=await api(`/employees/${e.id}/bonus-allocation/preview`,earnedInput)
 assert.equal(earned.additionalOvertimeCents,1000)
 await api(`/employees/${e.id}/bonuses`,{...earnedInput,payPeriodId:periods[1].id,classification:'NONDISCRETIONARY',paymentType:'ANNUAL_LUMP_SUM',confirmed:true,source:'Synthetic earned bonus proportional to full week worked hours',allocationMethod:'PROPORTIONAL_EARNED_HOURS',allocationMethodVerified:true,allocationFingerprint:earned.fingerprint},'POST',201)
 ...
 for(const [index,period] of periods.entries()){
  ...
  if(index){assert.equal(preview.employees[0].payItems.find(p=>p.kind==='BONUS_OVERTIME').amountCents,1000);assert.equal(preview.employees[0].payItems.find(p=>p.kind==='BONUS').allocatedBonus.version,2)}
  assert.equal(preview.employees[0].paidLeaveMinutes,60)
  assert.equal(preview.employees[0].paidLeavePayCents,initial==='HOURLY'&&index===0?2500:3000)
  assert.equal(preview.employees[0].workweekPayments[0].leaveCoverage.entries.length,1)
 ...
 assert.equal(frozen.reduce((n,r)=>n+Number(r.regular_pay_cents)+Number(r.overtime_pay_cents)+Number(r.other_taxable_pay_cents),0),initial==='HOURLY'?211200:231500)
 ...
 const bonuses=final.paymentReconciliation.paid.flatMap(p=>p.bonusPayments)
 assert.equal(bonuses.length,1);assert.equal(bonuses[0].bonusAllocation.additionalOvertimeCents,1000)
 assert.equal(final.paymentReconciliation.paid.flatMap(p=>p.paidLeaveEntries).length,2)
 assert.equal(new Set(final.paymentReconciliation.paid.flatMap(p=>p.paidLeaveEntries).map(l=>l.id)).size,2)
