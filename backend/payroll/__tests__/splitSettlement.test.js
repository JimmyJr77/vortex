...
for(const initial of ['HOURLY','SALARY'])test(`${initial}-first split payroll approves, settles once and delivers frozen statements`,{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 const e=await api('/employees',{employeeNumber:`SPLIT-${initial}`,legalFirstName:'Split',legalLastName:'Settlement',hireDate:'2026-08-03',payType:initial,hourlyRateCents:2500,annualSalaryCents:5200000},'POST',201)
 ...
 await h.pool.query("UPDATE payroll_employee SET employment_status='ONBOARDING',hire_date='2026-08-10',termination_date=NULL,pay_type=$2,hourly_rate_cents=3000,annual_salary_cents=$3,salary_review=NULL WHERE id=$1",[e.id,initial==='HOURLY'?'SALARY':'HOURLY',initial==='HOURLY'?5200000:null])
 if(initial==='HOURLY')await api(`/employees/${e.id}/salary-review`,review)
 else await h.pool.query("INSERT INTO payroll_pay_rate(facility_id,employee_id,effective_on,hourly_rate_cents,reason) VALUES(1,$1,'2026-08-10',3000,'New hourly agreement')",[e.id])
 ...
 for(const day of ['03','04','05','10'])await h.pool.query("INSERT INTO payroll_time_entry(facility_id,employee_id,clock_in,clock_out,source,status) VALUES(1,$1,$2,$3,'ADMIN','APPROVED')",[e.id,`2026-08-${day}T12:00Z`,`2026-08-${day}T20:00Z`])
 ...
 const calculated=preview.employees[0];assert.equal(calculated.grossPayCents,initial==='HOURLY'?260000:224000);assert.equal(calculated.sickLeaveAccrualMinutes,64)
 ...
 assert.deepEqual(frozen.statement_snapshot.splitCompensation,calculated.splitCompensation)
 assert.equal(statementLines(frozen).lines.reduce((n,l)=>n+Number(l[2]),0),Number(frozen.net_pay_cents))
 assert.equal((await h.pool.query('SELECT SUM(minutes)::int n FROM payroll_leave_transaction WHERE employee_id=$1',[e.id])).rows[0].n,64)
