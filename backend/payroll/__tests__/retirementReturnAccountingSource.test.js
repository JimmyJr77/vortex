...
for(const automatic of [false,true])test(`return accounting verifies original sources and ${automatic?'automatic':'manual'} posting`,{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 try{
  ...
  if(automatic){
   ...
   assert.equal((await f.api(approvalPath)).history.find(row=>row.id===renewed.id).attempts.length,2)
  }else{
  ...
  assert.equal(returnHistory.length,2);const retainedReturn=returnHistory.find(row=>row.id===renewed.id);assert.ok(retainedReturn);assert.equal(retainedReturn.journal.result.status,'NOT_FOUND');assert.equal(retainedReturn.claimed,true)
