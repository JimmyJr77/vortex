...
for(const initial of ['HOURLY','SALARY'])test(`${initial}-first allocated bonus and leave settle across adjacent periods with preserved overtime and payment history`,{skip:!process.env.PAYROLL_TEST_DATABASE_URL},async t=>{
 ...
 for(const period of periods)await api(`/employees/${e.id}/bonuses`,{payPeriodId:period.id,amountCents:10000,classification:'DISCRETIONARY',paymentType:'ANNUAL_LUMP_SUM',confirmed:true,source:'Synthetic annual award with no prior promise and employer discretion',amountDiscretionVerified:true,paymentDiscretionVerified:true,noPriorPromiseVerified:true},'POST',201)
 ...
 for(const [index,period] of periods.entries()){
  ...
  assert.equal(preview.employees[0].payItems.filter(i=>i.kind==='BONUS').length,1)
  assert.equal(preview.employees[0].payItems.find(i=>i.kind==='BONUS').allocatedBonus.amountCents,10000)
 ...
 assert.equal(frozen.reduce((n,r)=>n+Number(r.regular_pay_cents)+Number(r.overtime_pay_cents)+Number(r.other_taxable_pay_cents),0),initial==='HOURLY'?220200:240500)
 ...
 assert.equal(final.paymentReconciliation.paid.flatMap(p=>p.bonusPayments).length,2)
 assert.equal(new Set(final.paymentReconciliation.paid.flatMap(p=>p.bonusPayments).map(b=>b.adjustmentId)).size,2)
