...
export async function resolveCarrierReversalJournal(job,request,{allowCreate=false}={}){
 if(!uuid(job?.id)||typeof request!=='function'||!/^VTXR-[A-Za-z0-9_-]{16}$/.test(job.payload?.DocNumber)||!journalMatches(job.payload,job.payload))throw new Error('Invalid retained premium reversal.')
