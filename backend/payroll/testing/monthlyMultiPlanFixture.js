...
export async function monthlyMultiPlanFixture(h,{taxTreatment='POSTTAX',dentalTreatment='POSTTAX',hourlyRateCents=2500,authorize=true,hireDate='2026-09-01'}={}){
 ...
 await api('/settings',{legalBusinessName:'Monthly Benefits Fixture',businessAddress:'123 Test Street, Bowie MD',businessPhone:'5550100000',benefitsText:'Reviewed synthetic benefit offering.',benefitPlans:[{id:'medical',name:'Medical',description:'Synthetic coverage with reviewed employee contribution.',options:[{id:'family',label:'Family',employeeCostCents:12500,employerCostCents:45000,taxTreatment}]},{id:'dental',name:'Dental',description:'Synthetic reviewed dental coverage.',options:[{id:'dental-family',label:'Family',employeeCostCents:1000,employerCostCents:3000,taxTreatment:dentalTreatment}]}],expectedBenefitCatalog:'',benefitsCatalogEvidence:'Synthetic reviewed carrier rate sheet',benefitsCatalogConfirmed:true},'PATCH')
 ...
 const employee=await api('/employees',{employeeNumber:'MONTHLY-BENEFITS',legalFirstName:'Monthly',legalLastName:'Benefits',hireDate,hourlyRateCents},'POST',201)
 ...
 packet=await api('/benefits-election',{choice:'ENROLL',signature:'Monthly Benefits',confirmed:true,displayedTerms:packet.policy.benefitsText,requestKey:'monthly-benefit-election',selections:[{planId:'medical',optionId:'family'},{planId:'dental',optionId:'dental-family'}],onboardingCycle:1},'POST',200,true)
 const task=packet.tasks.find(t=>t.task_key==='PAY_REVIEW')
 packet=await api(`/employees/${employee.id}/onboarding/${task.id}/review`,{status:'COMPLETE',note:'Verified carrier coverage and employee contribution',paySetupFingerprint:packet.paySetup.fingerprint,benefitsReview:{disposition:'ENROLLED',effectiveOn:hireDate,summary:'Reviewed family coverage enrollment with employee contribution.',evidenceReference:'Synthetic carrier confirmation',confirmed:true}})
