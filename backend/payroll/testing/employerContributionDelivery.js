...
export async function assertEmployerContributionDelivery(h,api,reservation,server,provider,setNow){
 ...
 assert.equal(employee.items[0].contributions[0].employeeAmountCents,1400)
 assert.equal(employee.items[0].contributions[0].employerMatchingCents,600)
 assert.equal(employee.items[0].contributions[0].employerNonelectiveCents,400)
