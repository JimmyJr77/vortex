...
export async function preparedReceiptFixture(h,{api,employee,signed,pdf},rowKey='B'){
 ...
 return {taskId:task,pageCount:review.pageCount,path,body}
}
export async function completedReceiptFixture(h,fixture,rowKey='B'){
 const prepared=await preparedReceiptFixture(h,fixture,rowKey),{taskId:task,path,body}=prepared
 const result=await fixture.api(path+'/sign',body)
 return {...result,...prepared,taskSnapshot:(await h.pool.query('SELECT * FROM payroll_compliance_task WHERE id=$1',[task])).rows[0],documentSnapshot:(await h.pool.query('SELECT * FROM payroll_private_document WHERE id=$1',[result.documentId])).rows[0]}
}
