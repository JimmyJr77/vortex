...
export async function createHarness({retirementReceiptReader=async()=>({status:'UNAVAILABLE'}),retirementAllocationTransfer=async()=>({status:'TRANSPORT_UNCERTAIN'}),retirementSftpVerifier=async()=>({status:'UNAVAILABLE'}),remittanceNow=()=>new Date(),retirementNow=()=>new Date(),quickbooksFetcher=async()=>{throw new Error('Synthetic QuickBooks transport unavailable')},databaseNow,providerIntake,adminMiddleware,invitationSender,carrierNoticeSender,payrollNow=()=>new Date('2051-01-01T12:00:00Z'),paymentFetcher=async()=>{throw new Error('Synthetic payment transport unavailable')}}={}) {
 ...
 for(const file of ['812_payroll_operations.sql','813_payroll_onboarding.sql','815_payroll_existing_account_link.sql','816_payroll_i9_participation_verification_date.sql','817_payroll_retire_availability_onboarding.sql','823_payroll_check_document_observation_order.sql','824_payroll_carrier_return_review_order.sql','825_payroll_settlement_unsent_retry.sql','826_payroll_employer_retirement_eligibility.sql','827_payroll_employer_retirement_ledger.sql','828_payroll_employer_retirement_accounting.sql','829_payroll_employer_retirement_remittance.sql','830_payroll_benefit_continuation.sql','831_payroll_health_plan_qualification.sql','832_payroll_health_election.sql','833_payroll_historical_employment_wages.sql'])await pool.query(await fs.readFile(new URL(`../../migrations/${file}`,import.meta.url),'utf8'))
