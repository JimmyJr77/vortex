import {retirementReceiptLocationInput} from './retirementReceiptBinding.js'
import {createHash,randomUUID} from 'node:crypto'
import {retirementScheduleUuid as uuid} from './retirementDispatchScheduleState.js'
const fail=(message,status=409)=>Object.assign(new Error(message),{status})
export async function retirementReplacementReceiptBindingState(db,facility,id){
 if(!uuid(id))throw fail('Choose the retained allocation delivery.',400)
 const source=(await db.query(`SELECT a.id,(a.preview->>'configurationId')::uuid AS configuration_id,a.file_name,a.preview->>'planId' AS plan_id,(a.preview->'allocation'->>'planRevisionId')::uuid AS plan_revision_id,(a.preview->'allocation'->>'formatId')::uuid AS format_id,cl.authorization_id AS claim_id,cl.created_at AS claimed_at,c.display_configuration,
 EXISTS(SELECT 1 FROM payroll_retirement_replacement_cancellation WHERE authorization_id=a.id) AS cancelled
 FROM payroll_retirement_replacement_authorization a JOIN payroll_retirement_sftp_configuration c ON c.id=(a.preview->>'configurationId')::uuid LEFT JOIN payroll_retirement_replacement_claim cl ON cl.authorization_id=a.id AND cl.kind='ALLOCATION' WHERE a.facility_id=$1 AND a.id=$2`,[facility,id])).rows[0]
 ...
 const history=(await db.query('SELECT id,claim_id,contract_id,revision,disposition,directory,file_name,reference,created_at FROM payroll_retirement_replacement_receipt_binding WHERE facility_id=$1 AND allocation_id=$2 ORDER BY revision DESC',[facility,id])).rows,latest=history[0]
...
export function registerRetirementReplacementReceiptBindings(app,pool){
 const path='/api/admin/payroll/retirement-replacement-authorizations/:id/receipt-binding'
 const endpoint=work=>async(req,res)=>{res.setHeader('Cache-Control','no-store');let db;try{db=await pool.connect();await db.query('BEGIN');const facility=req.canonicalAccess.facilityId;await db.query('SELECT pg_advisory_xact_lock(hashtextextended($1,0))',[`payroll-payment-connection:${facility}`]);await db.query('SELECT facility_id FROM payroll_settings WHERE facility_id=$1 FOR UPDATE',[facility]);const state=await retirementReplacementReceiptBindingState(db,facility,req.params.id),data=await work(db,req,facility,state);await db.query('COMMIT');res.json({success:true,data})}catch(e){await db?.query('ROLLBACK').catch(()=>{});res.status(e.status||500).json({success:false,message:e.status?e.message:'Unable to retain receipt binding.'})}finally{db?.release()}}
 ...
 app.post(path,endpoint(async(db,req,facility,state)=>{
  ...
  const prior=(await db.query('SELECT id,request_fingerprint FROM payroll_retirement_replacement_receipt_binding WHERE facility_id=$1 AND request_key=$2',[facility,b.requestKey])).rows[0]
