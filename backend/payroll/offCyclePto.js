import {ptoStateReviewInput,ptoStateReviewBasis,applyPtoStateReview} from './ptoStateWithholding.js'
...
export async function loadOffCyclePtoPreview(db,facility,periodId,paymentDate,context,runId=null){
 ...
 try{
  ...
  calculated.ptoStateWithholdingBasis=ptoStateReviewBasis({facility:String(facility),employeeId:String(employeeId),payoutId:String(payout.id),paymentDate,year,workState:employee.work_state,residenceState:employee.residence_state,grossWagesCents:calculated.grossPayCents,marylandWagesCents:calculated.grossPayCents-pretax,pretaxCents:pretax,rothCents:roth,federalIncomeTaxCents:federal,ficaCents:calculated.socialSecurityTaxCents+calculated.medicareTaxCents+calculated.additionalMedicareTaxCents,retirementProposalFingerprint:proposal?.fingerprint||null,payoutFingerprint:payout.review.fingerprint,historyFingerprint:history.fingerprint,historySource:context.historySource,federalMethod:context.federalMethod||'FLAT_22',aggregateFingerprint:aggregateBasis?.fingerprint||null,taxElection:election})
  if(!context.stateWithholdingReview)throw fail('Record a verified state income-tax calculation for this standalone unused-vacation payout before approval.')
  calculated.ptoStateWithholdingReview=applyPtoStateReview(context.stateWithholdingReview,calculated.ptoStateWithholdingBasis)
  state=calculated.ptoStateWithholdingReview.stateIncomeTaxCents
 ...
 calculated.withholdingMethod=federal!==null&&state!==null?'2026-standalone-federal-supplemental-pto-reviewed-md'+(proposal?'-401k':''):null
