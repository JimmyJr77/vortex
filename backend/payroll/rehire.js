...
export function registerRehireRoutes(app,pool){
 app.post('/api/admin/payroll/employees/:id/rehire',async(req,res)=>{
  ...
  try{
   ...
   const tasks=(await db.query("SELECT id,task_key FROM payroll_onboarding_task WHERE facility_id=$1 AND employee_id=$2 AND task_key<>'AVAILABILITY' ORDER BY id FOR UPDATE",[facility,employee.id])).rows
