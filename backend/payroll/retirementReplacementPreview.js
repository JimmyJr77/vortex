...
export async function prepareRetirementReplacement(db,facility,id,input,{fetcher=fetch,paymentFetcher=fetch,reader=readRetirementSftpReceipt,transfer=transferRetirementAllocation,now=()=>new Date(),reservationId=null,requireDelivered=false}={}){
 ...
 const absent=await transfer(config,{name:input.fileName,bytes:Buffer.from(file.csv)},{mode:requireDelivered?'RECOVER':'VERIFY_ABSENCE'})
 if(requireDelivered?absent.status!=='REMOTE_FILE_VERIFIED':absent.status!=='REMOTE_PATHS_ABSENT')throw fail(requireDelivered?'Verify the exact delivered replacement allocation file before funding.':'The replacement staging and delivery paths must both be absent.')
