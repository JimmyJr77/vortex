...
for(const authorizedWorker of [false,true])test(`admin completes ${authorizedWorker?'finite-authorization':'citizen'} receipt replacement and recovers a lost response`,async({page})=>{
 ...
 try{
  ...
  for(const [label,value] of [['Actual replacement document title',authorizedWorker?'Employment Authorization Document':'U.S. Passport'],['Replacement issuing authority',authorizedWorker?'USCIS':'U.S. Department of State'],['Replacement document number','SYNTHETIC-ACTUAL'],['Replacement expiration (if any)',authorizedWorker?'2030-01-01':'2036-01-01'],['Receipt examiner full name','Reviewer Alice'],['Receipt examiner initials','RA'],['Amendment explanation','The actual document replaces the retained lost-document receipt.\nOriginal retained notes remain available.']])await work.getByLabel(label,{exact:true}).fill(value)
  await work.getByRole('button',{name:'Save unfinished receipt amendment',exact:true}).click()
  await expect(work.getByRole('status')).toContainText('saved securely')
  await work.getByLabel('Replacement document number',{exact:true}).fill('UNSAVED')
  await expect(work.getByText('You have unsaved receipt entries or examination notes.',{exact:true})).toBeVisible()
  await page.reload();await page.getByRole('button',{name:'People & onboarding',exact:true}).click();await page.locator('summary').filter({hasText:'Employer I-9 review'}).click();await records.locator('summary').filter({hasText:'Current certification'}).click();await records.getByRole('button',{name:'Replace receipt with actual document',exact:true}).click()
  await expect(work.getByLabel('Replacement document number',{exact:true})).toHaveValue('SYNTHETIC-ACTUAL')
  ...
  for(const [label,value] of [['Examiner identity and authority','Authenticated examiner reviewed the actual replacement document.'],['How the actual document matches this receipt','The original document matches the previously retained receipt.'],['Official receipt acceptance rule URL','https://www.uscis.gov/i-9-central'],['Document acceptance and future-review evidence','Unexpired original document matches the retained receipt.'],['Verified document validity through (if any)',authorizedWorker?'2030-01-01':'2036-01-01']])await work.getByLabel(label,{exact:true}).fill(value)
  for(const [label,value] of [['Replacement examination method','PHYSICAL'],['Original examined in employee’s physical presence','yes'],['Replacement document acceptance','STANDARD'],['Employment authorization has no expiration',authorizedWorker?'no':'yes'],['This document requires reverification',authorizedWorker?'yes':'no'],['Next receipt follow-up action',authorizedWorker?'REVERIFICATION':'NONE'],['No further follow-up is required',authorizedWorker?'no':'yes']])await work.getByLabel(label,{exact:true}).selectOption(value)
  if(authorizedWorker){await work.getByLabel('Current authorization expiration (if any)',{exact:true}).fill('2030-01-01');await work.getByLabel('Next follow-up date (if required)',{exact:true}).fill('2030-01-01')}
  ...
  await work.getByRole('button',{name:'Save unfinished receipt amendment',exact:true}).click()
  await expect(work.getByText('Unfinished receipt amendment saved securely for your admin account.',{exact:true})).toBeVisible()
  await work.getByRole('button',{name:'Reload saved receipt amendment (replaces unsaved entries)',exact:true}).click()
  await expect(work.getByText('Your private unfinished receipt amendment was restored. Review and confirm everything again.',{exact:true})).toBeVisible()
  await work.getByRole('button',{name:'Prepare receipt amendment',exact:true}).click()
  await expect(work.getByLabel('Receipt examiner electronic signature',{exact:true})).toHaveValue('',{timeout:30000})
  await expect(work.getByLabel('Document acceptance and future-review evidence',{exact:true})).toHaveValue('Unexpired original document matches the retained receipt.')
  ...
  await copies.getByRole('checkbox',{name:'Use replacement copy 1',exact:true}).check();await copies.getByRole('button',{name:'Review replacement copy 1 (2 pages)',exact:true}).click()
  ...
  await work.getByLabel('Examiner identity and authority',{exact:true}).fill('Authenticated named examiner reviewed the actual replacement document.\nEmployee was present.')
  await expect(work.getByLabel('Receipt examiner electronic signature',{exact:true})).toHaveValue('',{timeout:30000})
  for(const label of checks)await expect(work.getByRole('checkbox',{name:label,exact:true})).not.toBeChecked()
  for(const label of checks)await work.getByRole('checkbox',{name:label,exact:true}).check()
  await work.getByLabel('Receipt examiner electronic signature',{exact:true}).fill('Reviewer Alice')
  ...
  const next=(await h.pool.query("SELECT kind,due_on::text FROM payroll_i9_signature_followup WHERE row_key LIKE 'RECEIPT:%'")).rows
  expect(next).toEqual(authorizedWorker?[{kind:'REVERIFICATION',due_on:'2030-01-01'}]:[])
