...
test('annual review exposes retirement payment evidence and clears it when sources disappear or refresh fails', async ({page}) => {
  ...
  const report = {year: 2026, status: 'RECONCILED', pretaxDeferrals: '100.25', rothDeferrals: '50.99', hasEmployeeDeferrals: true, hasEmployerContributions: true, employerMatching: '60.00', employerNonelective: '40.00', records: [
    {runId: '101', paymentDate: '2026-09-15', planId: 'standard', planName: 'Synthetic Standard Plan', ordinaryPretaxCents: 10025, ordinaryRothCents: 2000, catchUpPretaxCents: 0, catchUpRothCents: 3099, employerMatchingCents: 6000, employerNonelectiveCents: 4000},
  ]}
  ...
  await expect(evidence).toContainText('2026 employer matching obligations: $60.00')
  await expect(evidence).toContainText('2026 employer nonelective obligations: $40.00')
  await expect(evidence).toContainText('do not establish provider posting')
  ...
  await expect(evidence).toContainText('Employer matching obligation: $60.00')
  await expect(evidence).toContainText('Employer nonelective obligation: $40.00')
