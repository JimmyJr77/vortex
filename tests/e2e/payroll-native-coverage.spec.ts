...
for(const initial of ['HOURLY'])test(`${initial}-first native hourly payroll supplies historical coverage for subsequent salary settlement`,async({page})=>{
 ...
 assert.equal(original.workweekPayments[0].coverage,undefined)
 ...
 assert.equal(evidence.paymentReconciliation.paid[0].coverageSource,'FROZEN_NATIVE_HOURLY')
 ...
 assert.deepEqual((await h.pool.query('SELECT statement_snapshot FROM payroll_run_employee WHERE payroll_run_id=$1',[first.id])).rows[0].statement_snapshot,original)
 ...
 const source=page.getByText(`Run ${first.id}: dated payment coverage verified from its saved hourly payroll.`,{exact:true})
 await expect(source).toBeVisible()
 await expect(source.locator('..')).toContainText('Unsettled difference — straight-time: $0.00 · premium: $0.00')
 await source.locator('..').screenshot({path:'/tmp/payroll-native-coverage-mobile.png'})
