...
for(const initial of ['HOURLY','SALARY'])test(`${initial}-first allocated bonus and paid leave preserve earnings and payment evidence`,async({page})=>{
 ...
 await expect(page.getByText(/Reviewed bonus #\d+: \$100.00 · discretionary annual payment/)).toHaveCount(2)
 await page.getByText('Paid leave 2026-08-08: $30.00 · 1.00 hours',{exact:true}).locator('..').screenshot({path:`/tmp/payroll-allocated-bonus-${initial.toLowerCase()}-mobile.png`})
