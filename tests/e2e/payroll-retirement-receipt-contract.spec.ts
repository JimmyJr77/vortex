...
for(const employer of [false,true])test(`admin retains ${employer?'combined employer':'employee'} receipt definitions after a lost save and source change`,async({page})=>{
 ...
 try{
  ...
  if(employer){
   const allocation=page.getByRole('region',{name:'Recordkeeper allocation format standard',exact:true})
   await allocation.getByRole('combobox',{name:'Recordkeeper amount representation',exact:true}).selectOption('CENTS')
   await allocation.getByRole('combobox',{name:'Recordkeeper date representation',exact:true}).selectOption('ISO')
   await allocation.getByRole('combobox',{name:'Include column headers',exact:true}).selectOption('YES')
   await allocation.getByRole('checkbox',{name:'Include employer matching and nonelective contribution columns',exact:true}).check()
   const inputs=allocation.getByRole('textbox',{name:/column header$/}),columns=[...allocationFormatFixture().columns,...extra.map(field=>({field,header:field}))]
   await expect(inputs).toHaveCount(10)
   for(let i=0;i<columns.length;i++)await inputs.nth(i).fill(columns[i].header)
   await allocation.getByRole('textbox',{name:'Recordkeeper specification and verification reference',exact:true}).fill('Reviewed provider combined employee and employer allocation specification')
   await allocation.getByRole('checkbox',{name:/I verified the exact columns/}).check()
   await allocation.getByRole('button',{name:'Retain allocation format',exact:true}).click()
   await expect(allocation).toContainText('Allocation format revision 2 · VERIFIED')
   expect((await f.api(f.formatPath)).history[0].format.columns).toHaveLength(10)
  }
  ...
  const headers=panel.getByRole('textbox',{name:/^Receipt header:/});await expect(headers).toHaveCount(employer?15:13)
  const receiptColumns=[...f.body.contract.columns,...(employer?extra.map(field=>({field,header:field})):[])]
  for(let i=0;i<receiptColumns.length;i++)await headers.nth(i).fill(receiptColumns[i].header)
  ...
  if(employer){await expect(panel.getByRole('button',{name:'Save reviewed receipt contract',exact:true})).toBeDisabled();await panel.getByRole('checkbox',{name:'I verified separate cumulative employer matching and nonelective amounts in the provider receipt.',exact:true}).check();await panel.getByRole('checkbox',{name:'I confirm this receipt contract action and reviewed specification.',exact:true}).check()}
  ...
  const saved=await f.api(f.path);expect(saved.history).toHaveLength(1);expect(saved.history[0].contract.columns[0].field).toBe('sourceSha256');expect(saved.history[0].contract.employerContributionsConfirmed===true).toBe(employer)
  if(employer){await page.setViewportSize({width:390,height:1100});await panel.screenshot({path:'/tmp/payroll-employer-receipt-columns-mobile.png'});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true)}
  ...
  await f.api(f.formatPath,{planRevisionId:f.body.planRevisionId,expectedRevision:employer?2:1,requestKey:randomUUID(),format:{...allocationFormatFixture(),dateFormat:'US'}})
  await panel.getByRole('button',{name:'Load receipt contract history',exact:true}).click();await expect(panel).toContainText('Receipt contract: SOURCE CHANGED');await expect(reference).toHaveValue('Retained draft review after a provider allocation format change');await expect(headers).toHaveCount(13);await expect(panel.getByRole('checkbox',{name:'I confirm this receipt contract action and reviewed specification.',exact:true})).not.toBeChecked()
  ...
  await page.setViewportSize({width:390,height:1100});await panel.screenshot({path:`/tmp/payroll-retirement-${employer?'employer-':''}receipt-contract-mobile.png`});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);expect(errors).toEqual([])
