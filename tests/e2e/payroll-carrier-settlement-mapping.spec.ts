...
test('admin retains carrier bank mappings with historical funding and changed evidence',async({page})=>{
 ...
 const old=process.env.PAYROLL_DOCUMENT_KEY;process.env.PAYROLL_DOCUMENT_KEY='39'.repeat(32);let bad=false,creates=0
 const accounts=[{Id:'7',Name:'Employer Bank',Active:true,AccountType:'Bank'},{Id:'8',Name:'Carrier Premiums Payable',Active:true,AccountType:'Other Current Liability'},{Id:'9',Name:'Premium Expense',Active:true,AccountType:'Expense'}]
 ...
 try{
  ...
  const panel=page.getByRole('region',{name:'Carrier settlement accounts',exact:true}),load=panel.getByRole('button',{name:'Review carrier settlement accounts',exact:true}),save=panel.getByRole('button',{name:'Verify and retain carrier mapping',exact:true})
  await load.click();await expect(panel).toContainText('No carrier settlement mapping has been retained.')
  await panel.getByLabel('Carrier settlement funding',{exact:true}).selectOption(String(first));await panel.getByLabel('Carrier settlement bank',{exact:true}).selectOption('7');await panel.getByLabel('Carrier settlement liability',{exact:true}).selectOption('8')
  await expect(panel.getByLabel('Carrier settlement bank',{exact:true}).getByRole('option',{name:'Premium Expense'})).toHaveCount(0)
  await panel.getByLabel('Carrier settlement review reference',{exact:true}).fill('Reviewed historical bank and premium liability records');await expect(save).toBeDisabled();await panel.getByRole('checkbox').check()
  bad=true;await save.click();await expect(panel.getByRole('alert')).toContainText('active USD');bad=false;await load.click();await panel.getByRole('checkbox').check();await save.click();await expect(panel.getByRole('status')).toContainText('mapping verified and retained')
  expect((await h.pool.query('SELECT * FROM payroll_carrier_settlement_mapping')).rowCount).toBe(1)
  await page.reload();await page.getByRole('button',{name:'Reports & QuickBooks',exact:true}).click();await load.click();await expect(panel).toContainText('Employer Bank → Carrier Premiums Payable');await expect(panel).toContainText(`Funding revision ${first}`)
  ...
  await panel.screenshot({path:'/tmp/payroll-carrier-settlement-mapping-mobile.png'});expect(creates).toBe(0);expect(errors).toEqual([])
