...
test('coach can browse the collection, inspect daily plans, and open supplemental notes', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /12wk$/ })).toHaveCount(10)
  await page.getByRole('button', { name: 'Rotational Force: Upper Body 12wk', exact: true }).click()
  await expect(page.getByRole('tablist', { name: 'Accelerator programs' }).getByRole('tab')).toHaveCount(10)
  await expect(page.getByRole('heading', { name: 'Build a balanced release' })).toBeVisible()
  ...
  const notes = page.getByRole('button', { name: /Coaching notes for Med Ball Rotational Scoop Toss/ })
  await notes.click()
  await expect(page.getByRole('dialog')).toContainText('Finish balanced for 2 seconds')
  ...
  await expect(page.getByRole('tabpanel', { name: '4. Primary strength' })).toContainText('1 × 4 easy reps / side')
  ...
  await expect(page.getByRole('tabpanel', { name: '1. Prepare' })).toContainText('have not been supplied')

  await page.getByRole('button', { name: 'Class 2:', exact: false }).click()
  await expect(page.getByRole('complementary', { name: 'Equipment for the day' })).toContainText('Battle rope')
  await expect(page.getByRole('complementary', { name: 'Equipment for the day' })).not.toContainText('Slam ball')
  await page.getByRole('tab', { name: '3. Resilience' }).click()
  ...
  await page.getByRole('dialog').getByRole('button', { name: /07 Receive/ }).click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
  await page.getByRole('tab', { name: '2. Explosiveness' }).click()
  await expect(page.getByRole('tabpanel', { name: '2. Explosiveness' })).toContainText('controlled handoffs')

  await page.getByRole('tab', { name: 'Distance Running 12wk', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'Distance Running 12wk' })).toBeVisible()
  await expect(page.getByText('This workout view is next in the collection.')).toBeVisible()
