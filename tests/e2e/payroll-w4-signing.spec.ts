...
for(const exemption of [false,true])test(`employee completes W-4 internally, resumes and signs, then admin applies it: exemption=${exemption}`,async({page})=>{
 ...
 try{
  ...
  const initialSsn=panel.getByLabel('Social security number',{exact:true}),initialAddress=panel.getByRole('textbox',{name:'Address',exact:true})
  await expect(initialSsn).toHaveValue('');await expect(initialAddress).toHaveValue('')
  await expect(initialSsn).toHaveAttribute('autocomplete','new-password');await expect(initialSsn).toHaveAttribute('data-1p-ignore','true');await expect(initialAddress).toHaveAttribute('data-form-type','other')
  ...
  if(!exemption){
   ...
   const job1=worksheet.getByRole('textbox',{name:'Job 1: annual taxable wages in dollars',exact:true})
   const job2=worksheet.getByRole('textbox',{name:'Job 2: annual taxable wages in dollars',exact:true})
   const calculate=worksheet.getByRole('button',{name:'Calculate multiple-job withholding',exact:true})
   const apply=worksheet.getByRole('button',{name:'Apply worksheet to Step 4(c)',exact:true})
   const downloadButton=worksheet.getByRole('button',{name:'Download worksheet for my records',exact:true})
   const step4c=panel.getByRole('textbox',{name:'Step 4(c): Extra withholding in dollars',exact:true})
   await job1.fill('150000')
   await job2.fill('130000')
   ...
   await expect(worksheet.getByRole('status')).toContainText('At least two jobs have annual taxable wages over $120,000 each')
   await expect(worksheet.getByRole('alert')).toHaveCount(0)
   await expect(worksheet.getByRole('link',{name:'Use the IRS withholding estimator',exact:true})).toHaveAttribute('href','https://www.irs.gov/W4App')
   await expect(worksheet.getByRole('link',{name:'Review Publication 505 worksheets',exact:true})).toHaveAttribute('href','https://www.irs.gov/publications/p505')
   await expect(apply).toHaveCount(0)
   await expect(downloadButton).toHaveCount(0)
   await expect(step4c).toHaveValue('10.00')
   await expect(panel.getByRole('checkbox').first()).toBeChecked()
   await step4c.fill('123.45')
   await expect(step4c).toHaveValue('123.45')
   await step4c.fill('10.00')
   await job2.fill('120000')
   await expect(worksheet.getByRole('status')).toHaveCount(0)
   await calculate.click()
   await expect(apply).toBeDisabled()
   await expect(downloadButton).toBeVisible()
   await expect(step4c).toHaveValue('10.00')
   await job2.fill('invalid')
   await calculate.click()
   await expect(worksheet.getByRole('alert')).toBeVisible()
   await expect(apply).toHaveCount(0)
   await expect(downloadButton).toHaveCount(0)
   await job2.fill('30000')
   await expect(worksheet.getByRole('alert')).toHaveCount(0)
   await job1.fill('80000')
   await calculate.click()
   ...
   await calculate.click()
   await expect(apply).toBeDisabled()
   await expect(worksheet.getByRole('checkbox',{name:'This W-4 is for the highest paying job.',exact:true})).not.toBeChecked()
   await worksheet.getByRole('checkbox',{name:'This W-4 is for the highest paying job.',exact:true}).check()
  ...
  if(!exemption)await expect(panel.getByRole('textbox',{name:'Step 4(c): Extra withholding in dollars',exact:true})).toHaveValue('285.00')
  await prepare.click()
  await expect(panel).toContainText('Page 1 of 5 displayed and review visit saved.')
  ...
  expect(pdf.getForm().getTextField('topmostSubform[0].Page1[0].f1_11[0]').getText()||'').toBe(exemption?'':'285.00')
  ...
  await expect(extra).toHaveValue(exemption?'0':'285');await expect(extra).toBeDisabled()
  ...
  expect(elected.federal.extraWithholdingCents).toBe(exemption?0:28500);expect(elected.federal.filingStatus).toBe(exemption?null:'HEAD_OF_HOUSEHOLD');expect(elected.w4Source.submissionId).toBe(rows.rows[0].id);expect(elected.w4ReviewRequired).toBeUndefined()
