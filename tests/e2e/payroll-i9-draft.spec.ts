...
test('I-9 employee draft resumes, retries once and displays all official instructions',async({page})=>{
 ...
 try{
  ...
  await admin.getByLabel('E-Verify participation status verified on',{exact:true}).fill('2026-09-02')
  await expect(admin.getByText("The employer's E-Verify enrollment status and hiring-site participation records were reviewed on 2026-09-02 and confirm that the employer and hiring site do not participate in E-Verify.",{exact:false})).toBeVisible()
  await admin.getByRole('combobox',{name:'Employer and hiring-site E-Verify participation',exact:true}).selectOption('true')
  await expect(admin.getByText('the employer and hiring site participate in E-Verify.',{exact:false})).toBeVisible()
  await admin.getByRole('combobox',{name:'Employer and hiring-site E-Verify participation',exact:true}).selectOption('false')
  await expect(admin.getByLabel('Offer and participation verification evidence',{exact:true})).toHaveCount(0)
  ...
  await expect(admin.getByLabel('E-Verify participation status verified on',{exact:true})).toHaveValue('2026-09-02')
