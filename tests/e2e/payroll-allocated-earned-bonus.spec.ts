...
import {statementLines} from '../../backend/payroll/payStatement.js'
for(const initial of ['HOURLY','SALARY'])test(`${initial}-first earned bonus and leave settle with linked additional overtime`,async({page})=>{
 ...
 await expect(page.getByText('Paid leave 2026-08-08: $30.00 · 1.00 hours',{exact:true})).toBeVisible()
 await expect(page.getByText(/earned annual payment · additional overtime \$10.00/)).toBeVisible()
 await page.getByText('Paid leave 2026-08-08: $30.00 · 1.00 hours',{exact:true}).locator('..').screenshot({path:`/tmp/payroll-allocated-earned-${initial.toLowerCase()}-mobile.png`})
