...
for(const salary of [false,true])test(`admin reopens a former employee with fresh onboarding: salary=${salary}`,async({page})=>{
 ...
 try{
  ...
  await expect(page.getByText('0 of 12 steps complete',{exact:true})).toBeVisible()
  ...
  await expect(page.getByText('1 of 12 steps complete',{exact:true})).toBeVisible()
  ...
  expect((await h.pool.query('SELECT COUNT(*)::int n FROM payroll_onboarding_task WHERE employee_id=$1 AND onboarding_cycle=2',[e.id])).rows[0].n).toBe(12)
  if(!salary){
