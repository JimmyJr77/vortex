...
test('employee sees unresolved returned funding despite a retained participant posting',async({page})=>{
 ...
 await expect(panel).not.toContainText('Provider-reported posted amount: $14.00');await expect(panel).not.toContainText('PRIVATE-PARTICIPANT');await expect(panel).not.toContainText('Synthetic batch 1')
  await page.setViewportSize({width:390,height:1100});await panel.screenshot({path:'/tmp/payroll-employee-retirement-return-mobile.png'});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);expect(errors).toEqual([])
