...
test('admin reviews signed final-pay benefits, retries a lost response and refreshes changed coverage before payroll',async({page})=>{
 ...
 try{
  const {api,employee,periods,coverageBody}=await benefitContinuationFixture(h)
  ...
  await panel.screenshot({path:'/tmp/payroll-benefit-continuation-mobile.png'});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);expect(errors).toEqual([])
