...
test('admin retains annual sources with exact retry and preserves a draft when another review arrives',async({page})=>{
 ...
 try{
  ...
  const employer=panel.getByRole('region',{name:'External employer funding balances',exact:true})
  await employer.getByRole('combobox',{name:'Employer balance review',exact:true}).selectOption('REVIEWED')
  await employer.getByRole('textbox',{name:'External compensation under the employer funding formula ($)',exact:true}).fill('10000')
  await employer.getByRole('textbox',{name:'This plan’s external employer matching contributions ($)',exact:true}).fill('350')
  await employer.getByRole('textbox',{name:'This plan’s external employer nonelective contributions ($)',exact:true}).fill('150')
  await employer.getByRole('textbox',{name:'External employer funding evidence',exact:true}).fill('Reviewed actual external employer compensation and contribution statements.')
  await panel.getByRole('textbox',{name:'External annual additions for the employer limit group ($)',exact:true}).fill('500')
  ...
  const retained=(await h.pool.query('SELECT facts FROM payroll_retirement_annual_source')).rows[0].facts
  expect(retained.employerFunding).toEqual({compensationCents:1000000,matchingCents:35000,nonelectiveCents:15000,reference:'Reviewed actual external employer compensation and contribution statements.'})
  expect(retained.externalPlanCompensationCents).toBe(0)
  await panel.getByText('Annual source revision 1 · 2026-09-11',{exact:true}).click()
  await employer.getByRole('textbox',{name:'This plan’s external employer matching contributions ($)',exact:true}).fill('1')
  await panel.getByRole('button',{name:'Copy retained annual inputs into draft',exact:true}).click()
  await expect(employer.getByRole('textbox',{name:'This plan’s external employer matching contributions ($)',exact:true})).toHaveValue('350')
  await expect(employer.getByRole('textbox',{name:'External compensation under the employer funding formula ($)',exact:true})).toHaveValue('10000')
