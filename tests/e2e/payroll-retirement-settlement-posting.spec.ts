...
test('retirement settlement commits one claim, recovers lost journal response and preserves cancellation/retry history',async({page})=>{
 ...
 try{
  ...
  await page.setViewportSize({width:390,height:1100});await panel.screenshot({path:'/tmp/payroll-retirement-settlement-posting-mobile.png'});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true)
