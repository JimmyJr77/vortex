...
test('admin maps separate retirement accounts for direct sync and CSV export',async({page})=>{
 ...
 const expense={Id:'8',FullyQualifiedName:'Employer retirement expense',Active:true,AccountType:'Expense',CurrencyRef:{value:'USD'}}
 const h=await createHarness({quickbooksFetcher:async(url,options)=>{expect(options.method).toBe('GET');return new Response(JSON.stringify(String(url).includes('/account/7')?{Account:account}:String(url).includes('/account/8')?{Account:expense}:{QueryResponse:{Account:[account,expense]}}),{status:200})}})
 ...
 try{
  ...
  await page.getByRole('combobox',{name:'Employer retirement expense (required for employer contributions)',exact:true}).selectOption('8')
  ...
  expect((await h.pool.query('SELECT account_ids FROM payroll_quickbooks_connection')).rows[0].account_ids.employerRetirement).toBe('8')
  await page.getByLabel('Employer Retirement Expense Account',{exact:true}).fill('Employer retirement expense')
  ...
  expect((await h.pool.query('SELECT employer_retirement_expense_account FROM payroll_accounting_mapping WHERE facility_id=1')).rows[0].employer_retirement_expense_account).toBe('Employer retirement expense')
