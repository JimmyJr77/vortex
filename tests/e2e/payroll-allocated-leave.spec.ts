...
for(const initial of ['HOURLY','SALARY'])test(`${initial}-first allocated paid leave settles across adjacent periods without duplicate pay or worked leave hours`,async({page})=>{
 ...
 assert.equal(frozen.reduce((n,r)=>n+Number(r.regular_pay_cents)+Number(r.overtime_pay_cents)+Number(r.other_taxable_pay_cents),0),initial==='HOURLY'?200200:220500)
 ...
 await page.getByText('Paid leave 2026-08-08: $30.00 · 1.00 hours',{exact:true}).locator('..').screenshot({path:`/tmp/payroll-paid-leave-${initial.toLowerCase()}-mobile.png`})
