...
test('retirement timing retains a lost-response retry and refreshes contribution dates and suspension',async({page})=>{
 ...
 try{
  ...
  await timing.getByRole('checkbox',{name:/I verified that employer matching/}).check()
  await timing.getByRole('textbox',{name:'Employer contribution funding evidence',exact:true}).fill('Plan and provider instructions confirm employer matching and nonelective funding with each payroll')
  await timing.getByRole('checkbox',{name:/I reviewed actual segregation capacity/}).check();await timing.getByRole('button',{name:'Retain contribution timing',exact:true}).click();await expect(timing).toContainText('Synthetic lost timing response')
  ...
  expect((await h.pool.query('SELECT policy FROM payroll_retirement_timing_review')).rows[0].policy.employerFunding).toEqual({schedule:'WITH_PAYROLL',confirmed:true,reference:'Plan and provider instructions confirm employer matching and nonelective funding with each payroll'})
  await expect(timing).toContainText('Employer matching and nonelective contributions follow this payroll schedule. Evidence:')
  await expect(source).toContainText('Reviewed deposit target: 2026-09-22',{timeout:40000})
  await page.reload();await page.getByRole('button',{name:'Employer setup',exact:true}).click();await page.getByRole('button',{name:'Load retirement plan history',exact:true}).click()
  await expect(timing).toContainText('Employer matching and nonelective contributions follow this payroll schedule. Evidence:')
  await timing.getByLabel('Timing applies to withholding from',{exact:true}).fill('2026-01-01')
  await timing.getByRole('textbox',{name:'Employer capacity and provider timing evidence',exact:true}).fill('Suspend reliance pending updated plan and provider instructions')
  await page.setViewportSize({width:390,height:1100});await timing.screenshot({path:'/tmp/payroll-retirement-timing-mobile.png'})
  await timing.getByRole('combobox',{name:'Timing disposition',exact:true}).selectOption('SUSPENDED');await timing.getByRole('checkbox',{name:/I reviewed actual segregation capacity/}).check();await timing.getByRole('button',{name:'Retain contribution timing',exact:true}).click()
