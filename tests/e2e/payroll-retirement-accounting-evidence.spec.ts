...
import {runRetirementContributionSweep} from '../../backend/payroll/retirementContributionAutomation.js'
test('admin sees verified settlement accounting and reopens review after changed source journal',async({page})=>{
 test.skip(!process.env.PAYROLL_TEST_DATABASE_URL,'Requires isolated database');test.setTimeout(90000)
 const old=process.env.PAYROLL_DOCUMENT_KEY;process.env.PAYROLL_DOCUMENT_KEY=randomBytes(32).toString('hex');let journal=null,settlement=null,posts=0;let changed=false;const closed=false,wrong=false
 ...
 try{
  ...
  await panel.getByRole('checkbox',{name:'I confirm the retained retirement settlement posting action.',exact:true}).check();await panel.getByRole('button',{name:'Post authorized retirement settlement',exact:true}).click();await expect(panel).toContainText('Journal outcome: UNCERTAIN')
  await panel.getByRole('button',{name:'Recover retirement settlement journals',exact:true}).click();await expect(panel).toContainText('Journal outcome: SYNCED');expect(posts).toBe(1)
  await page.getByRole('button',{name:'Review contribution reconciliation',exact:true}).click();const assessment=page.getByRole('region',{name:`Contribution reconciliation ${a.id}`,exact:true});await expect(assessment).toContainText('Settlement accounting: MATCHED')
  const retained=assessment.getByRole('region',{name:'Contribution assessment history',exact:true})
  await runRetirementContributionSweep(h.pool,{facility:1})
  await retained.getByRole('button',{name:'Refresh latest contribution history',exact:true}).click()
  await expect(retained).toContainText('Admin assessment')
  await retained.getByRole('button',{name:'Check contribution now',exact:true}).click()
  await expect(retained).toContainText('Recorded outcome: REVIEW REQUIRED')
  changed=true;await panel.getByRole('button',{name:'Recover retirement settlement journals',exact:true}).click();await expect(assessment).toContainText('Settlement accounting: REVIEW REQUIRED',{timeout:40000})
  await retained.getByRole('button',{name:'Check contribution now',exact:true}).click()
  await expect(retained).toContainText('Admin assessment')
  await h.pool.query('INSERT INTO payroll_retirement_contribution_failure(facility_id,authorization_id,message) VALUES(1,$1,$2)',[a.id,'Synthetic reconciliation read failure; retained for recovery history.'])
  await expect(retained).toContainText('Synthetic reconciliation read failure',{timeout:40000})
  await page.setViewportSize({width:390,height:1100});await assessment.screenshot({path:'/tmp/payroll-retirement-accounting-evidence-mobile.png'});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true)
