...
test('admin reviews recordkeeper format, retries a lost save, downloads exact allocations and suspends export',async({page})=>{
 ...
 try{
  ...
  await format.getByRole('checkbox',{name:/I verified the exact columns/}).check();await format.getByRole('button',{name:'Retain allocation format',exact:true}).click();await expect(format).toContainText('Synthetic lost format response')
  ...
  await format.getByRole('combobox',{name:'Allocation format disposition',exact:true}).selectOption('SUSPENDED');await format.getByRole('checkbox',{name:/I verified the exact columns/}).check();await format.getByRole('button',{name:'Retain allocation format',exact:true}).click();await expect(format).toContainText('Allocation format revision 2 · SUSPENDED')
