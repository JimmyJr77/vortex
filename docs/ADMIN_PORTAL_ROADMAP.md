...
Recurring class pricing is resolved for the entire family before a member filter is applied.
Tuition promos run first, then the family multi-class tier uses all eligible post-promo class
tuition. Annual memberships, waitlists, one-time classes, paused enrollments, and out-of-period
classes are excluded. Annual membership entitlements and yearly renewals remain member-specific.
...
+ `NotificationBell` (live `/api/admin/notifications`, thread deep links) + `PortalNavButtons` + scrollable tabs) over a white `container-admin` content area. Tabs are
...
| Class Master | `classSetup/AdminClassSetupOverview.tsx` | Spreadsheet of all classes; unlock to edit cells; Copy mode pastes fields incl. sport tags + full schedule (replaces target slots); Schedule column shows `[active dates] · [day/time] · [spaces]` and embeds slots with existing-slots card above the builder (Scheduling tab keeps card below) | `classes.view` |
...
| Billing Overview | `AdminBillingOverview.tsx` | Facility-wide household table: YTD paid, last 3 months billed/paid, outstanding, upcoming recurring, future credits, account balance, autopay, card last4 or Link; family name opens Account Billing | `billing.view` |
| Customer Billing | `AdminCustomerBilling.tsx` | Household lookup, family-wide class pricing, individual annual memberships and auto-renewal, ledger/activity audit, custom collections, refunds, statements, billing contact and payment-method links | `billing.view` (writes use `billing.manage`, `family_billing.manage`, or `billing.statements.manage`) |
...
- **Customer Billing** ([backend/billing/customerBillingRoutes.js](../backend/billing/customerBillingRoutes.js); `billing.*`, `family_billing.manage`, `billing.statements.manage`): facility-wide `GET /api/admin/customer-billing/overview` (all member families: YTD, last 3 months billed/paid, outstanding, upcoming recurring, future credits, account balance, autopay, Stripe last4 or Link), normalized individual/family search, household overview, whole-family effective class pricing, member-level annual memberships and auto-renewal, paged transactions/activity, CSV export, exact custom collections, refund previews/treatments, and payment-method links. [familyEnrollmentPricing.js](../backend/billing/familyEnrollmentPricing.js) remains household-scoped even when the response is filtered to one member. Existing statement, receipt, billing-contact, and Stripe operation services remain canonical.
...
| Billing | `family_billing_account`, `billing_charge`, `billing_payment`, `billing_refund`, `billing_subscription`, `billing_statement`, `billing_statement_line`, `enrollment_price_adjustment`, `billing_payment_application`, `billing_account_activity` | `008`, `010`, `053`, `763`, `768`, `769` |
