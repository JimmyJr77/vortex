...
- The fresh integrated browser run at `ec69aa82` finished with **184 passes and 23 failures out of 207** (1.1 hours), including failures in all four native complete journeys. Its exact log hash, unchanged 3,503 source inputs and failure list are recorded in `PAYROLL_BROWSER_INTEGRATED_2026_09_21.json`. This is the latest full browser result; the older four passing native journeys do not establish current integrated stability.
...
## Integrated browser failure investigation

The unchanged-source `ec69aa82` run is non-green: 184 passed / 23 failed, including four full native journeys. A separate retirement-return reproduction retained a trace at `/tmp/payroll-return-refresh-diagnostic-results/payroll-employee-retiremen-db2f8-etained-participant-posting/trace.zip`. It shows Vite connecting at 11,649ms, connected at 11,806ms, reporting connection lost at 14,114ms and navigating/reconnecting at 14,147ms. The page returned to Home before its 30-second contribution refresh. Only the initial contribution read is retained in that trace; provider return assertions passed. This proves a development reload disrupted that scenario, not that every failure has the same cause.

A separate I-9 failure is a test selector defect: the unqualified status locator matched five status elements after PDF review. A candidate fix selects the exact Draft restored status; validation is pending. Other timeouts and UI failures remain unclassified.

An isolated candidate now includes a reviewed retry for QuickBooks settlement attempts with retained proof that no create request was attempted. It preserves immutable jobs, commits each retry claim before external work, blocks unknown/transmitted/pending attempts, verifies current sources and provider absence, and exposes review history. Nineteen focused tests, six distinct local API scenarios, TypeScript and one complete browser retry journey passed; production source is not yet changed. Failed runner setup attempts remain in `/tmp/payroll-settlement-unsent-browser*.log`. The successful browser run uses installed Node 22, Playwright's asynchronous module loader and an isolated same-origin server. Its mobile result was visually inspected.

A static test frontend was built successfully in the candidate, and its payroll sign-in page was verified in the browser with no captured errors. Retirement refresh, the I-9 selector correction and the unassisted native journey are being reproduced against this build without Vite's development HMR connection. Tests retain their original business assertions. Candidate location and ongoing handles are in `/tmp/vortex-settlement-unsent-candidate/IMPLEMENTATION_STATUS.md`; candidate artifacts are not release evidence until integrated and verified against main.
