...
| **Informational MOP** | 1 | C4-MOP-11 `audience_skill_level_adherence` (legacy check id; difficulty-cap adherence) |
...
| C4-MOP-04 | Training experience (legacy check id) | request experience vs profile | yes | `audience_implied_skill_level` | yes |
| C4-MOP-05 | Scaling cohort | age + training experience → cohort matrix | yes | `audience_scaling_cohort` | yes |
...
| C4-MOP-11 | Difficulty-cap adherence (legacy check id) | exercise difficulty vs audience caps | yes | `audience_skill_level_adherence` | yes (info) |
...
| C4-MOS-01 | Audience inputs valid | age order, training-experience enum | yes | `audience_inputs_valid` | yes |
