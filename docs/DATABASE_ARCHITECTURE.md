...
**Event calendar + messaging ([070](../backend/migrations/070_event_calendar_and_rsvp.sql), [073](../backend/migrations/073_event_calendar_item_classes.sql)):** `coaching.event_calendar_item` stores 5 Ws rows per `public.events` row; `coaching.event_rsvp` tracks member RSVP. **`073`** adds junction `coaching.event_calendar_item_class (calendar_item_id, scheduling_form_id)` so schedule inbox rows can scope visibility to enrolled classes. Event threads link via `coaching.message_thread_link` (`object_type='event'`, roles `canonical` | `discussion`); multiple discussion boards per event are supported via [messageEventThreads.js](../backend/platform/messageEventThreads.js) `provisionAdditionalEventBoard`.
...
| `signupRows[].pricingBreakdown.totals.totalCents` as admin "Est. cost" per class | old `MemberEnrollmentsTab` via `GET .../pricing-summary` ([handlers.js](../backend/scheduling/handlers.js) `adminMemberPricingSummary`) | **Superseded** for the admin accounts→enrollments view by `GET .../enrollments` ([adminEnrollmentsView.js](../backend/scheduling/adminEnrollmentsView.js)), which uses the live preview `existingClasses[].monthlyPrice` (marginal allocation that reconciles to the true monthly total) and `billing_subscription` gross/net. The stored `scheduling_signup.pricing_breakdown` snapshot encodes bundle/tier math from enrollment time and does **not** reconcile per-class (root cause of "$300/class" while total was $600). `pricing_breakdown` is still used for receipts — do not drop; only its per-class display usage was removed. `/pricing-summary` still powers [MemberPricingModal.tsx](../src/components/admin/MemberPricingModal.tsx) — retire only after that caller migrates. |
...
| `sport_pricing_default` table + `GET/PUT /api/admin/scheduling/sport-defaults` | [add_discount_engine.sql](../backend/migrations/add_discount_engine.sql), removed from [registerRoutes.js](../backend/scheduling/registerRoutes.js) (2026-06) | Program/class pricing only via [pricingDefaults.js](../backend/programs/pricingDefaults.js). Pre-drop: `SELECT COUNT(*) FROM sport_pricing_default;` and `SELECT COUNT(*) FROM pricing_benefit_selection WHERE scope_level = 'sport';` |
...
| Dead pricing emails `scheduling/confirmationEmail.js` (`sendConfirmationEmail`), `scheduling/waitlistEmail.js` (`sendWaitlistEmail`) | Superseded (Billing Overhaul Phase 0) | Enrollment receipt now carries a **cost summary** via `buildReceiptPricingSummary`/`formatReceiptPricingSummaryEmail` in [pricing.js](../backend/scheduling/pricing.js), rendered by [enrollmentReceiptEmail.js](../backend/email/enrollmentReceiptEmail.js) and [EnrollmentReceiptPage.tsx](../src/components/EnrollmentReceiptPage.tsx) | Candidate: still zero imports (dead code). Do not re-wire; remove once confirmed unused. |
...
| `048_program_pricing_cost_options.sql` / `049_multi_class_pass.sql` not in [initPlatformTables](../backend/platform/initTables.js) list | Columns `programs.pricing_cost_options` + `programs.multi_class_pass_packages` are only created lazily by `ensureProgramPricingColumns()`; the **public offerings/enroll read path** (`loadFormDetail` → `loadProgramPricingRow`) read them without triggering the ensure, so on a fresh boot before any program-management route ran, `/api/scheduling/forms/:id/offerings` `500`'d with `column "pricing_cost_options" does not exist` (seen transiently right after a deploy/restart). | **Fixed** (2026-07): `loadFormDetail` now calls `ensureProgramPricingColumns(pool)` before reading pricing columns (self-heals the public path). Long-term: add `048`/`049` to the `initPlatformTables` list (mind `049` FKs to `billing_charge`/`scheduling_signup`) or `npm run migrate:all` per environment. |
