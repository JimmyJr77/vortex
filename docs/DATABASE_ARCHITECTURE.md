...
**Participant structure ([179](../backend/migrations/179_coaching_exercise_participant_structure.sql), registered in [initTables.js](../backend/platform/initTables.js)):** adds `coaching.exercise.participant_structure TEXT NOT NULL DEFAULT 'individual'` with named CHECK `coaching_exercise_participant_structure_check` (`individual` | `pairs` | `group`) — how many athletes a drill needs. The whole migration is wrapped in a `DO` block guarded on the column not existing, so the heuristic backfill runs **once per database** and boot re-runs never overwrite later coach edits. Backfill rules: (1) name contains "partner" → `pairs`; (2) name contains "two-partner" → `group`; (3) `coaching_execution` setup/execution_steps require a partner with no "coach or partner" / wall / pad / equipment alternative and the partner isn't merely spotting → `pairs`. Spotters and coach-delivered cues deliberately do NOT count as pairs (`requires_spotting` lives on `exercise_safety_profile` and is unrelated). API: `GET /api/coach/exercises?paired=true` (pairs+group) and `?participant_structure=` filters; column accepted on exercise POST/PUT; surfaced in `buildExerciseCard` `movement_identity`.
