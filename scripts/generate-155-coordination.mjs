/**
 * Generates:
 *   backend/migrations/157_coaching_coordination_infrastructure_and_seed.sql
 *   backend/migrations/158_coaching_coordination_cards.sql
 *
 * Run: node scripts/generate-155-coordination.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/coordination-all-cards.json')
const data = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))
const CARDS = data.cards
const SLUGS = CARDS.map((c) => c.slug)

const PHASE_ORDER_BASE = {
  movement_intelligence: 600,
  output: 520,
  resilience: 570,
}
...
const VALIDATION_EDU = {
  title: 'Coordination under duress without conditioning drift or wrong phase intent',
  short_summary:
...
function skillForCard(card) {
  ...
  if (risk >= 4 || card.primaryPhaseKey === 'output') return 'INTERMEDIATE'
...
let seedSql = `-- Top 50 Coordination Under Duress Exercise Library — infrastructure, taxonomy, and 50-card seed.
...
INSERT INTO coaching.equipment (key, name, sort_order) VALUES
  ('colored_ball', 'Colored Ball / Multi-Color Balls', 24),
  ('balance_pad', 'Balance Pad / Airex Pad', 25),
  ('football', 'Football', 26),
  ('ladder', 'Agility Ladder', 27)
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} coordination slots\n`
...
seedSql += `-- Insert 50 coordination exercises
...
seedSql += CARDS.map((card) => {
  ...
  return `  (${sqlStr(card.name)}, ${sqlStr(card.slug)}, ${sqlStr(card.description)}, ${sqlStr(skill)}, 8, ${sqlInt(d.default_sets ?? 2)}, ${sqlInt(reps)}, ${sqlInt(work)}, ${sqlInt(d.default_rest_seconds ?? 45)}, ${sqlInt(d.est_seconds_per_set ?? 75)}, ${sqlStr(card.cardSummary)}, ${sqlStr(card.coachLanguage)}, ${sqlStr(card.athleteLanguage)}, ${sqlStr(card.family)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.subrole)}, ${sqlStr(card.slot)}, ${jsonb(seedReq(card)).replace('::jsonb', '')}::jsonb, ${jsonb(seedExec(card)).replace('::jsonb', '')}::jsonb)`
}).join(',\n')
...
seedSql += CARDS.map((card) => {
  const d = card.dosage ?? {}
  return `  (${sqlStr(card.slug)}, ${sqlStr(d.volume_unit ?? 'attempts')}, ${sqlInt(d.default_sets ?? 2)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 45)}, ${d.default_distance != null ? sqlStr(String(d.default_distance)) : 'NULL'}, ${sqlInt(d.est_seconds_per_set ?? 75)}, ${sqlInt(d.default_rpe_min ?? 3)}, ${sqlInt(d.default_rpe_max ?? 5)})`
}).join(',\n')
seedSql += `
...
seedSql += CARDS.map((card) => {
  ...
  return `  (${sqlStr(card.slug)}, ${sqlBool(r.can_be_daily)}, ${sqlInt(r.weekly_max_frequency ?? 5)}, ${sqlInt(r.minimum_hours_between_hard_exposures ?? 24)}, ${sqlBool(r.counts_as_high_intensity)}, ${sqlBool(r.counts_as_high_impact)}, ${sqlBool(r.counts_as_neural)}, ${sqlBool(r.counts_as_tissue_stress)}, ${sqlBool(r.counts_as_conditioning)})`
}).join(',\n')
seedSql += `
...
FROM (VALUES\n`
  seedSql += CARDS.map((card) => {
    const guidance = card.scaling?.[ck] ?? card.scaling?.adult_beginner ?? 'Scale one variable at a time: reduce speed first, then decision complexity, stance difficulty, or partner pressure.'
...
let hydrateSql = `-- Top 50 Coordination Under Duress Exercise Library — full Card v2 hydration.
-- IDEMPOTENT. Generated by scripts/generate-155-coordination.mjs (migrations 157/158)
...
UPDATE coaching.exercise SET
  ...
  default_rest_seconds = ${sqlInt(d.default_rest_seconds ?? 45)},
  est_seconds_per_set = ${sqlInt(d.est_seconds_per_set ?? 75)},
...
UPDATE coaching.exercise_dosage_profile d SET
  ...
  default_rpe_max = ${sqlInt(d.default_rpe_max ?? 5)},
...
const seedPath = path.join(__dirname, '../backend/migrations/157_coaching_coordination_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/158_coaching_coordination_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'coordination cards')
