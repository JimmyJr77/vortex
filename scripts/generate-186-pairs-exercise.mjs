/**
 * Generates:
 *   backend/migrations/188_coaching_pairs_exercise_infrastructure_and_seed.sql
 *   backend/migrations/189_coaching_pairs_exercise_cards.sql
 *
 * Prerequisite: node scripts/ingest-pairs-exercise-sources.mjs
 * Run: node scripts/generate-186-pairs-exercise.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/pairs-exercise-all-cards.json')
const MIG_DIR = path.join(__dirname, '../backend/migrations')

const data = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))
const RAW_CARDS = data.cards ?? []

function detectMergeSlugs(cards) {
  const files = fs.readdirSync(MIG_DIR)
    .filter((f) => f.endsWith('.sql') && !f.startsWith('188_') && !f.startsWith('189_'))
...
const MERGE_SLUGS = detectMergeSlugs(RAW_CARDS)
const CARDS = RAW_CARDS.map((c) => ({
  ...c,
  _action: MERGE_SLUGS.has(c.slug) ? 'merge' : 'insert',
}))
const INSERT_CARDS = CARDS.filter((c) => c._action === 'insert')
const MERGE_CARDS = CARDS.filter((c) => c._action === 'merge')
...
/** Equipment keys already seeded in prior migrations — skip re-insert. */
const EXISTING_EQUIPMENT = new Set([
  'none', 'bands', 'box', 'beam', 'bar', 'rings', 'kettlebell', 'dumbbell', 'sled',
  'medicine_ball', 'mat', 'cones', 'wall', 'parallettes', 'bench_or_box', 'pull_up_bar',
  'partner', 'reaction_ball', 'rope', 'tennis_ball', 'towel',
])

const NEW_EQUIPMENT = {}

const VALIDATION_EDU = {
  title: 'Pairs exercise library without group chaos or conditioning drift',
  short_summary:
    'Paired drills require exactly two people with defined roles, capped partner force, and quality gates — not random partner contests, fatigue games, or group drills mislabeled as pairs work.',
  what_it_is:
    'The Top 50 Pairs library spans partner med-ball throws, manual resistance, perturbation control, reactive agility mirrors, and cooperative locomotion across Output, Movement Intelligence, Resilience, Capacity, and Prepare & Access.',
  why_it_matters:
    'A live partner creates timing, resistance, and perception constraints solo drills cannot replicate. Poor role definition, surprise force, or extended sets after quality drops raise injury risk and train sloppy communication.',
  programming_guidance:
    'Define athlete and partner roles before the first rep. Dose both roles; switch before fatigue changes behavior. Keep Output and reactive work fresh with full rest. Stop when speed, spacing, catch quality, or communication breaks.',
  common_misuse:
    'Do not let partners add random force, chase competition over quality, or extend reps until breathing decides the outcome. Do not use pairs drills that require three or more people or hide conditioning as speed work.',
}
...
function sqlDistance(v) {
  return v == null ? 'NULL::integer' : String(Number(v))
}
...
function slotRows() {
  ...
  for (const card of CARDS) {
    ...
    if (!seen.has(key)) {
      const base = PHASE_ORDER_BASE[card.primaryPhaseKey] ?? 600
...
function defaultIntensityCeiling(card) {
  return card.phaseProfile?.intensity_ceiling ?? (card.primaryPhaseKey === 'resilience' ? 'moderate' : 'high')
}

function defaultDosageFallback(card) {
  const d = card.dosage ?? {}
  if (card.primaryPhaseKey === 'resilience') {
    return {
      sets: d.default_sets ?? 2,
      rest: d.default_rest_seconds ?? 45,
      est: d.est_seconds_per_set ?? 60,
      rpeMin: d.default_rpe_min ?? 4,
      rpeMax: d.default_rpe_max ?? 6,
    }
  }
  return {
    sets: d.default_sets ?? 3,
    rest: d.default_rest_seconds ?? 90,
    est: d.est_seconds_per_set ?? 40,
    rpeMin: d.default_rpe_min ?? 5,
    rpeMax: d.default_rpe_max ?? 7,
  }
}

function equipmentInserts() {
  const used = new Set()
  for (const card of CARDS) {
    for (const t of card.equipment ?? []) used.add(t.key)
  }
  const rows = []
  for (const key of used) {
    if (EXISTING_EQUIPMENT.has(key)) continue
    const meta = NEW_EQUIPMENT[key]
    if (!meta) throw new Error(`Unknown equipment key "${key}" — add to NEW_EQUIPMENT`)
    rows.push(`  ('${key}', '${meta[0].replace(/'/g, "''")}', ${meta[1]})`)
  }
  return rows
}
...
let seedSql = `-- Pairs exercise infrastructure, taxonomy, and ${INSERT_CARDS.length}-card seed.
-- IDEMPOTENT. Generated by scripts/generate-186-pairs-exercise.mjs (migrations 188/189)
-- ${CARDS.length} total cards (${INSERT_CARDS.length} insert, ${MERGE_CARDS.length} merge hydration).
-- Cluster: ${data.cluster?.focus ?? 'pairs'} (${data.cluster?.card_count ?? CARDS.length} cards).

`

const eqRows = equipmentInserts()
if (eqRows.length > 0) {
  seedSql += `INSERT INTO coaching.equipment (key, name, sort_order) VALUES\n${eqRows.join(',\n')}\nON CONFLICT (key) DO UPDATE SET name = EXCLUDED.name, sort_order = EXCLUDED.sort_order;\n\n`
}

const slots = slotRows()
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  const phaseSlots = slots.filter((s) => s.phase === phase)
  seedSql += `-- ${phase} pairs exercise slots\n`
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} new pairs exercises\n`
...
FROM (VALUES\n`
  ...
  seedSql += INSERT_CARDS.map((card) => {
    const d = card.dosage ?? {}
    const fb = defaultDosageFallback(card)
    const reps = d.default_reps ?? null
    const work = d.default_work_seconds ?? null
    return `  (${sqlStr(card.name)}, ${sqlStr(card.slug)}, ${sqlStr(card.description)}, 8, ${sqlInt(d.default_sets ?? fb.sets)}, ${sqlInt(reps)}, ${sqlInt(work)}, ${sqlInt(d.default_rest_seconds ?? fb.rest)}, ${sqlInt(d.est_seconds_per_set ?? fb.est)}, ${sqlStr(card.cardSummary)}, ${sqlStr(card.coachLanguage)}, ${sqlStr(card.athleteLanguage)}, ${sqlStr(card.family)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.subrole)}, ${sqlStr(card.slot)}, ${movementRequirementsJson(card).replace('::jsonb', '')}::jsonb, ${jsonb(seedExec(card)).replace('::jsonb', '')}::jsonb)`
  }).join(',\n')
...
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    ...
    return `  (${sqlStr(card.slug)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.slot)}, ${sqlStr(pp.role ?? 'primary')}, ${sqlInt(pp.fit_weight ?? 5)}, ${sqlBool(pp.freshness_required)}, ${sqlInt(pp.fatigue_sensitivity ?? 3)}, ${sqlInt(pp.fatigue_cost ?? 3)}, ${sqlInt(pp.technical_complexity ?? 2)}, ${sqlInt(pp.impact_level ?? 0)}, ${sqlStr(pp.intensity_ceiling ?? 'high')})`
  }).join(',\n')
  seedSql += `
...
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    const d = card.dosage ?? {}
    const fb = defaultDosageFallback(card)
    return `  (${sqlStr(card.slug)}, ${sqlStr(d.volume_unit ?? 'reps')}, ${sqlInt(d.default_sets ?? fb.sets)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? fb.rest)}, ${sqlDistance(d.default_distance)}, ${sqlInt(d.est_seconds_per_set ?? fb.est)}, ${sqlInt(d.default_rpe_min ?? fb.rpeMin)}, ${sqlInt(d.default_rpe_max ?? fb.rpeMax)})`
  }).join(',\n')
  seedSql += `
...
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    const s = card.safety ?? {}
    return `  (${sqlStr(card.slug)}, ${sqlInt(s.risk_level ?? 2)}, ${sqlInt(s.impact_level ?? 0)}, ${sqlBool(s.requires_spotting)}, ${sqlStr(s.requires_coach_supervision ?? 'recommended')}, ${sqlTextArray(s.readiness_checks)}, ${sqlTextArray(s.contraindications)}, ${sqlTextArray(s.common_substitutions)})`
  }).join(',\n')
  seedSql += `
...
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    ...
    return `  (${sqlStr(card.slug)}, ${sqlBool(r.can_be_daily)}, ${sqlInt(r.weekly_max_frequency ?? 3)}, ${sqlInt(r.minimum_hours_between_hard_exposures ?? 24)}, ${sqlBool(r.counts_as_high_intensity)}, ${sqlBool(r.counts_as_high_impact)}, ${sqlBool(r.counts_as_neural)}, ${sqlBool(r.counts_as_tissue_stress)}, ${sqlBool(r.counts_as_conditioning)})`
  }).join(',\n')
  seedSql += `
...
FROM (VALUES\n`
    seedSql += INSERT_CARDS.map((card) => {
      const guidance = card.scaling?.[ck] ?? card.scaling?.adult_beginner ?? 'Scale partner force, implement load, speed, distance, unpredictability, range, duration, or contact intensity one variable at a time. Regress by removing live partner input first.'
...
seedSql += `-- Pairs exercise cluster validation education
...
let hydrateSql = `-- Pairs exercise ${CARDS.length}-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-186-pairs-exercise.mjs (migrations 188/189)
-- ${INSERT_CARDS.length} insert + ${MERGE_CARDS.length} merge (${MERGE_CARDS.map((c) => c.slug).join(', ')})
...
for (const card of CARDS) {
  ...
  const pp = card.phaseProfile ?? {}
  const dos = card.dosage ?? {}
  const saf = card.safety ?? {}
  const reg = card.regimen ?? {}
  const fb = defaultDosageFallback(card)
  const sessionVolMin = dos.session_volume_min != null ? sqlStr(String(dos.session_volume_min)) : 'NULL'
  const sessionVolMax = dos.session_volume_max != null ? sqlStr(String(dos.session_volume_max)) : 'NULL'
  const defaultDistance = dos.default_distance != null ? sqlDistance(dos.default_distance) : 'NULL::integer'

  hydrateSql += `-- ${card.name} (${card.slug}) [${card._action}]
UPDATE coaching.exercise SET
  ...
  default_sets = ${sqlInt(dos.default_sets ?? fb.sets)},
  default_reps = ${sqlInt(dos.default_reps)},
  default_work_seconds = ${sqlInt(dos.default_work_seconds)},
  default_rest_seconds = ${sqlInt(dos.default_rest_seconds ?? fb.rest)},
  est_seconds_per_set = ${sqlInt(dos.est_seconds_per_set ?? fb.est)},
...
SELECT e.id, sp.id, ${sqlInt(pp.fit_weight ?? 5)}, ${sqlStr(pp.role ?? 'primary')}, ${sqlStr(card.slot)}, pos.order_index,
  ${sqlBool(pp.freshness_required)}, ${sqlInt(pp.fatigue_sensitivity ?? 3)}, ${sqlInt(pp.fatigue_cost ?? 3)},
  ${sqlInt(pp.technical_complexity ?? 2)}, ${sqlInt(pp.impact_level ?? 0)}, ${sqlStr(defaultIntensityCeiling(card))}
...
UPDATE coaching.exercise_dosage_profile d SET
  volume_unit = ${sqlStr(dos.volume_unit ?? 'reps')},
  default_sets = ${sqlInt(dos.default_sets ?? fb.sets)},
  default_reps = ${sqlInt(dos.default_reps)},
  ...
  default_rest_seconds = ${sqlInt(dos.default_rest_seconds ?? fb.rest)},
  est_seconds_per_set = ${sqlInt(dos.est_seconds_per_set ?? fb.est)},
  default_rpe_min = ${sqlInt(dos.default_rpe_min ?? fb.rpeMin)},
  default_rpe_max = ${sqlInt(dos.default_rpe_max ?? fb.rpeMax)},
...
SELECT e.id, 'Default', TRUE, ${sqlStr(dos.volume_unit ?? 'reps')}, ${sqlInt(dos.default_sets ?? fb.sets)}, ${sqlInt(dos.default_reps)},
  ${sqlInt(dos.default_work_seconds)}, ${sqlInt(dos.default_rest_seconds ?? fb.rest)}, ${defaultDistance},
  ${sqlInt(dos.est_seconds_per_set ?? fb.est)}, ${sqlInt(dos.default_rpe_min ?? fb.rpeMin)}, ${sqlInt(dos.default_rpe_max ?? fb.rpeMax)}
FROM coaching.exercise e
WHERE e.slug = ${sqlStr(card.slug)}
...
UPDATE coaching.exercise_regimen_rule r SET
  ...
  weekly_max_frequency = ${sqlInt(reg.weekly_max_frequency ?? 3)},
  minimum_hours_between_hard_exposures = ${sqlInt(reg.minimum_hours_between_hard_exposures ?? 24)},
...
SELECT e.id,
  ${sqlBool(reg.can_be_daily ?? false)},
  ${sqlInt(reg.weekly_max_frequency ?? 3)},
  ${sqlInt(reg.minimum_hours_between_hard_exposures ?? 24)},
  ${sqlBool(reg.counts_as_high_intensity ?? false)},
  ${sqlBool(reg.counts_as_high_impact ?? false)},
  ${sqlBool(reg.counts_as_neural ?? false)},
  ${sqlBool(reg.counts_as_tissue_stress ?? true)},
  ${sqlBool(reg.counts_as_conditioning ?? false)}
FROM coaching.exercise e
WHERE e.slug = ${sqlStr(card.slug)}
...
UPDATE coaching.exercise_safety_profile s SET
  risk_level = ${sqlInt(saf.risk_level ?? 2)},
  impact_level = ${sqlInt(saf.impact_level ?? 0)},
  requires_spotting = ${sqlBool(saf.requires_spotting)},
  requires_coach_supervision = ${sqlStr(saf.requires_coach_supervision ?? 'recommended')},
...
SELECT e.id, ${sqlInt(saf.risk_level ?? 2)}, ${sqlInt(saf.impact_level ?? 0)}, ${sqlBool(saf.requires_spotting ?? false)},
  ${sqlStr(saf.requires_coach_supervision ?? 'recommended')},
  ${sqlTextArray(saf.readiness_checks ?? [])},
  ${sqlTextArray(saf.contraindications ?? [])},
  ${sqlTextArray(saf.common_substitutions ?? [])}
FROM coaching.exercise e
WHERE e.slug = ${sqlStr(card.slug)}
...
FROM (VALUES\n`
  hydrateSql += CARDS.map((card) => {
    const guidance = card.scaling?.[ck] ?? card.scaling?.adult_beginner ?? 'Scale by leverage, band tension, range, tempo, and rest before adding complexity.'
    return `  (${sqlStr(card.slug)}, ${sqlStr(guidance)}, ${sqlStr(card.genderSpecificNotes ?? '')})`
  }).join(',\n')
  hydrateSql += `
) AS v(slug, guidance, gender_notes)
...
const seedPath = path.join(__dirname, '../backend/migrations/188_coaching_pairs_exercise_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/189_coaching_pairs_exercise_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'pairs exercise cards (', INSERT_CARDS.length, 'insert,', MERGE_CARDS.length, 'merge)')
