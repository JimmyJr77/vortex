/**
 * Generates backend/migrations/123_coaching_capacity_hinge_cards.sql
 * Run: node scripts/generate-123-capacity-hinge-cards.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import { CAPACITY_HINGE_CARDS, CAPACITY_HINGE_SLUGS } from './data/capacity-hinge-cards-11-18.mjs'
...
const VALIDATION_EDU = {
  title: 'Hinge / Posterior-Chain Capacity without stealing Output quality',
  short_summary:
...
let sql = `-- Capacity phase Hinge / Posterior-Chain Strength cards 11–18 (rich card v2 content).
...
for (const card of CAPACITY_HINGE_CARDS) {
  const pairing = {
...
sql += `-- Refresh weighted taxonomy tags for hinge / posterior-chain Capacity cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${CAPACITY_HINGE_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += CAPACITY_HINGE_CARDS.map((c) => {
    const guidance = c.scaling[ck] ?? c.scaling.adult_beginner ?? 'Conservative hinge dose — prioritize mechanics and full rest.'
...
const outPath = path.join(__dirname, '../backend/migrations/123_coaching_capacity_hinge_cards.sql')
fs.writeFileSync(outPath, sql)
console.log('Wrote', outPath, '—', CAPACITY_HINGE_CARDS.length, 'hinge / posterior-chain Capacity cards')
