/**
 * Generates:
 *   backend/migrations/175_coaching_sandbag_nontraditional_strength_infrastructure_and_seed.sql
 *   backend/migrations/176_coaching_sandbag_nontraditional_strength_cards.sql
 *
 * Prerequisite: node scripts/ingest-sandbag-nontraditional-strength-sources.mjs
 * Run: node scripts/generate-175-sandbag-nontraditional-strength.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/sandbag-nontraditional-strength-all-cards.json')
const data = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))
const CARDS = data.cards ?? []
const INSERT_CARDS = CARDS.filter((c) => c._action !== 'merge')
const SLUGS = CARDS.map((c) => c.slug)

const PHASE_ORDER_BASE = { capacity: 760 }
...
const VALIDATION_EDU = {
  title: 'Sandbag and non-traditional strength without conditioning drift or wrong phase intent',
  short_summary:
...
function slotRows() {
  ...
  for (const card of CARDS) {
    ...
    if (!seen.has(key)) {
      const base = PHASE_ORDER_BASE[card.primaryPhaseKey] ?? 550
...
// ── 175: infrastructure + seed ─────────────────────────────────────────────

let seedSql = `-- Sandbag & non-traditional strength infrastructure, taxonomy, and ${INSERT_CARDS.length}-card seed.
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} sandbag & non-traditional strength slots\n`
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} sandbag & non-traditional strength exercises\n`
...
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    const s = card.safety ?? {}
    return `  (${sqlStr(card.slug)}, ${sqlInt(s.risk_level ?? 2)}, ${sqlInt(s.impact_level ?? 0)}, ${sqlBool(s.requires_spotting)}, ${sqlStr(s.requires_coach_supervision ?? 'optional')}, ${sqlTextArray(s.readiness_checks)}, ${sqlTextArray(s.contraindications)}, ${sqlTextArray(s.common_substitutions)})`
  }).join(',\n')
  seedSql += `
...
FROM (VALUES\n`
    seedSql += INSERT_CARDS.map((card) => {
      const guidance = card.scaling?.[ck] ?? card.scaling?.adult_beginner ?? 'Scale by load, range, tempo, and rest before adding complexity.'
...
seedSql += `-- Sandbag & non-traditional strength cluster validation education
...
let hydrateSql = `-- Sandbag & non-traditional strength ${CARDS.length}-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-175-sandbag-nontraditional-strength.mjs (migrations 175/176)
...
const seedPath = path.join(__dirname, '../backend/migrations/175_coaching_sandbag_nontraditional_strength_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/176_coaching_sandbag_nontraditional_strength_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'sandbag & non-traditional strength cards')
