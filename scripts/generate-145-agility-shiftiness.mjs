/**
 * Generates:
 *   backend/migrations/145_coaching_agility_shiftiness_infrastructure_and_seed.sql
 *   backend/migrations/146_coaching_agility_shiftiness_cards.sql
 *
 * Run: node scripts/generate-145-agility-shiftiness.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/agility-shiftiness-all-cards.json')
const data = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))
const CARDS = data.batches.flatMap((b) => b.cards)
...
const VALIDATION_EDU = {
  title: 'Agility / Shiftiness without pre-fatigue or wrong phase intent',
  short_summary:
...
// ── 145: infrastructure + seed ─────────────────────────────────────────────

let seedSql = `-- Agility / Shiftiness phase infrastructure, taxonomy support, and 50-card seed.
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} agility / shiftiness slots\n`
...
seedSql += `-- Insert 50 agility / shiftiness exercises
...
// skeleton tags from first tenet/method per card
...
FROM (VALUES\n`
  seedSql += CARDS.map((card) => {
    const guidance = card.scaling?.[ck] ?? card.scaling?.adult_beginner ?? 'Scale by speed, amplitude, and plant quality before adding complexity.'
...
let hydrateSql = `-- Agility / Shiftiness 50-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-145-agility-shiftiness.mjs
...
const seedPath = path.join(__dirname, '../backend/migrations/145_coaching_agility_shiftiness_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/146_coaching_agility_shiftiness_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'agility / shiftiness cards')
