/**
 * Generates:
 *   backend/migrations/198_coaching_landmine_exercise_library_infrastructure_and_seed.sql
 *   backend/migrations/199_coaching_landmine_exercise_library_cards.sql
 *
 * Source: scripts/data/landmine-exercise-cards-all-50.json
 * Run: node scripts/generate-184-landmine-exercise-library.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/landmine-exercise-cards-all-50.json')
const MIG_DIR = path.join(__dirname, '../backend/migrations')

const raw = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))

const SUBROLE_MAP = {
  upper_body_pull_strength: 'pull_hang_grip_strength',
...
const VALIDATION_EDU = {
  title: 'Landmine exercise library without conditioning drift or wrong phase intent',
  short_summary:
  ...
  what_it_is: lib.purpose ?? 'Coachable landmine exercise library covering one-handed, two-handed, ball-grip, and handle-grip variations.',
  why_it_matters:
  ...
  programming_guidance: lib.programmingRule
    ?? 'Output variations require freshness, low volume, and full recovery; Capacity uses progressive load; Resilience uses bracing and control without fatigue chasing.',
  common_misuse:
...
function normalizeMovementRequirements(card) {
  ...
  const grip = req.gripProfile ?? card.gripProfile
  return {
    ...
    postural_shape: req.posturalShape ?? null,
    breathing_demand: req.breathingDemand ?? null,
    balance_demand: req.balanceDemand ?? null,
    coordination_demand: req.coordinationDemand ?? null,
    impact_level: req.impactLevel ?? pp.impactLevel ?? safety.impactLevel ?? 0,
    grip_profile: grip ?? null,
  }
}
...
function pairingLogic(card) {
  ...
  return {
    pairs_well_before: p.pairsWellBefore ?? p.pairs_well_before ?? [],
    pairs_well_after: p.pairsWellAfter ?? p.pairs_well_after ?? [],
    good_for_sessions: p.goodForSessions ?? p.good_for_sessions ?? [],
    avoid_before: p.avoidBefore ?? p.avoid_before ?? [],
    avoid_after: p.avoidAfter ?? p.avoid_after ?? [],
    do_not_use_when: p.doNotUseWhen ?? p.do_not_use_when ?? [],
  }
}
...
function scalingGuidance(card, cohortKey) {
  const camel = SCALING_CAMEL[cohortKey]
  return card.scaling?.[camel]
    ?? card.scaling?.adultBeginner
    ?? card.scalingGuidance
    ?? 'Scale by load, range, stance, tempo, attachment choice, and rest before adding complexity.'
}
...
// ── 184: infrastructure + seed ─────────────────────────────────────────────

let seedSql = `-- Landmine exercise library infrastructure, taxonomy, and ${INSERT_CARDS.length}-card seed.
...
-- Library: ${lib.title ?? 'Top 50 Landmine Exercise Library'}
...
${METHODOLOGY_SEED.map(([k, n, d, o]) => `  (${sqlStr(k)}, ${sqlStr(n)}, ${sqlStr(d)}, ${o})`).join(',\n')}
...
${BODY_REGION_SEED.map(([k, n, o]) => `  (${sqlStr(k)}, ${sqlStr(n)}, ${o})`).join(',\n')}
ON CONFLICT (key) DO UPDATE SET name = EXCLUDED.name, sort_order = EXCLUDED.sort_order;
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} landmine exercises\n`
...
FROM (VALUES\n`
  ...
  seedSql += INSERT_CARDS.map((card) => {
    ...
    return `  (${sqlStr(card.name)}, ${sqlStr(card.slug)}, ${sqlStr(card.description)}, 8, ${sqlInt(d.default_sets)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds)}, ${sqlInt(d.est_seconds_per_set)}, ${sqlStr(card.cardSummary)}, ${sqlStr(card.coachLanguage)}, ${sqlStr(card.athleteLanguage)}, ${sqlStr(card.family)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.subrole)}, ${sqlStr(card.slot)}, ${jsonb(normalizeMovementRequirements(card)).replace('::jsonb', '')}::jsonb, ${jsonb(normalizeCoachingExecution(card)).replace('::jsonb', '')}::jsonb)`
  }).join(',\n')
...
seedSql += `-- Landmine library validation education
...
let hydrateSql = `-- Landmine exercise library ${CARDS.length}-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-184-landmine-exercise-library.mjs (migrations 198/199)
...
const seedPath = path.join(MIG_DIR, '198_coaching_landmine_exercise_library_infrastructure_and_seed.sql')
const hydratePath = path.join(MIG_DIR, '199_coaching_landmine_exercise_library_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'landmine cards (', INSERT_CARDS.length, 'insert,', MERGE_SLUGS.size, 'merge)')
