/**
 * Generates:
 *   backend/migrations/208_coaching_box_jump_infrastructure_and_seed.sql
 *   backend/migrations/209_coaching_box_jump_cards.sql
 *
 * Source: scripts/data/box-jump-exercise-cards-all-50.json
 * Run: node scripts/generate-208-box-jump.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/box-jump-exercise-cards-all-50.json')
...
const PHASE_ORDER_BASE = {
  ...
  movement_intelligence: 278,
  resilience: 535,
  output: 781,
  capacity: 415,
  sustained_capacity: 614,
}
...
const VALIDATION_EDU = {
  title: 'Box jump programming without ego height or conditioning drift',
  short_summary:
...
function inferVolumeUnit(card, d) {
  ...
  if (card.primaryPhaseKey === 'sustained_capacity') return 'rounds'
...
function parseDosage(card) {
  ...
  return {
    ...
    default_rpe_min: d.default_rpe_min ?? 5,
    default_rpe_max: d.default_rpe_max ?? 7,
...
function difficultyFromCard(card) {
  ...
  return {
    technical: d.technical ?? 5,
    load: d.load ?? 5,
    complexity: d.complexity ?? 5,
    overall: d.overall ?? 5,
    recommended_age_min: d.recommended_age_min ?? 8,
    recommended_age_max: d.recommended_age_max ?? null,
    attention_demand: d.attention_demand ?? 'moderate',
    notes: d.notes ?? null,
  }
}
...
function pairingLogic(card) {
  return {
    ...
    good_for_sessions: card.goodForSessions ?? ['box jump', 'plyometrics', 'landing prep'],
    avoid_before: [],
    avoid_after: [],
    do_not_use_when: card.doNotUseWhen ?? [],
    notes: card.pairing?.notes ?? null,
  }
}

function mediaLibrary(card) {
  const refs = card.mediaReferences ?? []
  return {
    demo_video_sources: [],
    coaching_articles: [],
...
function scalingGuidance(card, cohortKey) {
  ...
  return s[cohortKey]
    ...
    ?? 'Scale by box height, approach complexity, landing demand, external load, and total contacts. Progress only when the athlete keeps quiet landings and full reset quality.'
}
...
function slotRows() {
  ...
  for (const card of CARDS) {
    ...
    if (!seen.has(key)) {
      const base = PHASE_ORDER_BASE[card.primaryPhaseKey] ?? 781
...
SELECT e.id, ${sqlInt(diff.technical)}, ${sqlInt(diff.load)}, ${sqlInt(diff.complexity)}, ${sqlInt(diff.overall)},
  ${sqlInt(diff.recommended_age_min)}, NULL::integer, ${sqlStr(diff.attention_demand)}, ${sqlStr(diff.notes)}, 'derived'
...
// ── 208: infrastructure + seed ─────────────────────────────────────────────

let seedSql = `-- Box jump focused exercise library: infrastructure and ${INSERT_CARDS.length}-card seed.
...
-- Cluster: ${sqlStr(data.cluster?.topic ?? 'box_jump_focused_exercises')}
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} new box-jump exercises\n`
...
FROM (VALUES\n`
  ...
  seedSql += INSERT_CARDS.map((card) => {
    ...
    const diff = difficultyFromCard(card)
    return `  (${sqlStr(card.name)}, ${sqlStr(card.slug)}, ${sqlStr(card.description)}, ${sqlInt(diff.recommended_age_min)}, ${sqlInt(d.default_sets)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds)}, ${sqlInt(d.est_seconds_per_set)}, ${sqlStr(participantStructure(card))}, ${sqlStr(card.cardSummary)}, ${sqlStr(card.coachLanguage)}, ${sqlStr(card.athleteLanguage)}, ${sqlStr(card.family)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.subrole)}, ${sqlStr(card.slot)}, ${movementRequirementsJson(card).replace('::jsonb', '')}::jsonb, ${jsonb(seedExec(card)).replace('::jsonb', '')}::jsonb)`
  }).join(',\n')
...
SELECT e.id, '${ck}', '${ck.replace(/_/g, ' ')}', 'baseline', v.guidance, ${sqlStr(null)}
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) =>
...
let hydrateSql = `-- Box jump focused 50-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-208-box-jump.mjs (migrations 208/209)
...
for (const ck of COHORT_KEYS) {
  ...
  gender_specific_notes = ${sqlStr(null)}
FROM (VALUES\n`
...
SELECT e.id, '${ck}', '${ck.replace(/_/g, ' ')}', 'baseline', v.guidance, v.gender_notes
FROM (VALUES\n`
  hydrateSql += CARDS.map((card) =>
    `  (${sqlStr(card.slug)}, ${sqlStr(scalingGuidance(card, ck))}, ${sqlStr(card.genderSpecificNotes ?? null)})`,
  ).join(',\n')
  hydrateSql += `
...
const seedPath = path.join(__dirname, '../backend/migrations/208_coaching_box_jump_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/209_coaching_box_jump_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'box-jump cards (', INSERT_CARDS.length, 'insert,', MERGE_COUNT, 'merge-only)')
