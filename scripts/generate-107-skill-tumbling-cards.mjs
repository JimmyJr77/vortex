/**
 * Generates backend/migrations/107_coaching_skill_tumbling_cards.sql
 * Run: node scripts/generate-107-skill-tumbling-cards.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import { SKILL_TUMBLING_CARDS, SKILL_TUMBLING_SLUGS } from './data/skill-tumbling-cards-11-24.mjs'
...
const VALIDATION_EDU = {
  title: 'Rotation / Tumbling Foundations without pre-fatigue',
  short_summary:
...
let sql = `-- Skill / Movement Intelligence tumbling cards 11–24 (rich card v2 content).
...
for (const card of SKILL_TUMBLING_CARDS) {
  const pairing = {
...
sql += `-- Refresh weighted taxonomy tags for tumbling cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${SKILL_TUMBLING_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += SKILL_TUMBLING_CARDS.map((c) => {
...
const outPath = path.join(__dirname, '../backend/migrations/107_coaching_skill_tumbling_cards.sql')
fs.writeFileSync(outPath, sql)
console.log('Wrote', outPath, '—', SKILL_TUMBLING_CARDS.length, 'skill tumbling cards')
