/**
 * Generates backend/migrations/119_coaching_output_reactive_tumbling_cards.sql
 * Run: node scripts/generate-119-output-reactive-tumbling-cards.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import {
  OUTPUT_REACTIVE_TUMBLING_CARDS,
  OUTPUT_REACTIVE_TUMBLING_SLUGS,
} from './data/output-reactive-tumbling-cards-46-50.mjs'
...
function mapVolumeUnit(unit) {
  if (unit === 'reactive_sprints' || unit === 'drops' || unit === 'rounds') return 'attempts'
  return unit ?? 'attempts'
}

function mapIntensityCeiling(ceiling) {
  if (ceiling === 'high_to_max') return 'max'
  return ceiling ?? 'high'
}

function movementRequirementsJson(card) {
  const base = { ...(card.movementRequirements ?? {}) }
  if (card.setupMetadata) base.setup_metadata = card.setupMetadata
  if (card.setupRequirements) base.setup_requirements = card.setupRequirements
  if (card.reactiveOutputProfile) base.reactive_output_profile = card.reactiveOutputProfile
  return jsonb(base)
}
...
const VALIDATION_EDU = {
  title: 'Reactive Agility & Tumbling Output without chaos',
  short_summary:
...
let sql = `-- Output phase Reactive Agility & Tumbling Output cards 46–50 (rich card v2 content).
...
for (const card of OUTPUT_REACTIVE_TUMBLING_CARDS) {
  const pairing = {
...
UPDATE coaching.exercise_dosage_profile d SET
  volume_unit = ${sqlStr(mapVolumeUnit(d.volume_unit))},
  default_sets = ${d.default_sets},
  default_reps = ${d.default_reps ?? 'NULL'},
...
sql += `-- Refresh weighted taxonomy tags for reactive / tumbling Output cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${OUTPUT_REACTIVE_TUMBLING_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += OUTPUT_REACTIVE_TUMBLING_CARDS.map((c) => {
    const guidance = c.scaling[ck] ?? c.scaling.adult_beginner ?? 'Conservative reactive/tumbling dose — prioritize cue quality, spacing, and full rest.'
...
const outPath = path.join(__dirname, '../backend/migrations/119_coaching_output_reactive_tumbling_cards.sql')
fs.writeFileSync(outPath, sql)
console.log('Wrote', outPath, '—', OUTPUT_REACTIVE_TUMBLING_CARDS.length, 'reactive / tumbling Output cards')
