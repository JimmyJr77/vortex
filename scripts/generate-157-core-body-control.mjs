/**
 * Generates:
 *   backend/migrations/159_coaching_core_body_control_infrastructure_and_seed.sql
 *   backend/migrations/160_coaching_core_body_control_cards.sql
 *
 * Run: node scripts/generate-157-core-body-control.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/core-body-control-all-cards.json')
const data = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))
const CARDS = data.cards ?? []
const SLUGS = CARDS.map((c) => c.slug)

const PHASE_ORDER_BASE = {
  prepare_and_access: 141,
  movement_intelligence: 272,
  resilience: 561,
}
...
const EQUIPMENT_SEED = [
  ['bands_optional', 'Bands (Optional)', 100],
  ['cable_machine_optional', 'Cable Machine (Optional)', 101],
  ['dumbbell_optional', 'Dumbbell (Optional)', 102],
  ['kettlebell_optional', 'Kettlebell (Optional)', 103],
  ['rings_optional', 'Rings (Optional)', 104],
  ['slider_optional', 'Slider (Optional)', 105],
  ['sliders_optional', 'Sliders (Optional)', 106],
  ['stability_ball', 'Stability Ball', 107],
  ['wall_optional', 'Wall (Optional)', 108],
]

const VALIDATION_EDU = {
  title: 'Core & Body Control without conditioning drift or wrong phase intent',
  short_summary:
...
function defaultScaling(card, ck) {
  return card.scaling?.[ck] ?? card.scaling?.adult_beginner
    ?? 'Scale by lever length, support, range of motion, tempo, load, breathing demand, and total volume.'
}

// ── 159: infrastructure + seed ─────────────────────────────────────────────

let seedSql = `-- Core & Body Control — phase infrastructure, taxonomy support, and ${CARDS.length}-card seed.
-- IDEMPOTENT. Generated by scripts/generate-157-core-body-control.mjs (migrations 159/160)
-- Cluster: ${data.cluster?.focus ?? 'core_body_control_work'} (${data.cluster?.card_count ?? CARDS.length} cards).
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} core & body control slots\n`
...
seedSql += `-- Insert ${CARDS.length} core & body control exercises (existing slugs skipped)
...
seedSql += CARDS.map((card) => {
  const d = card.dosage ?? {}
  return `  (${sqlStr(card.name)}, ${sqlStr(card.slug)}, ${sqlStr(card.description)}, 8, ${sqlInt(d.default_sets ?? 2)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 30)}, ${sqlInt(d.est_seconds_per_set ?? 40)}, ${sqlStr(card.cardSummary)}, ${sqlStr(card.coachLanguage)}, ${sqlStr(card.athleteLanguage)}, ${sqlStr(card.family)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.subrole)}, ${sqlStr(card.slot)}, ${jsonb(seedReq(card)).replace('::jsonb', '')}::jsonb, ${jsonb(seedExec(card)).replace('::jsonb', '')}::jsonb)`
}).join(',\n')
...
let hydrateSql = `-- Core & Body Control ${CARDS.length}-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-157-core-body-control.mjs (migrations 159/160)
...
const seedPath = path.join(__dirname, '../backend/migrations/159_coaching_core_body_control_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/160_coaching_core_body_control_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'core & body control cards')
