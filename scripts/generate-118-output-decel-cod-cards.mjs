/**
 * Generates backend/migrations/118_coaching_output_decel_cod_cards.sql
 * Run: node scripts/generate-118-output-decel-cod-cards.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import { OUTPUT_DECEL_COD_CARDS, OUTPUT_DECEL_COD_SLUGS } from './data/output-decel-cod-cards-39-45.mjs'
...
function movementRequirementsJson(card) {
  ...
  if (card.codStressProfile) base.cod_stress_profile = card.codStressProfile
  return jsonb(base)
}
...
const VALIDATION_EDU = {
  title: 'Deceleration / COD Power without pre-fatigue',
  short_summary:
...
let sql = `-- Output phase Deceleration / COD Power cards 39–45 (rich card v2 content).
...
for (const card of OUTPUT_DECEL_COD_CARDS) {
  const pairing = {
...
sql += `-- Refresh weighted taxonomy tags for deceleration / COD cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${OUTPUT_DECEL_COD_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += OUTPUT_DECEL_COD_CARDS.map((c) => {
    const guidance = c.scaling[ck] ?? c.scaling.adult_beginner ?? 'Conservative COD dose — prioritize plant quality and full rest.'
...
const outPath = path.join(__dirname, '../backend/migrations/118_coaching_output_decel_cod_cards.sql')
fs.writeFileSync(outPath, sql)
console.log('Wrote', outPath, '—', OUTPUT_DECEL_COD_CARDS.length, 'deceleration / COD cards')
