/**
 * Generates backend/migrations/114_coaching_output_max_velocity_cards.sql
 * Run: node scripts/generate-114-output-max-velocity-cards.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import { OUTPUT_MAX_VELOCITY_CARDS, OUTPUT_MAX_VELOCITY_SLUGS } from './data/output-max-velocity-cards-11-18.mjs'
...
const VALIDATION_EDU = {
  title: 'Max-Velocity Exposure without pre-fatigue',
  short_summary:
...
let sql = `-- Output phase Max-Velocity Exposure cards 11–18 (rich card v2 content).
...
for (const card of OUTPUT_MAX_VELOCITY_CARDS) {
  const pairing = {
...
sql += `-- Refresh weighted taxonomy tags for max-velocity cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${OUTPUT_MAX_VELOCITY_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += OUTPUT_MAX_VELOCITY_CARDS.map((c) => {
    const guidance = c.scaling[ck] ?? c.scaling.adult_beginner ?? 'Conservative max-velocity dose — prioritize quality and full rest.'
...
const outPath = path.join(__dirname, '../backend/migrations/114_coaching_output_max_velocity_cards.sql')
fs.writeFileSync(outPath, sql)
console.log('Wrote', outPath, '—', OUTPUT_MAX_VELOCITY_CARDS.length, 'max-velocity cards')
