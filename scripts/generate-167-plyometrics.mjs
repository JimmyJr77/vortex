/**
 * Generates:
 *   backend/migrations/173_coaching_plyometrics_infrastructure_and_seed.sql
 *   backend/migrations/174_coaching_plyometrics_cards.sql
 *
 * Run: node scripts/generate-167-plyometrics.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/plyometrics-all-cards.json')
...
const VALIDATION_EDU = {
  title: 'Plyometrics without conditioning drift or pre-fatigue',
  short_summary:
...
let seedSql = `-- Plyometrics (Output) phase infrastructure and ${INSERT_CARDS.length}-card seed.
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} plyometrics slots\n`
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} new plyometrics exercises\n`
...
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    const guidance = card.scaling?.[ck] ?? card.scaling?.adult_beginner ?? 'Scale by reducing amplitude, contacts, approach speed, direction change, surface hardness, external load, or reactive complexity.'
...
let hydrateSql = `-- Plyometrics 50-card full hydration (Output, card v2).
-- IDEMPOTENT. Generated by scripts/generate-167-plyometrics.mjs (migrations 173/174)
...
const seedPath = path.join(__dirname, '../backend/migrations/173_coaching_plyometrics_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/174_coaching_plyometrics_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'plyometrics cards (', INSERT_CARDS.length, 'insert,', MERGE_COUNT, 'merge-only)')
