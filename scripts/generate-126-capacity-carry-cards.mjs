/**
 * Generates backend/migrations/126_coaching_capacity_carry_cards.sql
 * Run: node scripts/generate-126-capacity-carry-cards.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import { CAPACITY_CARRY_CARDS, CAPACITY_CARRY_SLUGS } from './data/capacity-carry-cards-37-44.mjs'
...
const VALIDATION_EDU = {
  title: 'Carry / Trunk / Loaded-Bracing Capacity without stealing skill quality',
  short_summary:
...
let sql = `-- Capacity phase Carry / Trunk / Loaded-Bracing Strength cards 37–44 (rich card v2 content).
...
for (const card of CAPACITY_CARRY_CARDS) {
  const pairing = {
...
sql += `-- Refresh weighted taxonomy tags for carry / loaded-bracing Capacity cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${CAPACITY_CARRY_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += CAPACITY_CARRY_CARDS.map((c) => {
    const guidance = c.scaling[ck] ?? c.scaling.adult_beginner ?? 'Conservative carry dose — prioritize posture and full rest.'
...
const outPath = path.join(__dirname, '../backend/migrations/126_coaching_capacity_carry_cards.sql')
fs.writeFileSync(outPath, sql)
console.log('Wrote', outPath, '—', CAPACITY_CARRY_CARDS.length, 'carry / loaded-bracing Capacity cards')
