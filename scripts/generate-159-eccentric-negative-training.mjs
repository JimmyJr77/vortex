/**
 * Generates:
 *   backend/migrations/163_coaching_eccentric_negative_training_infrastructure_and_seed.sql
 *   backend/migrations/164_coaching_eccentric_negative_training_cards.sql
 *
 * Run: node scripts/generate-159-eccentric-negative-training.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/eccentric-negative-training-all-cards.json')
...
const EQUIPMENT_SEED = [
  ['ab_wheel', 'Ab Wheel', 109],
  ['machine', 'Selectorized Machine', 110],
  ['step', 'Step / Plyo Box', 111],
  ['barbell_optional', 'Barbell (Optional)', 112],
  ['cable_optional', 'Cable (Optional)', 113],
  ['dumbbells_optional', 'Dumbbells (Optional)', 114],
  ['plates_optional', 'Weight Plates (Optional)', 115],
]

const VALIDATION_EDU = {
  title: 'Eccentric / negative training without wrong phase intent or pre-fatigue',
  short_summary:
...
function defaultScaling(card, ck) {
  return card.scaling?.[ck] ?? card.scaling?.adult_beginner
    ?? 'Scale by reducing range, tempo, load, lever length, instability, impact, or weekly frequency before accepting pain, shaking, breath holding, or position loss.'
}

// ── 163: infrastructure + seed ─────────────────────────────────────────────

let seedSql = `-- Eccentric / Negative Training — Resilience phase infrastructure, taxonomy support, and 50-card seed.
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} eccentric negative training slots\n`
...
seedSql += `-- Insert 50 eccentric negative training exercises
...
seedSql += CARDS.map((card) => {
  ...
  return `  (${sqlStr(card.slug)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.slot)}, ${sqlInt(pp.fit_weight ?? 5)}, ${sqlBool(pp.freshness_required ?? false)}, ${sqlInt(pp.fatigue_sensitivity ?? 3)}, ${sqlInt(pp.fatigue_cost ?? 3)}, ${sqlInt(pp.technical_complexity ?? 2)}, ${sqlInt(pp.impact_level ?? 0)}, ${sqlStr(pp.intensity_ceiling ?? 'moderate')})`
}).join(',\n')
seedSql += `
...
seedSql += CARDS.map((card) => {
  const d = card.dosage ?? {}
  return `  (${sqlStr(card.slug)}, ${sqlStr(d.volume_unit ?? 'reps')}, ${sqlInt(d.default_sets ?? 3)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 75)}, ${d.default_distance != null ? sqlStr(String(d.default_distance)) : 'NULL'}, ${sqlInt(d.est_seconds_per_set ?? 105)}, ${sqlInt(d.default_rpe_min ?? 5)}, ${sqlInt(d.default_rpe_max ?? 7)})`
}).join(',\n')
seedSql += `
...
seedSql += CARDS.map((card) => {
  ...
  return `  (${sqlStr(card.slug)}, ${sqlBool(r.can_be_daily)}, ${sqlInt(r.weekly_max_frequency ?? 3)}, ${sqlInt(r.minimum_hours_between_hard_exposures ?? 24)}, ${sqlBool(r.counts_as_high_intensity)}, ${sqlBool(r.counts_as_high_impact)}, ${sqlBool(r.counts_as_neural)}, ${sqlBool(r.counts_as_tissue_stress ?? true)}, ${sqlBool(r.counts_as_conditioning)})`
}).join(',\n')
seedSql += `
...
let hydrateSql = `-- Eccentric / Negative Training — 50-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-159-eccentric-negative-training.mjs
...
for (const card of CARDS) {
  ...
  const media = {
    ...
    internal_notes: [
      ...
      'Eccentric negative training cluster card; source: eccentric_negative_training_cards_all_50.',
    ],
  }
  const pp = card.phaseProfile ?? {}
  const d = card.dosage ?? {}
...
const seedPath = path.join(__dirname, '../backend/migrations/163_coaching_eccentric_negative_training_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/164_coaching_eccentric_negative_training_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'eccentric negative training cards')
