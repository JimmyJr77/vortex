/**
 * Generates backend/migrations/117_coaching_output_jump_power_cards.sql
 * Run: node scripts/generate-117-output-jump-power-cards.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import {
  OUTPUT_JUMP_POWER_CARDS,
  OUTPUT_JUMP_POWER_SLUGS,
} from './data/output-jump-power-cards-28-38.mjs'
...
const VALIDATION_EDU = {
  title: 'Jump, Throw & Explosive Power without pre-fatigue',
  short_summary:
...
let sql = `-- Output phase Jump, Throw & Explosive Power cards 28–38 (rich card v2 content).
...
for (const card of OUTPUT_JUMP_POWER_CARDS) {
  const pairing = {
...
sql += `-- Refresh weighted taxonomy tags for jump / throw power Output cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${OUTPUT_JUMP_POWER_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += OUTPUT_JUMP_POWER_CARDS.map((c) => {
    const guidance = c.scaling[ck] ?? c.scaling.adult_beginner ?? 'Low-volume explosive reps with full reset; stop when power quality drops.'
...
const outPath = path.join(__dirname, '../backend/migrations/117_coaching_output_jump_power_cards.sql')
fs.writeFileSync(outPath, sql)
console.log('Wrote', outPath, '—', OUTPUT_JUMP_POWER_CARDS.length, 'output jump/throw power cards')
