/**
 * Generates:
 *   backend/migrations/169_coaching_mobility_infrastructure_and_seed.sql
 *   backend/migrations/170_coaching_mobility_cards.sql
 *
 * Run: node scripts/generate-167-mobility.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import { MERGE_SLUGS, SLUG_RENAMES } from './data/mobility-merge-overrides.mjs'

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const SOURCE = path.join(__dirname, 'data/mobility-all-cards.json')
const MIG_DIR = path.join(__dirname, '../backend/migrations')

const data = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))
const RAW_CARDS = (data.cards ?? []).map((c) => {
  const renamed = SLUG_RENAMES[c.slug]
  return renamed ? { ...c, slug: renamed, _renamedFrom: c.slug } : c
})

const CARDS = RAW_CARDS.map((c) => ({
  ...c,
  _action: MERGE_SLUGS.has(c.slug) || MERGE_SLUGS.has(c._renamedFrom) ? 'merge' : 'insert',
}))
...
const VALIDATION_EDU = {
  title: 'Dynamic mobility without fatigue drift or wrong phase intent',
  short_summary:
...
function defaultScaling(card) {
  return card.scalingGuidance ?? 'Scale by range, support, tempo, and total volume before adding complexity.'
}

// ── 167: infrastructure + seed ─────────────────────────────────────────────

let seedSql = `-- Prepare & Access dynamic mobility infrastructure and ${INSERT_CARDS.length}-card seed.
...
CROSS JOIN (VALUES\n`
  seedSql += phaseSlots.map((s) =>
    `  (${sqlStr(s.slot)}, ${sqlStr(s.name)}, ${sqlStr(s.description)}, ${s.order_index}, 1, ${sqlStr(s.subrole)})`,
  ).join(',\n')
  seedSql += `
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} mobility exercises\n`
...
FROM (VALUES\n`
  ...
  seedSql += INSERT_CARDS.map((card) => {
    ...
    return `  (${sqlStr(card.name)}, ${sqlStr(card.slug)}, ${sqlStr(card.description)}, 'BEGINNER', 8, ${sqlInt(d.default_sets ?? 1)}, ${sqlInt(reps)}, ${sqlInt(work)}, ${sqlInt(d.default_rest_seconds ?? 0)}, ${sqlInt(d.est_seconds_per_set ?? 45)}, ${sqlStr(card.cardSummary)}, ${sqlStr(card.coachLanguage)}, ${sqlStr(card.athleteLanguage)}, ${sqlStr(card.family)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.subrole)}, ${sqlStr(card.slot)}, ${jsonb(seedReq(card)).replace('::jsonb', '')}::jsonb, ${jsonb(seedExec(card)).replace('::jsonb', '')}::jsonb)`
  }).join(',\n')
...
SELECT e.id, sp.id, COALESCE(m.fit_weight, 5), 'primary', m.slot, pos.order_index,
  COALESCE(m.freshness_required, FALSE), COALESCE(m.fatigue_sensitivity, 1), COALESCE(m.fatigue_cost, 1),
  COALESCE(m.technical_complexity, 2), COALESCE(m.impact_level, 0), COALESCE(m.intensity_ceiling, 'low')
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    const pp = card.phaseProfile ?? {}
    return `  (${sqlStr(card.slug)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.slot)}, ${sqlInt(pp.fit_weight ?? 5)}, ${sqlBool(pp.freshness_required)}, ${sqlInt(pp.fatigue_sensitivity ?? 1)}, ${sqlInt(pp.fatigue_cost ?? 1)}, ${sqlInt(pp.technical_complexity ?? 2)}, ${sqlInt(pp.impact_level ?? 0)}, ${sqlStr(pp.intensity_ceiling ?? 'low')})`
  }).join(',\n')
  seedSql += `
...
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    const d = card.dosage ?? {}
    return `  (${sqlStr(card.slug)}, ${sqlStr(d.volume_unit ?? 'reps')}, ${sqlInt(d.default_sets ?? 1)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 0)}, NULL::integer, ${sqlInt(d.est_seconds_per_set ?? 45)}, ${sqlInt(d.default_rpe_min ?? 1)}, ${sqlInt(d.default_rpe_max ?? 3)})`
  }).join(',\n')
  seedSql += `
...
SELECT e.id,
  ...
  COALESCE(m.weekly_max_frequency, 7),
...
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    ...
    return `  (${sqlStr(card.slug)}, ${sqlBool(r.can_be_daily ?? true)}, ${sqlInt(r.weekly_max_frequency ?? 7)}, ${sqlInt(r.minimum_hours_between_hard_exposures ?? 0)}, ${sqlBool(r.counts_as_high_intensity)}, ${sqlBool(r.counts_as_high_impact)}, ${sqlBool(r.counts_as_neural)}, ${sqlBool(r.counts_as_tissue_stress)}, ${sqlBool(r.counts_as_conditioning)})`
  }).join(',\n')
  seedSql += `
...
FROM (VALUES\n`
    seedSql += INSERT_CARDS.map((card) => {
      const guidance = card.scaling?.[ck] ?? card.scaling?.adult_beginner ?? defaultScaling(card)
...
let hydrateSql = `-- Prepare & Access dynamic mobility 50-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-167-mobility.mjs
...
for (const card of CARDS) {
  ...
  hydrateSql += `-- ${card.name} (${card.slug})${card._action === 'merge' ? ' [merge]' : ''}\n`
  hydrateSql += `UPDATE coaching.exercise SET
  ...
  default_sets = ${sqlInt(d.default_sets ?? 1)},
  default_reps = ${sqlInt(d.default_reps)},
  default_work_seconds = ${sqlInt(d.default_work_seconds)},
  default_rest_seconds = ${sqlInt(d.default_rest_seconds ?? 0)},
  est_seconds_per_set = ${sqlInt(d.est_seconds_per_set ?? 45)},
...
UPDATE coaching.exercise_phase_profile p SET
  ...
  fatigue_sensitivity = ${sqlInt(pp.fatigue_sensitivity ?? 1)},
...
SELECT e.id, sp.id, ${sqlInt(pp.fit_weight ?? 5)}, ${sqlStr(pp.role ?? 'primary')}, ${sqlStr(card.slot)}, pos.order_index,
  ${sqlBool(pp.freshness_required)}, ${sqlInt(pp.fatigue_sensitivity ?? 1)}, ${sqlInt(pp.fatigue_cost ?? 1)},
  ${sqlInt(pp.technical_complexity ?? 2)}, ${sqlInt(pp.impact_level ?? 0)}, ${sqlStr(pp.intensity_ceiling ?? 'low')}
FROM coaching.exercise e
JOIN coaching.session_phase sp ON sp.key = ${sqlStr(card.primaryPhaseKey)}
JOIN coaching.phase_order_slot pos ON pos.key = ${sqlStr(card.slot)} AND pos.phase_id = sp.id
...
UPDATE coaching.exercise_dosage_profile d SET
  ...
  default_rest_seconds = ${sqlInt(d.default_rest_seconds ?? 0)},
  est_seconds_per_set = ${sqlInt(d.est_seconds_per_set ?? 45)},
  default_rpe_min = ${sqlInt(d.default_rpe_min ?? 1)},
  default_rpe_max = ${sqlInt(d.default_rpe_max ?? 3)},
...
SELECT e.id, 'Default', TRUE, ${sqlStr(d.volume_unit ?? 'reps')}, ${sqlInt(d.default_sets ?? 1)}, ${sqlInt(d.default_reps)},
  ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 0)}, ${defaultDistance},
  ${sqlInt(d.est_seconds_per_set ?? 45)}, ${sqlInt(d.default_rpe_min ?? 1)}, ${sqlInt(d.default_rpe_max ?? 3)}
FROM coaching.exercise e
WHERE e.slug = ${sqlStr(card.slug)}
...
UPDATE coaching.exercise_regimen_rule r SET
  ...
  weekly_max_frequency = ${sqlInt(r.weekly_max_frequency ?? 7)},
...
SELECT e.id,
  ${sqlBool(r.can_be_daily ?? true)}, ${sqlInt(r.weekly_max_frequency ?? 7)}, ${sqlInt(r.minimum_hours_between_hard_exposures ?? 0)},
  ${sqlBool(r.counts_as_high_intensity)}, ${sqlBool(r.counts_as_high_impact)}, ${sqlBool(r.counts_as_neural)},
...
SELECT e.id, ${sqlInt(s.risk_level ?? 1)}, ${sqlInt(s.impact_level ?? 0)}, ${sqlBool(s.requires_spotting)},
...
const seedPath = path.join(MIG_DIR, '169_coaching_mobility_infrastructure_and_seed.sql')
const hydratePath = path.join(MIG_DIR, '170_coaching_mobility_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'mobility cards (', INSERT_CARDS.length, 'insert,', MERGE_CARDS.length, 'merge)')
