/**
 * Generates:
 *   backend/migrations/200_coaching_rotational_power_infrastructure_and_seed.sql
 *   backend/migrations/201_coaching_rotational_power_cards.sql
 *
 * Prerequisite: scripts/data/rotational-power-exercise-library-all-50.json
 * Run: node scripts/generate-200-rotational-power.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/rotational-power-exercise-library-all-50.json')
const data = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))
const LIBRARY = data.library ?? data.cluster ?? {}
...
const METHODOLOGY_KEY_MAP = {
  loaded_strength: 'resistance_calisthenics',
  resistance_training: 'resistance_calisthenics',
  speed_agility: 'neural',
  medicine_ball: null,
}

const PHYSIOLOGY_KEY_MAP = {
  tissue_range_access: 'control_stability',
...
function detectMergeSlugs(cards) {
  ...
  for (const card of cards) {
    for (const file of fs.readdirSync(migDir)) {
      ...
      const fp = path.join(migDir, file)
      if (!fs.existsSync(fp)) continue
      const content = fs.readFileSync(fp, 'utf8')
      if (content.includes(`'${card.slug}'`)) {
        merge.add(card.slug)
        break
      }
    }
...
function normalizePairing(card) {
  ...
  return {
    pairs_well_before: p.pairs_well_before ?? p.pairsWellBefore ?? card.pairsWellBefore ?? [],
    pairs_well_after: p.pairs_well_after ?? p.pairsWellAfter ?? card.pairsWellAfter ?? [],
    good_for_sessions: p.good_for_sessions ?? p.goodForSessions ?? card.goodForSessions ?? ['rotational_power', 'rotational_athlete_prep'],
    avoid_before: p.avoid_before ?? p.avoidBefore ?? [],
    avoid_after: p.avoid_after ?? p.avoidAfter ?? [],
    do_not_use_when: p.do_not_use_when ?? p.doNotUseWhen ?? card.doNotUseWhen ?? [],
    notes: p.notes ?? p.coach_note ?? null,
  }
}

function mapMethodologyKey(key) {
  const mapped = METHODOLOGY_KEY_MAP[key]
...
function facetTags(card, field) {
  const raw = field === 'body_regions' ? (card.body_regions ?? card.bodyRegions ?? []) : (card[field] ?? [])
  const out = []
  const seen = new Set()
  for (const t of raw) {
    let key = t.key
    if (field === 'methodologies') {
      key = mapMethodologyKey(key)
      if (!key) continue
    }
    if (field === 'physiology') key = mapPhysiologyKey(key)
    if (field === 'equipment') key = mapEquipmentKey(key)
...
const CARDS = RAW_CARDS.map((c) => ({
  ...
  pairingLogic: normalizePairing(c),
}))
...
const PHASE_ORDER_BASE = {
  prepare_and_access: 148,
  movement_intelligence: 668,
  output: 788,
  capacity: 505,
  resilience: 758,
}
...
const VALIDATION_EDU = {
  title: 'Rotational power without lumbar twisting under fatigue or wrong phase intent',
  short_summary:
  ...
  what_it_is:
    LIBRARY.title ?? 'Top 50 Rotational Athletes and Rotational Power Exercise Library',
  why_it_matters:
    LIBRARY.purpose ?? 'Build rotational athletes who can access rotation, control rotation, produce force through the ground and trunk, express high-quality rotational power, and brake or redirect rotational momentum safely.',
  programming_guidance:
    LIBRARY.programming_rule ?? 'Rotational power is quality-first. Access and sequencing can happen early at low fatigue; high-intent throws, jumps, and cuts belong in Output while fresh; loaded rotational strength belongs in Capacity; anti-rotation and braking belong in Resilience.',
  common_misuse:
...
function seedExec(card) {
  ...
  return {
    ...
    quality_gate: exec.quality_gate ?? null,
    stop_signs: exec.stop_signs ?? [],
  }
}

function defaultScaling(card, ck) {
  return card.scaling?.[ck] ?? card.scalingGuidance
    ?? 'Scale by range, speed, load, stance, throw distance, or reactive complexity first. Progress only when rotation source, finish, landing, and reset stay repeatable.'
}

// ── 200: infrastructure + seed ─────────────────────────────────────────────

let seedSql = `-- Rotational power exercise library: infrastructure and ${INSERT_CARDS.length}-card seed.
...
-- Library: ${sqlStr(LIBRARY.title ?? 'Top 50 Rotational Athletes and Rotational Power Exercise Library')}
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} rotational power slots\n`
...
seedSql += `-- Insert ${INSERT_CARDS.length} new rotational power exercises
...
seedSql += INSERT_CARDS.map((card) => {
  const d = card.dosage ?? {}
  return `  (${sqlStr(card.name)}, ${sqlStr(card.slug)}, ${sqlStr(card.description)}, 8, ${sqlInt(d.default_sets ?? 2)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 30)}, ${sqlInt(d.est_seconds_per_set ?? 40)}, ${sqlStr(card.cardSummary)}, ${sqlStr(card.coachLanguage)}, ${sqlStr(card.athleteLanguage)}, ${sqlStr(card.family)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.subrole)}, ${sqlStr(slotKeyFor(card))}, ${movementRequirementsJson(card).replace('::jsonb', '')}::jsonb, ${jsonb(seedExec(card)).replace('::jsonb', '')}::jsonb)`
}).join(',\n')
...
seedSql += INSERT_CARDS.map((card) => {
  const d = card.dosage ?? {}
  return `  (${sqlStr(card.slug)}, ${sqlStr(d.volume_unit ?? 'reps')}, ${sqlInt(d.default_sets ?? 2)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 30)}, ${sqlDistance(d.default_distance)}, ${sqlInt(d.est_seconds_per_set ?? 40)}, ${sqlInt(d.default_rpe_min ?? 3)}, ${sqlInt(d.default_rpe_max ?? 6)})`
}).join(',\n')
seedSql += `
...
let hydrateSql = `-- Rotational power ${CARDS.length}-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-200-rotational-power.mjs (migrations 200/201)
...
for (const card of CARDS) {
  const pairing = card.pairingLogic ?? {}
  const media = {
    demo_video_sources: [],
    coaching_articles: [],
  ...
  const defaultDistance = sqlDistance(d.default_distance)
...
hydrateSql += CARDS.map((c) =>
  `  (${sqlStr(c.slug)}, ${sqlStr(c.whyItWorks)}, ${sqlStr(c.selectionRationale ?? c.whyItWorks)}, ${sqlStr(c.whyItGoesHere)}, ${sqlStr(c.bestPlacement)}, ${sqlStr(c.commonMisuse)}, ${sqlStr(c.scalingGuidance)})`,
).join(',\n')
hydrateSql += `
...
const seedPath = path.join(__dirname, '../backend/migrations/200_coaching_rotational_power_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/201_coaching_rotational_power_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'rotational power cards (', INSERT_CARDS.length, 'insert,', MERGE_COUNT, 'merge-only)')
