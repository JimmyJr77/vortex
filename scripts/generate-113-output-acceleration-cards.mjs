/**
 * Generates backend/migrations/113_coaching_output_acceleration_cards.sql
 * Run: node scripts/generate-113-output-acceleration-cards.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import {
  OUTPUT_ACCELERATION_CARDS,
  OUTPUT_ACCELERATION_SLUGS,
} from './data/output-acceleration-cards-1-10.mjs'
...
function mapVolumeUnit(unit) {
  if (unit === 'sprints' || unit === 'races') return 'attempts'
...
const VALIDATION_EDU = {
  title: 'Acceleration Output without fatigue',
  short_summary:
...
let sql = `-- Output phase acceleration cards 1–10 (rich card v2 content).
...
for (const card of OUTPUT_ACCELERATION_CARDS) {
  const pairing = {
...
sql += `-- Refresh weighted taxonomy tags for acceleration Output cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${OUTPUT_ACCELERATION_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += OUTPUT_ACCELERATION_CARDS.map((c) => {
    const guidance = c.scaling[ck] ?? c.scaling.adult_beginner ?? 'Short acceleration reps with full reset; stop when speed drops.'
...
const outPath = path.join(__dirname, '../backend/migrations/113_coaching_output_acceleration_cards.sql')
fs.writeFileSync(outPath, sql)
console.log('Wrote', outPath, '—', OUTPUT_ACCELERATION_CARDS.length, 'output acceleration cards')
