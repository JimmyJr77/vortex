/**
 * Generates:
 *   backend/migrations/167_coaching_isometrics_infrastructure_and_seed.sql
 *   backend/migrations/168_coaching_isometrics_cards.sql
 *
 * Prerequisite: node scripts/ingest-isometrics-sources.mjs
 * Run: node scripts/generate-167-isometrics.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/isometrics-all-cards.json')
...
function detectMergeSlugs(cards) {
  const files = fs.readdirSync(MIG_DIR)
    .filter((f) => f.endsWith('.sql') && !f.startsWith('167_') && !f.startsWith('168_'))
...
const PHASE_ORDER_BASE = {
  prepare_and_access: 691,
  movement_intelligence: 701,
  capacity: 701,
  resilience: 696,
}
...
const EQUIPMENT_SEED = [
  ['anchor_point', 'Anchor Point', 110],
  ['doorframe', 'Doorframe', 111],
  ['mini_band_optional', 'Mini Band (Optional)', 112],
  ['pins', 'Rack Pins / Safety Pins', 113],
  ['rack_optional', 'Rack (Optional)', 114],
  ['slant_board_optional', 'Slant Board (Optional)', 115],
  ['straps_optional', 'Straps (Optional)', 116],
  ['support_optional', 'Support / Box (Optional)', 117],
]

const VALIDATION_EDU = {
  title: 'Isometrics without breath-holding contests or wrong phase intent',
  short_summary:
    'Static holds, overcoming presses, and joint-angle isometrics must match phase intent — low-fatigue activation in Prepare & Access, shape ownership in Movement Intelligence, tissue tolerance in Resilience, and loaded bracing in Capacity — never fatigue finishers or max-effort holds before fresh Output.',
  what_it_is:
    'Isometrics cards span trunk anti-movement holds, lower-body joint-angle strength, foot-ankle resilience, upper-body support and overcoming presses, and loaded pin/rack isometrics for athletes building position ownership, tissue tolerance, and force at specific angles without visible movement.',
  why_it_matters:
    'Isometric quality depends on exact joint angles, breathing, intent, and stop rules. Poor placement, breath-holding contests, or holds after fatigue steal from speed, skill, and neural output while raising blood-pressure and compensation risk.',
  programming_guidance:
    'Scale by position quality first, then duration, then load or intent. Use Prepare & Access for activation and access; Movement Intelligence for shape holds; Resilience for anti-movement and tissue tolerance; Capacity for loaded overcoming isometrics with full rest.',
  common_misuse:
    'Do not turn holds into pain-tolerance or breath-holding contests; do not stack multiple high-intent isometrics before Output; do not program pin/rack isometrics without safe setup and supervision.',
}
...
function defaultDosageFallback(card) {
  ...
  if (phase === 'prepare_and_access') {
    return { sets: 2, rest: 30, est: 50, rpeMin: 2, rpeMax: 4 }
  }
  if (phase === 'capacity') {
    return { sets: 3, rest: 90, est: 120, rpeMin: 6, rpeMax: 8 }
  }
  return { sets: d.default_sets ?? 2, rest: d.default_rest_seconds ?? 40, est: d.est_seconds_per_set ?? 60, rpeMin: 4, rpeMax: 6 }
}

function defaultScaling(card, ck) {
  return card.scaling?.[ck] ?? card.scaling?.adult_beginner
    ?? 'Scale isometrics first by position quality, then hold duration, then external load or intent.'
}
...
let seedSql = `-- Isometrics cross-phase infrastructure, taxonomy support, and ${INSERT_CARDS.length}-card seed.
...
-- Cluster: ${data.cluster?.focus ?? 'isometrics'} (${data.cluster?.card_count ?? CARDS.length} cards).
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} isometrics slots\n`
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} new isometrics exercises\n`
...
seedSql += `-- Isometrics cluster validation education
...
let hydrateSql = `-- Isometrics ${CARDS.length}-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-167-isometrics.mjs (migrations 167/168)
...
const seedPath = path.join(MIG_DIR, '167_coaching_isometrics_infrastructure_and_seed.sql')
const hydratePath = path.join(MIG_DIR, '168_coaching_isometrics_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'isometrics cards (', INSERT_CARDS.length, 'insert,', MERGE_CARDS.length, 'merge)')
