/**
 * Generates:
 *   backend/migrations/149_coaching_loaded_strength_infrastructure_and_seed.sql
 *   backend/migrations/150_coaching_loaded_strength_cards.sql
 *
 * Prerequisite: node scripts/ingest-loaded-strength-sources.mjs
 * Run: node scripts/generate-147-loaded-strength.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/loaded-strength-all-cards.json')
const data = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))
const CARDS = data.clusters.flatMap((c) => c.cards)
const INSERT_CARDS = CARDS.filter((c) => c._action === 'insert')
const SLUGS = CARDS.map((c) => c.slug)

const PHASE_ORDER_BASE = { capacity: 450 }
...
const VALIDATION_EDU = {
  title: 'Loaded strength without conditioning drift or wrong phase intent',
  short_summary:
    'Barbell and dumbbell Capacity strength must use intentional load, full rest, and strength-first scoring — not fatigue circuits, plyometric substitution, or Output-speed intent.',
  what_it_is:
    'Loaded strength cards span barbell and dumbbell squat, hinge, push, pull, carry, and trunk-accessory patterns for Capacity-phase force and tissue development.',
  why_it_matters:
    'Strength reserve supports speed, jumping, and resilience when dosed correctly. Poor placement, short rest, or mis-tagged explosiveness trains sloppy mechanics and steals from Output work.',
  programming_guidance:
    'Place after Prepare & Access, Skill, and true Output when in the same session. Score Strength high; add Explosiveness only with crisp concentric intent, low fatigue, and full recovery.',
  common_misuse:
    'Do not program heavy strength after conditioning, use high-rep burnout sets, or tag strength lifts as explosiveness when delivery is fatigued or slow.',
}
...
-- IDEMPOTENT. Generated by scripts/generate-147-loaded-strength.mjs (migrations 149/150)
-- ${CARDS.length} total cards (${INSERT_CARDS.length} insert, ${CARDS.length - INSERT_CARDS.length} merge-only).

INSERT INTO coaching.equipment (key, name, sort_order) VALUES
  ('squat_rack', 'Squat Rack', 27),
  ('plates', 'Weight Plates', 28),
  ('landmine', 'Landmine', 29),
  ('low_step', 'Low Step / Block', 30),
  ('bench_optional', 'Bench (Optional)', 31),
  ('bench_or_floor', 'Bench or Floor', 32),
  ('small_plate_or_wedge', 'Small Plate / Wedge', 33)
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} loaded strength slots\n`
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} new loaded strength exercises\n`
...
seedSql += `-- Loaded strength cluster validation education
...
let hydrateSql = `-- Loaded strength ${CARDS.length}-card full hydration (barbell + dumbbell, card v2).
-- IDEMPOTENT. Generated by scripts/generate-147-loaded-strength.mjs (migrations 149/150)
...
SELECT
  ...
  e.description,
  v.why_works,
  'Develops tagged tenets and force/tissue capacity for loaded strength in the Capacity phase.',
  v.why_here,
  v.best_placement,
  v.misuse,
  v.scale_guidance
...
const seedPath = path.join(__dirname, '../backend/migrations/149_coaching_loaded_strength_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/150_coaching_loaded_strength_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'loaded strength cards (', INSERT_CARDS.length, 'insert,', CARDS.length - INSERT_CARDS.length, 'merge)')
