/**
 * Generates:
 *   backend/migrations/196_coaching_kicking_athletes_infrastructure_and_seed.sql
 *   backend/migrations/197_coaching_kicking_athletes_cards.sql
 *
 * Prerequisite: node scripts/ingest-kicking-athletes-sources.mjs
 * Run: node scripts/generate-186-kicking-athletes.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/kicking-athletes-all-cards.json')
...
const PHASE_ORDER_BASE = {
  ...
  movement_intelligence: 641,
  output: 381,
  capacity: 671,
  resilience: 731,
  sustained_capacity: 621,
  restore: 911,
}
...
const VALIDATION_EDU = {
  title: 'Kicking athletes without wrong phase intent or conditioning drift',
  short_summary:
...
function defaultScaling(card, ck) {
  return card.scaling?.[ck] ?? card.scaling?.adult_beginner
    ?? 'Scale by range, speed, load, target height, partner pressure, or impact first. Progress only one variable at a time after the quality gate is owned.'
}

// ── 196: infrastructure + seed ─────────────────────────────────────────────

let seedSql = `-- Kicking Athletes — phase infrastructure, taxonomy support, and ${CARDS.length}-card seed.
-- IDEMPOTENT. Generated by scripts/generate-186-kicking-athletes.mjs (migrations 196/197)
-- Cluster: ${data.cluster?.focus ?? 'kicking_athletes_kickboxing_mma_soccer'} (${data.cluster?.card_count ?? CARDS.length} cards).
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} kicking athletes slots\n`
...
seedSql += `-- Insert ${CARDS.length} kicking athletes exercises
...
seedSql += CARDS.map((card) => {
  const d = card.dosage ?? {}
  return `  (${sqlStr(card.name)}, ${sqlStr(card.slug)}, ${sqlStr(card.description)}, 8, ${sqlInt(d.default_sets ?? 2)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 30)}, ${sqlInt(d.est_seconds_per_set ?? 40)}, ${sqlStr(card.cardSummary)}, ${sqlStr(card.coachLanguage)}, ${sqlStr(card.athleteLanguage)}, ${sqlStr(card.family)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.subrole)}, ${sqlStr(slotKeyFor(card))}, ${jsonb(seedReq(card)).replace('::jsonb', '')}::jsonb, ${jsonb(seedExec(card)).replace('::jsonb', '')}::jsonb)`
}).join(',\n')
...
let hydrateSql = `-- Kicking Athletes ${CARDS.length}-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-186-kicking-athletes.mjs (migrations 196/197)
...
const seedPath = path.join(__dirname, '../backend/migrations/196_coaching_kicking_athletes_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/197_coaching_kicking_athletes_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'kicking athletes cards')
