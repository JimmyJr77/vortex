/**
 * Generates:
 *   backend/migrations/155_coaching_bodyweight_strength_infrastructure_and_seed.sql
 *   backend/migrations/156_coaching_bodyweight_strength_cards.sql
 *
 * Prerequisite: node scripts/ingest-bodyweight-strength-sources.mjs
 * Run: node scripts/generate-153-bodyweight-strength.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/bodyweight-strength-all-cards.json')
const data = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))
const CARDS = data.cards ?? []
const INSERT_CARDS = CARDS.filter((c) => c._action !== 'merge')
const MERGE_CARDS = CARDS.filter((c) => c._action === 'merge')
const SLUGS = CARDS.map((c) => c.slug)

const PHASE_ORDER_BASE = { capacity: 600 }
...
const NEW_EQUIPMENT = {
  bar_or_rings: ['Bar or Rings', 36],
  box_optional: ['Box (Optional)', 37],
  box_or_bench: ['Box or Bench', 38],
  box_or_chair_optional: ['Box or Chair (Optional)', 39],
  dip_bars: ['Dip Bars', 40],
  mat_optional: ['Mat (Optional)', 41],
  parallettes_optional: ['Parallettes (Optional)', 42],
  parallettes_or_blocks: ['Parallettes or Blocks', 43],
  parallettes_or_dip_bars_optional: ['Parallettes or Dip Bars (Optional)', 44],
  partner_or_anchor: ['Partner or Anchor', 45],
  rings_or_bar: ['Rings or Bar', 46],
  sliders_or_towels: ['Sliders or Towels', 47],
  step_optional: ['Step (Optional)', 48],
  straight_bar: ['Straight Bar', 49],
  upright_support_optional: ['Upright Support (Optional)', 50],
  wall_support_optional: ['Wall Support (Optional)', 51],
}

const VALIDATION_EDU = {
  title: 'Bodyweight strength without conditioning drift or wrong phase intent',
  short_summary:
    'Bodyweight Capacity strength must use leverage, tempo, pause, and full rest — not burpee-style fatigue circuits, jump-based delivery, or Output-speed intent.',
  what_it_is:
    'Bodyweight strength cards span squat, hinge, push, pull, dip, trunk, and lower-leg patterns for Capacity-phase relative force and tissue development using calisthenics leverage.',
  why_it_matters:
    'Relative strength reserve supports speed, jumping, and resilience when dosed correctly. Poor placement, short rest, or mis-tagged explosiveness trains sloppy mechanics and steals from Output work.',
  programming_guidance:
    'Place after Prepare & Access, Skill, and true Output when in the same session. Score Strength high; add Explosiveness only with crisp concentric intent, low fatigue, and full recovery. Do not program burpees, jump squats, kipping, or conditioning circuits as Capacity strength substitutes.',
  common_misuse:
    'Do not program bodyweight strength after conditioning, use high-rep burnout sets, or tag strength drills as explosiveness when delivery is fatigued, jump-based, or kipping.',
}
...
let seedSql = `-- Bodyweight strength infrastructure, taxonomy, and ${INSERT_CARDS.length}-card seed.
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} bodyweight strength slots\n`
...
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    const d = card.dosage ?? {}
    return `  (${sqlStr(card.slug)}, ${sqlStr(d.volume_unit ?? 'reps')}, ${sqlInt(d.default_sets ?? 3)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 120)}, ${sqlDistance(d.default_distance)}, ${sqlInt(d.est_seconds_per_set ?? 150)}, ${sqlInt(d.default_rpe_min ?? 6)}, ${sqlInt(d.default_rpe_max ?? 8)})`
  }).join(',\n')
  seedSql += `
...
FROM (VALUES\n`
    seedSql += INSERT_CARDS.map((card) => {
      const guidance = card.scaling?.[ck] ?? card.scaling?.adult_beginner ?? 'Scale by leverage, range, tempo, and rest before adding complexity.'
...
seedSql += `-- Bodyweight strength cluster validation education
...
let hydrateSql = `-- Bodyweight strength ${CARDS.length}-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-153-bodyweight-strength.mjs (migrations 155/156)
...
for (const card of CARDS) {
  ...
  const defaultDistance = d.default_distance != null ? sqlDistance(d.default_distance) : 'NULL::integer'

  hydrateSql += `-- ${card.name} (${card.slug}) [${card._action ?? 'insert'}]
UPDATE coaching.exercise SET
  ...
  default_rest_seconds = ${sqlInt(d.default_rest_seconds ?? 120)},
  est_seconds_per_set = ${sqlInt(d.est_seconds_per_set ?? 150)},
...
UPDATE coaching.exercise_phase_profile p SET
  ...
  fatigue_sensitivity = ${sqlInt(pp.fatigue_sensitivity ?? 3)},
...
UPDATE coaching.exercise_dosage_profile d SET
  ...
  default_rest_seconds = ${sqlInt(d.default_rest_seconds ?? 120)},
  est_seconds_per_set = ${sqlInt(d.est_seconds_per_set ?? 150)},
  default_rpe_min = ${sqlInt(d.default_rpe_min ?? 6)},
  default_rpe_max = ${sqlInt(d.default_rpe_max ?? 8)},
...
UPDATE coaching.exercise_safety_profile s SET
  ...
  requires_coach_supervision = ${sqlStr(s.requires_coach_supervision ?? 'recommended')},
...
const seedPath = path.join(__dirname, '../backend/migrations/155_coaching_bodyweight_strength_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/156_coaching_bodyweight_strength_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'bodyweight strength cards (', INSERT_CARDS.length, 'insert,', MERGE_CARDS.length, 'merge)')
