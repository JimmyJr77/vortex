/**
 * Generates backend/migrations/122_coaching_capacity_squat_cards.sql
 * Run: node scripts/generate-122-capacity-squat-cards.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import { CAPACITY_SQUAT_CARDS, CAPACITY_SQUAT_SLUGS } from './data/capacity-squat-cards-1-10.mjs'
...
const VALIDATION_EDU = {
  title: 'Squat / Knee-Dominant Capacity without stealing Output quality',
  short_summary:
...
let sql = `-- Capacity phase Squat / Knee-Dominant Strength cards 1–10 (rich card v2 content).
...
for (const card of CAPACITY_SQUAT_CARDS) {
  const pairing = {
...
sql += `-- Refresh weighted taxonomy tags for squat / knee-dominant Capacity cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${CAPACITY_SQUAT_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += CAPACITY_SQUAT_CARDS.map((c) => {
    const guidance = c.scaling[ck] ?? c.scaling.adult_beginner ?? 'Conservative squat dose — prioritize mechanics and full rest.'
...
const outPath = path.join(__dirname, '../backend/migrations/122_coaching_capacity_squat_cards.sql')
fs.writeFileSync(outPath, sql)
console.log('Wrote', outPath, '—', CAPACITY_SQUAT_CARDS.length, 'squat / knee-dominant Capacity cards')
