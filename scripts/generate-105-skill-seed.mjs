/**
 * Generates backend/migrations/105_coaching_skill_movement_intelligence_seed.sql
...
const PRIMARY_PHASE = 'skill_movement_intelligence'
...
function coachLangText(m) {
  return `Use in Skill / Movement Intelligence (${m.subrole.replace(/_/g, ' ')}) while the athlete is fresh.`
}
...
let sql = `-- Seed 50 Skill / Movement Intelligence movements (card v2 + subrole hierarchy).
...
sql += `-- 3) Skill / Movement Intelligence phase profiles\nINSERT INTO coaching.exercise_phase_profile (
...
// Legacy updates
sql += `-- 9) Legacy library slugs → Skill / Movement Intelligence (primary phase)\n`
...
// a-march note: primary_phase_key stays prepare_access from migration 097; skill profile already inserted in section 3.
sql += `-- 10) a-march dual profile stub: prepare_access primary unchanged; skill profile on technical_march is in section 3.
-- (No UPDATE to primary_phase_key for a-march.)

`

const outPath = path.join(__dirname, '../backend/migrations/105_coaching_skill_movement_intelligence_seed.sql')
