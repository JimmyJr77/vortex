/**
 * Generates:
 *   backend/migrations/177_coaching_resistance_band_body_resistance_infrastructure_and_seed.sql
 *   backend/migrations/178_coaching_resistance_band_body_resistance_cards.sql
 *
 * Prerequisite: node scripts/ingest-resistance-band-body-resistance-sources.mjs
 * Run: node scripts/generate-177-resistance-band-body-resistance.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/resistance-band-body-resistance-all-cards.json')
...
function detectMergeSlugs(cards) {
  const files = fs.readdirSync(MIG_DIR)
    .filter((f) => f.endsWith('.sql') && !f.startsWith('177_') && !f.startsWith('178_'))
...
const PHASE_ORDER_BASE = { capacity: 651, resilience: 720 }
...
const VALIDATION_EDU = {
  title: 'Resistance band and body resistance without conditioning drift or wrong phase intent',
  short_summary:
    'Band and body-resistance work must use leverage, tempo, tension, and full rest — not HIIT circuits, plyometric substitution, or Output-speed intent. Strength and body control are primary scores.',
  what_it_is:
    'Resistance band and body resistance cards span Capacity squat/hinge/push/pull strength and Resilience tissue-capacity, anti-movement, and joint-control patterns using bands, leverage, and minimal equipment.',
  why_it_matters:
    'Low-equipment resistance builds force, tissue tolerance, and position ownership when dosed correctly. Poor placement, short rest, or mis-tagged explosiveness trains sloppy mechanics and steals from Output work.',
  programming_guidance:
    'Place Capacity strength after Prepare & Access, Skill, and true Output when in the same session. Score Strength and body control honestly; keep Resilience work submaximal and tissue-focused. Do not program band/body circuits as Capacity strength substitutes.',
  common_misuse:
    'Do not chase reps, band tension, or speed after position quality drops. Do not program resistance work after conditioning or tag it as explosiveness when delivery is fatigued.',
}
...
let seedSql = `-- Resistance band + body resistance infrastructure, taxonomy, and ${INSERT_CARDS.length}-card seed.
...
-- Cluster: ${data.cluster?.focus ?? 'resistance_band_and_body_resistance'} (${data.cluster?.card_count ?? CARDS.length} cards).
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} resistance band / body resistance slots\n`
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} new bodyweight strength exercises\n`
...
FROM (VALUES\n`
  ...
  seedSql += INSERT_CARDS.map((card) => {
    ...
    return `  (${sqlStr(card.name)}, ${sqlStr(card.slug)}, ${sqlStr(card.description)}, 8, ${sqlInt(d.default_sets ?? fb.sets)}, ${sqlInt(reps)}, ${sqlInt(work)}, ${sqlInt(d.default_rest_seconds ?? fb.rest)}, ${sqlInt(d.est_seconds_per_set ?? fb.est)}, ${sqlStr(card.cardSummary)}, ${sqlStr(card.coachLanguage)}, ${sqlStr(card.athleteLanguage)}, ${sqlStr(card.family)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.subrole)}, ${sqlStr(card.slot)}, ${jsonb(seedReq(card)).replace('::jsonb', '')}::jsonb, ${jsonb(seedExec(card)).replace('::jsonb', '')}::jsonb)`
  }).join(',\n')
...
seedSql += `-- Resistance band + body resistance cluster validation education
...
VALUES (
  'validation_rule',
  'resistance_band_body_resistance_readiness',
  NULL,
...
let hydrateSql = `-- Resistance band + body resistance ${CARDS.length}-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-177-resistance-band-body-resistance.mjs (migrations 177/178)
...
const seedPath = path.join(__dirname, '../backend/migrations/177_coaching_resistance_band_body_resistance_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/178_coaching_resistance_band_body_resistance_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'resistance band / body resistance cards (', INSERT_CARDS.length, 'insert,', MERGE_CARDS.length, 'merge)')
