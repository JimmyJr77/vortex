/**
 * Generates:
 *   backend/migrations/151_coaching_kettlebell_strength_infrastructure_and_seed.sql
 *   backend/migrations/152_coaching_kettlebell_strength_cards.sql
 *
 * Prerequisite: node scripts/ingest-kettlebell-strength-sources.mjs
 * Run: node scripts/generate-151-kettlebell-strength.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/kettlebell-strength-all-cards.json')
const data = JSON.parse(fs.readFileSync(SOURCE, 'utf8'))
const CARDS = data.cards ?? []
const INSERT_CARDS = CARDS.filter((c) => c._action !== 'merge')
const SLUGS = CARDS.map((c) => c.slug)

const PHASE_ORDER_BASE = { capacity: 550 }
...
const VALIDATION_EDU = {
  title: 'Kettlebell strength without conditioning drift or wrong phase intent',
  short_summary:
    'Kettlebell Capacity strength must use intentional load, full rest, and strength-first scoring — not fatigue circuits, swing/snatch substitution, or Output-speed intent.',
  what_it_is:
    'Kettlebell strength cards span squat, hinge, push, pull, carry, and integrated trunk/shoulder patterns for Capacity-phase force and tissue development using kettlebell-specific loading.',
  why_it_matters:
    'Kettlebell strength reserve supports speed, jumping, and resilience when dosed correctly. Poor placement, short rest, or mis-tagged explosiveness trains sloppy mechanics and steals from Output work.',
  programming_guidance:
    'Place after Prepare & Access, Skill, and true Output when in the same session. Score Strength high; add Explosiveness only with crisp concentric intent, low fatigue, and full recovery. Do not program swings, snatches, or conditioning circuits as Capacity strength substitutes.',
  common_misuse:
    'Do not program heavy kettlebell strength after conditioning, use high-rep burnout sets, or tag strength lifts as explosiveness when delivery is fatigued or slow.',
}
...
// ── 151: infrastructure + seed ─────────────────────────────────────────────

let seedSql = `-- Kettlebell strength infrastructure, taxonomy, and ${INSERT_CARDS.length}-card seed.
...
INSERT INTO coaching.equipment (key, name, sort_order) VALUES
  ('bench_or_box', 'Bench or Box', 34),
  ('wedge_or_plates', 'Wedge / Small Plates', 35)
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} kettlebell strength slots\n`
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} kettlebell strength exercises\n`
...
seedSql += `-- Kettlebell strength cluster validation education
...
let hydrateSql = `-- Kettlebell strength ${CARDS.length}-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-151-kettlebell-strength.mjs (migrations 151/152)
...
const seedPath = path.join(__dirname, '../backend/migrations/151_coaching_kettlebell_strength_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/152_coaching_kettlebell_strength_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'kettlebell strength cards')
