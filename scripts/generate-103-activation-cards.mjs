...
import { ACTIVATION_ACCESS_CARDS, ACTIVATION_ACCESS_SLUGS } from './data/activation-access-cards-45-50.mjs'
...
let sql = `-- Activation Prepare/Access cards 45–50 (rich card v2 content).
...
for (const card of ACTIVATION_ACCESS_CARDS) {
  const pairing = {
...
sql += `-- Refresh weighted taxonomy tags for activation cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${ACTIVATION_ACCESS_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += ACTIVATION_ACCESS_CARDS.map((c) => {
