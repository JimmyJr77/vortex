/**
 * Generates:
 *   backend/migrations/186_coaching_medicine_balls_slam_balls_infrastructure_and_seed.sql
 *   backend/migrations/187_coaching_medicine_balls_slam_balls_cards.sql
 *
 * Run: node scripts/generate-182-medicine-balls-slam-balls.mjs
 */
import fs from 'fs'
...
const SOURCE = path.join(__dirname, 'data/medicine-balls-slam-balls-all-cards.json')
...
const VALIDATION_EDU = {
  title: 'Medicine-ball and slam-ball work without conditioning drift or wrong phase intent',
  short_summary:
    'Throws, slams, catches, trunk control, loaded ball strength, and partner timing must stay phase-honest — high intent and low volume in Output, control in Resilience, strength in Capacity — not random ball work or fatigue entertainment.',
  what_it_is:
    'Medicine-ball and slam-ball cards span explosive throws and slams, trunk control and anti-rotation, catch-and-decel skill, movement intelligence, partner exchange, loaded ball strength, and controlled late-session repeatability.',
  why_it_matters:
    'Ball drills give immediate feedback on bracing, timing, and force transfer. Poor load choice, short rest, unsafe spacing, or high rep counts after quality fades turn power and skill work into conditioning with higher injury risk.',
  programming_guidance:
    'Place after Prepare & Access and any needed Movement Intelligence; keep volume low and intent high for Output throws/slams; use full rest; stop when ball speed, catch quality, posture, or spacing drops; never use heavy slams or partner drills in unsafe lanes.',
  common_misuse:
    'Do not program medicine-ball or slam-ball work as a finisher or fatigue circuit; do not choose load that changes the pattern; do not count slow throws or sloppy catches after speed and posture have dropped.',
}

function pairingFromCard(card) {
  ...
  return {
    ...
    good_for_sessions: p.goodForSessions ?? card.goodForSessions ?? [],
    avoid_before: p.avoidBefore ?? card.avoidBefore ?? [],
    avoid_after: p.avoidAfter ?? card.avoidAfter ?? [],
    do_not_use_when: p.doNotUseWhen ?? card.doNotUseWhen ?? [],
  }
}

function coachSupervisionValue(safety) {
  const v = safety?.requires_coach_supervision
  if (v === true || v === 'required') return 'required'
  if (v === false || v === 'optional') return 'optional'
  return String(v ?? 'optional')
}
...
function genderNotes(card) {
  return card.scaling?.genderSpecificNotes ?? card.genderSpecificNotes ?? ''
}
...
let seedSql = `-- Medicine Balls & Slam Balls (Output) phase infrastructure and ${INSERT_CARDS.length}-card seed.
...
for (const phase of [...new Set(slots.map((s) => s.phase))]) {
  ...
  seedSql += `-- ${phase} medicine_balls_slam_balls slots\n`
...
if (INSERT_CARDS.length > 0) {
  seedSql += `-- Insert ${INSERT_CARDS.length} new medicine_balls_slam_balls exercises\n`
...
FROM (VALUES\n`
  ...
  seedSql += INSERT_CARDS.map((card) => {
    ...
    return `  (${sqlStr(card.name)}, ${sqlStr(card.slug)}, ${sqlStr(card.description)}, 8, ${sqlInt(d.default_sets ?? 3)}, ${sqlInt(reps)}, ${sqlInt(work)}, ${sqlInt(d.default_rest_seconds ?? 75)}, ${sqlInt(d.est_seconds_per_set ?? 100)}, ${sqlStr(card.cardSummary)}, ${sqlStr(card.coachLanguage)}, ${sqlStr(card.athleteLanguage)}, ${sqlStr(card.family)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.subrole)}, ${sqlStr(card.slot)}, ${movementRequirementsJson(card).replace('::jsonb', '')}::jsonb, ${jsonb(seedExec(card)).replace('::jsonb', '')}::jsonb)`
  }).join(',\n')
...
seedSql += INSERT_CARDS.map((card) => {
  ...
  return `  (${sqlStr(card.slug)}, ${sqlStr(card.primaryPhaseKey)}, ${sqlStr(card.slot)}, ${sqlInt(pp.fit_weight ?? 5)}, ${sqlBool(pp.freshness_required ?? true)}, ${sqlInt(pp.fatigue_sensitivity ?? 5)}, ${sqlInt(pp.fatigue_cost ?? 3)}, ${sqlInt(pp.technical_complexity ?? 2)}, ${sqlInt(pp.impact_level ?? 2)}, ${sqlStr(pp.intensity_ceiling ?? 'moderate')})`
}).join(',\n')
seedSql += `
...
seedSql += INSERT_CARDS.map((card) => {
  const d = card.dosage ?? {}
  return `  (${sqlStr(card.slug)}, ${sqlStr(d.volume_unit ?? 'attempts')}, ${sqlInt(d.default_sets ?? 3)}, ${sqlInt(d.default_reps)}, ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 75)}, ${d.default_distance != null ? sqlStr(String(d.default_distance)) : 'NULL'}, ${sqlInt(d.est_seconds_per_set ?? 100)}, ${sqlInt(d.default_rpe_min ?? 5)}, ${sqlInt(d.default_rpe_max ?? 7)})`
}).join(',\n')
seedSql += `
...
seedSql += INSERT_CARDS.map((card) => {
  const s = card.safety ?? {}
    return `  (${sqlStr(card.slug)}, ${sqlInt(s.risk_level ?? 1)}, ${sqlInt(s.impact_level ?? 1)}, ${sqlBool(s.requires_spotting)}, ${sqlStr(coachSupervisionValue(s))}, ${sqlTextArray(s.readiness_checks)}, ${sqlTextArray(s.contraindications)}, ${sqlTextArray(s.common_substitutions)})`
}).join(',\n')
seedSql += `
...
FROM (VALUES\n`
  seedSql += INSERT_CARDS.map((card) => {
    const guidance = card.scaling?.[ck] ?? card.scaling?.adult_beginner ?? 'Scale by reducing ball load, distance, rotation range, approach speed, or reactive demand before increasing volume.'
...
seedSql += `-- Medicine Balls & Slam Balls cluster validation education
...
VALUES (
  'validation_rule',
  'medicine_balls_slam_balls_readiness',
  NULL,
...
let hydrateSql = `-- Medicine Balls & Slam Balls 50-card full hydration (card v2).
-- IDEMPOTENT. Generated by scripts/generate-182-medicine-balls-slam-balls.mjs (migrations 186/187)

`

for (const card of CARDS) {
  const pairing = pairingFromCard(card)
  const media = {
    demo_video_sources: [],
    coaching_articles: [],
...
UPDATE coaching.exercise SET
  ...
  default_rest_seconds = ${sqlInt(d.default_rest_seconds ?? 75)},
  est_seconds_per_set = ${sqlInt(d.est_seconds_per_set ?? 100)},
...
SELECT e.id, sp.id, ${sqlInt(pp.fit_weight ?? 5)}, ${sqlStr(pp.role ?? 'primary')}, ${sqlStr(card.slot)}, pos.order_index,
  ${sqlBool(pp.freshness_required ?? true)}, ${sqlInt(pp.fatigue_sensitivity ?? 5)}, ${sqlInt(pp.fatigue_cost ?? 3)},
  ${sqlInt(pp.technical_complexity ?? 2)}, ${sqlInt(pp.impact_level ?? 2)}, ${sqlStr(pp.intensity_ceiling ?? 'moderate')}
FROM coaching.exercise e
JOIN coaching.session_phase sp ON sp.key = ${sqlStr(card.primaryPhaseKey)}
JOIN coaching.phase_order_slot pos ON pos.key = ${sqlStr(card.slot)} AND pos.phase_id = sp.id
...
UPDATE coaching.exercise_dosage_profile d SET
  ...
  default_rest_seconds = ${sqlInt(d.default_rest_seconds ?? 75)},
  est_seconds_per_set = ${sqlInt(d.est_seconds_per_set ?? 100)},
  default_rpe_min = ${sqlInt(d.default_rpe_min ?? 5)},
  default_rpe_max = ${sqlInt(d.default_rpe_max ?? 7)},
...
SELECT e.id, 'Default', TRUE, ${sqlStr(d.volume_unit ?? 'attempts')}, ${sqlInt(d.default_sets ?? 3)}, ${sqlInt(d.default_reps)},
  ${sqlInt(d.default_work_seconds)}, ${sqlInt(d.default_rest_seconds ?? 75)}, ${defaultDistance === 'NULL' ? 'NULL::integer' : defaultDistance},
  ${sqlInt(d.est_seconds_per_set ?? 100)}, ${sqlInt(d.default_rpe_min ?? 5)}, ${sqlInt(d.default_rpe_max ?? 7)}
FROM coaching.exercise e
WHERE e.slug = ${sqlStr(card.slug)}
...
UPDATE coaching.exercise_safety_profile s SET
  ...
  requires_coach_supervision = ${sqlStr(coachSupervisionValue(s))},
...
SELECT e.id, ${sqlInt(s.risk_level ?? 1)}, ${sqlInt(s.impact_level ?? 1)}, ${sqlBool(s.requires_spotting)},
  ${sqlStr(coachSupervisionValue(s))},
...
const seedPath = path.join(__dirname, '../backend/migrations/186_coaching_medicine_balls_slam_balls_infrastructure_and_seed.sql')
const hydratePath = path.join(__dirname, '../backend/migrations/187_coaching_medicine_balls_slam_balls_cards.sql')
fs.writeFileSync(seedPath, seedSql)
fs.writeFileSync(hydratePath, hydrateSql)
console.log('Wrote', seedPath)
console.log('Wrote', hydratePath, '—', CARDS.length, 'medicine-ball/slam-ball cards (', INSERT_CARDS.length, 'insert,', MERGE_COUNT, 'merge-only)')
