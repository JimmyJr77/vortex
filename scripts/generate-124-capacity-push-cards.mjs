/**
 * Generates backend/migrations/124_coaching_capacity_push_cards.sql
 * Run: node scripts/generate-124-capacity-push-cards.mjs
 */
import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import { CAPACITY_PUSH_CARDS, CAPACITY_PUSH_SLUGS } from './data/capacity-push-cards-19-26.mjs'
...
const VALIDATION_EDU = {
  title: 'Upper-Body Push Capacity without stealing hand-support quality',
  short_summary:
...
let sql = `-- Capacity phase Upper-Body Push Strength cards 19–26 (rich card v2 content).
...
for (const card of CAPACITY_PUSH_CARDS) {
  const pairing = {
...
sql += `-- Refresh weighted taxonomy tags for upper-body push Capacity cards
DELETE FROM coaching.exercise_tag t
USING coaching.exercise e
WHERE t.exercise_id = e.id
  AND e.slug IN (${CAPACITY_PUSH_SLUGS.map(sqlStr).join(', ')})
...
FROM (VALUES\n`
  sql += CAPACITY_PUSH_CARDS.map((c) => {
    const guidance = c.scaling[ck] ?? c.scaling.adult_beginner ?? 'Conservative push dose — prioritize mechanics and full rest.'
...
const outPath = path.join(__dirname, '../backend/migrations/124_coaching_capacity_push_cards.sql')
fs.writeFileSync(outPath, sql)
console.log('Wrote', outPath, '—', CAPACITY_PUSH_CARDS.length, 'upper-body push Capacity cards')
